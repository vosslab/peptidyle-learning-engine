...
run "security_baseline_plan" {
  ...
  assert {
    condition     = strcontains(aws_iam_role_policy.api_execution_secrets.policy, var.api_application_secrets_arn) && strcontains(aws_iam_role_policy.api_execution_secrets.policy, var.api_application_secrets_kms_key_arn) && !strcontains(aws_iam_role_policy.api_execution_secrets.policy, var.worker_application_secrets_arn) && !strcontains(aws_iam_role_policy.api_execution_secrets.policy, var.publisher_application_secrets_arn) && !strcontains(aws_iam_role_policy.api_execution_secrets.policy, var.worker_application_secrets_kms_key_arn) && !strcontains(aws_iam_role_policy.api_execution_secrets.policy, var.publisher_application_secrets_kms_key_arn)
    error_message = "API execution identity must be bound only to the API secret ARN and CMK."
  }
  assert {
