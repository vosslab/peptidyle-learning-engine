...
run "security_baseline_plan" {
  ...
  assert {
    condition     = strcontains(aws_iam_role_policy.publisher_storage.policy, "/questions/*/versions/*/restricted-assets/*") && !strcontains(aws_iam_role_policy.publisher_storage.policy, "/workspaces/")
    error_message = "Publisher private reads must be limited to the typed restricted Question Asset path."
  }
  assert {
