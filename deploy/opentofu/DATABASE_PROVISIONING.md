...
3. Create three Secrets Manager JSON values: the API value holds only the names
   ...
   grader URL when needed. The publisher value holds only `PLE_PUBLISHER_DATABASE_URL`.
   Each JSON value has its own CMK; its key policy permits only the matching
   ECS execution role through Secrets Manager (`kms:ViaService` and the exact
   secret encryption context), never the RDS master-secret CMK. Each URL uses `sslmode=verify-full`, exact login
   name, RDS endpoint hostname, and the reviewed RDS CA root bundled into the
   application image.
4. Before service rollout, probe TLS certificate validation, execute the
   application pool capability check, prove RLS isolation with both task
   login URLs, and rotate the RDS master secret independently of either app
   secret.
