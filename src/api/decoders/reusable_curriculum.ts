...
function completionRequirement(value: unknown, path: string): unknown {
  const record = decodeRecord(value, path);
...
function continuedPractice(value: unknown, path: string): unknown {
  const record = decodeRecord(value, path);
  const kind = decodeString(field(record, "kind", path), `${path}.kind`);
...
function defaults(value: unknown, path: string): unknown {
  ...
  return {
    time_limit_seconds: decodeNullable(
      field(record, "time_limit_seconds", path),
    ...
    late_submission: decodeStringEnum(
      field(record, "late_submission", path),
      `${path}.late_submission`,
      ["accept", "markLate", "reject"],
    ),
    deadline_behavior: decodeStringEnum(
      field(record, "deadline_behavior", path),
      `${path}.deadline_behavior`,
      ["autoSubmit"],
    ),
    activity_rules: {
      completion: completionRequirement(
        field(policies, "completion", `${path}.activity_rules`),
        `${path}.activity_rules.completion`,
      ),
      grade: decodeStringEnum(
        field(policies, "grade", `${path}.activity_rules`),
        `${path}.activity_rules.grade`,
        ["first", "latest", "highest", "instructorSelected"],
      ),
      continuedPractice: continuedPractice(
        field(policies, "continuedPractice", `${path}.activity_rules`),
        `${path}.activity_rules.continuedPractice`,
      ),
...
function assignmentEntry(
  ...
  if (kind === "fixed") {
    requireOnlyFields(record, path, ["kind", "question_id", "points_possible", "scoring_mode"]);
    ...
    decodeStringEnum(field(record, "scoring_mode", path), `${path}.scoring_mode`, [
      "normal",
      "fullCredit",
      "extraCredit",
      "excluded",
...
function questionView(value: unknown, path: string): void {
  ...
  const evidence = decodeRecord(
    field(catalog, "evidence", `${path}.catalog`),
    `${path}.catalog.evidence`,
  );
  const state = decodeStringEnum(
    field(evidence, "state", `${path}.catalog.evidence`),
    `${path}.catalog.evidence.state`,
    ["insufficientEvidence", "available"],
  );
  if (state === "insufficientEvidence") {
    requireOnlyFields(evidence, `${path}.catalog.evidence`, ["state"]);
  }
  decodeStringEnum(
