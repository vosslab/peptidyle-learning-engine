...
export const RESPONSE_WIDGET_STYLES = `
  .response-widget { display: grid; gap: var(--ple-space-3, 0.75rem); }
  .response-widget fieldset { display: grid; gap: var(--ple-space-2, 0.5rem); min-width: 0; border: 0; margin: 0; padding: 0; }
  ...
  .response-widget .primary-action, .response-widget .quiet-action { min-height: var(--ple-response-min-height, 56px); }
  .response-widget .response-control { box-sizing: border-box; width: 100%; padding: 0.65rem 0.75rem; }
  .response-widget .choice-list, .response-widget .ordering-list { display: grid; gap: 0.5rem; }
  .response-widget .choice-card { display: flex; align-items: center; gap: 0.65rem; box-sizing: border-box; padding: 0.6rem 0.75rem; border: 2px solid var(--ple-border); border-radius: var(--ple-radius-control, 0.375rem); background: var(--ple-card-surface); color: inherit; cursor: pointer; font-weight: 650; }
  ...
  .response-widget .choice-card input { width: 1.25rem; height: 1.25rem; margin: 0; accent-color: var(--ple-accent-strong); }
  .response-widget .choice-card input:focus-visible { outline: 0; }
  .response-widget .choice-number { display: inline-grid; width: 1.7rem; height: 1.7rem; place-items: center; border-radius: var(--ple-radius-control, 0.375rem); background: var(--ple-surface-soft); color: var(--ple-accent-strong); font-size: 0.82rem; font-weight: 820; }
  ...
  .response-widget .format-status { min-height: 2.75rem; padding: 0.55rem 0.75rem; border-left: 4px solid var(--ple-accent); background: var(--ple-surface-soft); color: var(--ple-accent-strong); font-weight: 680; }
  ...
  .response-widget > .primary-action, .response-widget > .quiet-action { width: 100%; }
  ...
  @media (max-width: 360px) {
    .response-widget .ordering-row { grid-template-columns: minmax(0, 1fr) 56px 56px; }
    .response-widget .choice-card { padding-inline: 0.65rem; }
  }
`;
