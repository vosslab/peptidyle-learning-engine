...
export const FEEDBACK_PANEL_STYLES = `
  .feedback-panel { display: grid; gap: var(--ple-space-2, 0.5rem); padding: 0.5rem 0.625rem; border: 0; border-left: 4px solid var(--ple-success); border-radius: 0; background: color-mix(in srgb, var(--ple-success) 7%, white); }
  .feedback-panel__heading { margin: 0; }
  .feedback-panel__status, .feedback-panel__score, .feedback-panel__empty { margin: 0; }
  .feedback-panel__section { display: grid; gap: var(--ple-space-2, 0.5rem); padding-top: var(--ple-space-2, 0.5rem); border-top: 1px solid var(--ple-border); }
  ...
  .feedback-panel__advance { min-height: var(--ple-response-min-height, 2.75rem); width: auto; justify-self: start; padding: 0.35rem 0.65rem; }
  @media (max-width: 30rem) { .feedback-panel { padding: 0.5rem; } }
