...
function scopeCopy(scope: ModifierScope): string {
  if (scope === "scheduleOffset") return "Group schedule offsets";
  if (scope === "groupAccommodation") return "Group accommodations";
  return "Individual exceptions";
}
...
function namedDeleteCopy(scope: ModifierScope, name: string): string {
  return `Remove the ${scopeCopy(scope).toLowerCase()} modifier for ${name}?`;
}

/**
 * This page deliberately takes a narrow preview-subject loader: ApiClient exposes group membership
 * but no course-wide membership list. The page never turns internal identifiers into learner labels.
 */
export function AssignmentAccessPage(props: AssignmentAccessPageProps): JSX.Element {
  ...
  const [scope, setScope] = createSignal<ModifierScope>("scheduleOffset");
  ...
  async function load(): Promise<void> {
    ...
    setMessage("");
    try {
      const groupPage = await props.client.listCourseGroups(props.courseId, undefined, 100);
      setGroups(groupPage.groups);
      setState("ready");
      void loadSubjects();
    } catch (error: unknown) {
  ...
  async function saveOffset(group: CourseGroupSummaryView, value: string): Promise<boolean> {
    const offsetSeconds = Number(value);
    return mutate(() =>
      props.client.putGroupScheduleOffset(
        props.courseId,
        props.assignmentId,
        group.reference,
        { offsetSeconds },
  ...
  async function savePolicy(
    kind: "groupAccommodation" | "individualException",
    target: string,
    mode: ModifierMode,
    draft: ModifierPatchDraft,
  ): Promise<boolean> {
    const request = (await import("./assignment_access/model")).policyRequest(mode, draft);
    if (kind === "groupAccommodation")
      return mutate(() =>
        props.client.putGroupAccommodation(
          props.courseId,
          props.assignmentId,
          target,
          request,
    ...
    return mutate(() =>
      props.client.putIndividualPolicyException(
        props.courseId,
        props.assignmentId,
        target,
        request,
  ...
  async function deleteModifier(
    ...
    if (!window.confirm(namedDeleteCopy(kind, name))) return false;
    if (kind === "scheduleOffset")
      return mutate(() =>
        props.client.deleteGroupScheduleOffset(
          props.courseId,
          props.assignmentId,
          target,
          revision(),
    ...
    else if (kind === "groupAccommodation")
      return mutate(() =>
        props.client.deleteGroupAccommodation(
          props.courseId,
          props.assignmentId,
          target,
          revision(),
    ...
    return mutate(() =>
      props.client.deleteIndividualPolicyException(
        props.courseId,
        props.assignmentId,
        target,
        revision(),
        ...
        <Match when={state() === "ready"}>
          <div class="assignment-access-grid">
            ...
            >
              ...
              <p>Choose the scope, then define an explicit schedule or policy modifier.</p>
              <div class="assignment-access-scope-list" role="group" aria-label="Modifier scope">
                <For
                  each={["scheduleOffset", "groupAccommodation", "individualException"] as const}
                >
                  {(choice) => (
                    <button
                      type="button"
              ...
              <Show when={scope() === "individualException" && !canPreview()}>
                <p class="assignment-access-help">
                  {subjectsState() === "loading"
                    ? "Loading authorized learner choices..."
                    : "An authorized course-members loader is needed to select learners by safe display name. This route does not expose raw membership references."}
                </p>
                <Show when={subjectsState() === "error"}>
                  <button type="button" onClick={() => void loadSubjects()}>
                    Retry learner choices
                  </button>
                </Show>
              </Show>
              <div class="assignment-access-actions">
                <button
                  type="button"
                  disabled={busy() || (scope() === "individualException" && !canPreview())}
                  onClick={(event) => {
                    dialogTrigger = event.currentTarget;
                    setDialogOpen(true);
                  }}
                >
                  Add or change {scopeCopy(scope()).toLowerCase()}
                </button>
              </div>
            ...
            >
              <h2 id="assignment-access-subject-heading">Preview a learner</h2>
              <Show
                when={canPreview()}
                fallback={
                  <p class="assignment-access-help">
                    {subjectsState() === "error"
                      ? "Learner choices could not load. Retry without losing modifier work."
