...
import { appendLibraryClassificationParameters, parseLibraryClassificationParameters } from "../api/library_classification_filter";
import { EMPTY_QUESTION_LIBRARY_BROWSE_QUERY, type QuestionLibraryBrowseQuery } from "./library_page_model";

function boundedValues(parameters: URLSearchParams, name: string): Array<string> {
  return parameters.getAll(name).filter((value) => value.trim().length > 0 && Array.from(value).length <= 256);
}
...
export function hasExactBrowseFilters(query: QuestionLibraryBrowseQuery): boolean {
  return query.discipline_uuid !== null || query.subjects.length > 0 || query.topics.length > 0 ||
    query.tag !== null || query.questionType !== null;
}
...
export function searchWithinResultsPath(query: QuestionLibraryBrowseQuery): string {
  ...
  for (const field of ["authorName", "backend", "tag", "questionType", "capability", "questionLicense", "usedInMyCourses"] as const) {
