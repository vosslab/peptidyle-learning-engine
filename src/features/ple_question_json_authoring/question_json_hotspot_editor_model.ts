// question_json_hotspot_editor_model.ts - immutable edits for keyboard-first HOTSPOT authoring.
...
export type PleQuestionJsonHotspotEditResult = {
  readonly response: PleQuestionJsonHotspotResponse;
  readonly changed: boolean;
  readonly error: string | null;
};

function changed(response: PleQuestionJsonHotspotResponse): PleQuestionJsonHotspotEditResult {
  return { response, changed: true, error: null };
}

function refused(
  response: PleQuestionJsonHotspotResponse,
  error: string,
): PleQuestionJsonHotspotEditResult {
...
function isInsideSurface(region: PleQuestionJsonHotspotRegion): boolean {
  return (
...
function nextRegionId(regions: ReadonlyArray<PleQuestionJsonHotspotRegion>): string {
  let suffix = 1;
...
function updatedRegion(
  response: PleQuestionJsonHotspotResponse,
  regionId: string,
  replacement: PleQuestionJsonHotspotRegion,
): PleQuestionJsonHotspotEditResult {
...
export function addHotspotRegion(
  response: PleQuestionJsonHotspotResponse,
): PleQuestionJsonHotspotEditResult {
  ...
  for (let y = 0; y < NORMALIZED_MAXIMUM; y += 1_000) {
    for (let x = 0; x < NORMALIZED_MAXIMUM; x += 1_000) {
      const region: PleQuestionJsonHotspotRegion = {
        id,
