// question_json_multi_fill_in_editor_model.ts - identity-safe edits for MULTI-FIB authoring.
...
export type PleQuestionJsonMultiFillInEditResult = {
...
function changed(
  ...
  focusId: string | null,
  status: string | null,
): PleQuestionJsonMultiFillInEditResult {
...
function multiFillResponse(
  source: PleQuestionJsonDocument,
): Extract<PleQuestionJsonDocument["response"], { readonly kind: "multiFillIn" }> | null {
...
function replaceBlank(
  source: PleQuestionJsonDocument,
  blankId: string,
  replacement: PleQuestionJsonBlank,
): PleQuestionJsonMultiFillInEditResult {
...
export function addMultiFillBlank(
  source: PleQuestionJsonDocument,
): PleQuestionJsonMultiFillInEditResult {
  const response = multiFillResponse(source);
...
export function reorderMultiFillBlanks(
  ...
  const blanks: PleQuestionJsonBlank[] = [];
...
function nextBlankId(blanks: ReadonlyArray<PleQuestionJsonBlank>): string {
