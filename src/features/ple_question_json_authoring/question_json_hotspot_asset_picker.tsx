// question_json_hotspot_asset_picker.tsx - private, keyboard-first selection of immutable hotspot images.
...
import {
  ...
  type PleQuestionJsonAssetClient,
  type PleQuestionJsonAssetDescriptor,
} from "./question_json_asset_client";

type AssetPickerPhase = "ready" | "loading" | "uploading";

export interface PleQuestionJsonHotspotAssetPickerProps {
  readonly client: PleQuestionJsonAssetClient;
  readonly workspace: WorkspaceId;
  /** Stable instructional purpose transmitted with each author-selected image. */
  readonly purpose: string;
  /** Retains an existing source selection when this surface reloads. */
  readonly selectedAssetId?: string;
  readonly disabled?: boolean;
  readonly onSelect: (asset: PleQuestionJsonAssetDescriptor) => void;
}

function userMessage(error: unknown): string {
  if (error instanceof PleQuestionJsonAssetProtocolError) return error.message;
  if (error instanceof PleQuestionJsonAssetRequestError) {
    switch (error.status) {
      case 401:
...
export function PleQuestionJsonHotspotAssetPicker(
  props: PleQuestionJsonHotspotAssetPickerProps,
): JSX.Element {
  const [assets, setAssets] = createSignal<ReadonlyArray<PleQuestionJsonAssetDescriptor>>([]);
  ...
  function selectedAsset(): PleQuestionJsonAssetDescriptor | undefined {
  ...
  return (
    <fieldset
      class="ple-question-json-authoring__hotspot-assets"
      aria-describedby="hotspot-image-help"
    >
      <legend>Image</legend>
      <p class="ple-question-json-authoring__help" id="hotspot-image-help">
      ...
      <Show when={phase() === "loading"}>
        <p class="ple-question-json-authoring__help" role="status">
          Loading your image library...
        </p>
      </Show>
      <Show when={phase() === "ready" && assets().length === 0 && errorMessage() === null}>
        <p class="ple-question-json-authoring__help">No images yet. Add the first image below.</p>
      </Show>
      <Show when={errorMessage() !== null}>
        <div class="ple-question-json-authoring__error" role="alert">
          <p>{errorMessage()}</p>
          <button
            type="button"
            class="quiet-action"
      ...
      <div class="ple-question-json-authoring__asset-upload">
        <label class="ple-question-json-authoring__field">
