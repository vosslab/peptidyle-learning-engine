...
} from "./question_json_fill_in_model";
import type {
...
export interface PleQuestionJsonFillInEditorProps {
  readonly response: () => PleQuestionJsonFillInResponse;
  readonly onResponseChange: (response: PleQuestionJsonFillInResponse) => void;
  readonly disabled?: boolean;
  readonly fieldErrors?: Readonly<Record<string, string | undefined>>;
}

/** Each row preserves its exact authored text while showing concise, local recovery guidance. */
export function PleQuestionJsonFillInEditor(props: PleQuestionJsonFillInEditorProps): JSX.Element {
  ...
  return (
    <fieldset class="ple-question-json-authoring__fill-in">
      ...
      <Index each={props.response().answers}>
        {(answer, index) => {
          const errorId = `ple-question-json-fill-in-answer-${index}-error`;
          ...
          return (
            <div class="ple-question-json-authoring__choice">
              ...
              <Show when={error() !== undefined}>
                <p class="ple-question-json-authoring__error" id={errorId} role="alert">
                  {error()}
                </p>
              </Show>
              <button
