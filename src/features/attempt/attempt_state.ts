...
export interface AttemptStateMachineOptions {
  readonly context: AttemptContext;
...
function initialState(
  ...
  clock: AttemptClock,
  envelope: QuestionEnvelope | null = null,
): AttemptState {
  return {
    phase: "loading",
    context,
 ...
 * question family, page layout, answer key, or grading rule.
 */
export function createAttemptStateMachine(
  options: AttemptStateMachineOptions,
): AttemptStateMachine {
  ...
  function publish(next: AttemptState): void {
    if (disposed) return;
    current = next;
  ...
  function resumeAfterReauthentication(): void {
    ...
    const state = { ...base(), phase: "answering" as const } satisfies AttemptState;
    publish(state);
  }
  ...
  async function loadNextAttempt(loadNext: () => Promise<NextAttempt>): Promise<void> {
    ...
    const advancing = { ...base(), phase: "advancing" as const } satisfies AttemptState;
    publish(advancing);
    try {
