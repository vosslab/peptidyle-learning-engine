...
- ``# Changelog parsing and serialization`` -- DayBlock / Entry
  ...
  ``add_entry`` / ``parse_file``, ``newest_date`` / ``find_duplicate_dates``.
  ``parse_day_blocks`` / ``split_day_block`` are implemented in
  changelog_parse and re-exported here.
...
# Import after dataclasses so changelog_parse can use DayBlock and Entry.
# local repo modules
