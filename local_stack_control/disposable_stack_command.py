...
def parse_args(argv: list[str]) -> argparse.Namespace:
	...
	parser = argparse.ArgumentParser(
		prog="python3 -m local_stack_control.disposable_stack_command",
		description=__doc__,
	)
...
def main() -> None:
	...
	try:
		...
		if args.action == "restart":
			local_stack_control.disposable_stack_adapter.require_mutating_capability(runner, disposable)
			if args.service != local_stack_control.disposable_stack_adapter.outage_service(disposable):
				raise local_stack_control.models.ControllerError(
			...
			result = local_stack_control.lifecycle.restart_lifecycle(
				...
				root,
				args.service,
				local_stack_control.disposable_stack_adapter.restart_options(
					disposable, args.timeout_seconds
				),
			)
			print(f"Disposable stack ready: {result.gateway_url}")
