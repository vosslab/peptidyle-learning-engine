...
REVOKE ALL ON FUNCTION ple_api.update_blueprint_classification(text, bigint, uuid, uuid, uuid, uuid, text[]) FROM PUBLIC;
...
REVOKE ALL ON FUNCTION ple_api.load_blueprint_promotion(text),
    ple_api.set_blueprint_promotion(text, bigint, boolean) FROM PUBLIC;
