...
REVOKE ALL ON FUNCTION ple_api.mark_course_instance_inactive(text, timestamp with time zone),
    ple_api.archive_course_student_records(text, timestamp with time zone),
    ple_api.delete_course_student_records(text, timestamp with time zone) FROM PUBLIC;
