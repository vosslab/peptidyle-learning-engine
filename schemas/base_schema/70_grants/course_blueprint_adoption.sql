...
GRANT EXECUTE ON FUNCTION ple_api.load_course_instance_blueprint(text, bigint) TO ple_data_owner;

REVOKE ALL ON FUNCTION ple_api.load_course_instance_blueprint(text, bigint) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION ple_api.load_course_instance_blueprint(text, bigint) TO ple_app;

REVOKE ALL ON FUNCTION ple_api.load_blueprint_assessment_copy_source(text, bigint) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION ple_api.load_blueprint_assessment_copy_source(text, bigint) TO ple_data_owner;
...
REVOKE ALL ON FUNCTION ple_api.list_blueprint_daughter_course_ids(text),
    ple_api.append_new_blueprint_assessments(text, bigint, bigint, text, jsonb) FROM PUBLIC;
