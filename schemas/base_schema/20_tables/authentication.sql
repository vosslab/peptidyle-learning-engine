...
CREATE TABLE ple_private.passkey_ceremony (
    ...
    target_account_id ple_data.account_id REFERENCES ple_private.account (account_id),
    browser_binding_hash bytea NOT NULL CHECK (pg_catalog.octet_length(browser_binding_hash) = 32),
...
COMMENT ON COLUMN ple_private.passkey_ceremony.target_account_id IS 'NULL means this optional fact is absent.';
COMMENT ON COLUMN ple_private.passkey_ceremony.consumed_at IS 'NULL means this optional fact is absent.';
COMMENT ON COLUMN ple_private.passkey.last_used_at IS 'NULL means this optional fact is absent.';
COMMENT ON COLUMN ple_private.passkey.revoked_at IS 'NULL means this optional fact is absent.';
