...
CREATE TABLE ple_data.student_record (
    ...
    student_account_id ple_data.account_id NOT NULL REFERENCES ple_private.account (account_id),
...
CREATE TABLE ple_private.course_invitation_event (
    ...
    performed_by_account_id ple_data.account_id NOT NULL REFERENCES ple_private.account (account_id),
    occurred_at timestamp with time zone NOT NULL,
    reason text NOT NULL CHECK (char_length(btrim(reason)) BETWEEN 1 AND 1000)
);
...
CREATE TABLE ple_audit.course_roster_event (
    ...
    course_instance_id ple_data.course_instance_id NOT NULL REFERENCES ple_data.course_instance(course_instance_id),
    student_account_id ple_data.account_id NOT NULL REFERENCES ple_private.account(account_id),
    acting_account_id ple_data.account_id NOT NULL REFERENCES ple_private.account(account_id),
