...
CREATE FUNCTION ple_private.assessment_start_decision(
    ...
    p_completed_assessment_attempt_count integer,
...
BEGIN
    ...
    IF p_assessment_status IS NULL
       ...
       OR p_completed_assessment_attempt_count IS NULL
       OR p_completed_assessment_attempt_count < 0
    ...
    ELSIF p_assessment_attempt_limit IS NOT NULL
          AND p_completed_assessment_attempt_count >= p_assessment_attempt_limit THEN
        RETURN 'assessment_attempt_limit_reached';
...
DECLARE evaluation_time timestamptz := pg_catalog.statement_timestamp(); completed_assessment_attempt_count integer;
BEGIN
    ...
    assessment_attempt_limit := COALESCE(accommodation_row.assessment_attempt_limit, assessment_row.assessment_attempt_limit);
    ...
    SELECT count(*)::integer INTO completed_assessment_attempt_count
     ...
     WHERE assessment_attempt.student_record_id = student_record_id_value
       ...
       AND (assessment_attempt.completed_at IS NOT NULL
            ...
            OR (assessment_attempt.expires_at IS NOT NULL AND assessment_attempt.expires_at <= evaluation_time));
...
RETURNS TABLE (assessment_attempt_reference_number bigint, assessment_attempt_number integer, course_reference_number bigint, course_short_name text, course_long_name text, course_theme text, assessment_reference_number text, assessment_title text, display_time_zone text, expires_at_millis bigint, timer_remaining_milliseconds bigint)
LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, ple_api, ple_data, ple_private AS $$
...
BEGIN
    ...
    SELECT assessment_attempt.reference_number, assessment_attempt.assessment_attempt_number, course.course_reference_number,
     ...
     WHERE assessment_attempt.assessment_attempt_id = assessment_attempt_id_value
       AND (assessment_attempt.completed_at IS NULL OR EXISTS (
...
CREATE FUNCTION ple_api.read_student_assessment_access(text, text) RETURNS TABLE (start_decision text, assessment_title text, question_count integer, points_possible double precision, assessment_attempt_time_limit_seconds integer, available_at timestamptz, due_at timestamptz, closes_at timestamptz, assessment_attempt_limit integer, late_work_rule text, evaluated_at timestamptz, display_time_zone text, previous_assessment_attempts jsonb) LANGUAGE sql SECURITY DEFINER SET search_path = pg_catalog, ple_private, ple_api, ple_data AS $$ SELECT * FROM ple_private.read_student_assessment_access((SELECT reference_number FROM ple_data.course_instance WHERE public_reference = $1), $2) $$;
...
CREATE FUNCTION ple_api.read_student_assessment_attempt_context(bigint) RETURNS TABLE (assessment_attempt_reference_number bigint, assessment_attempt_number integer, course_reference_number bigint, course_short_name text, course_long_name text, course_theme text, assessment_reference_number text, assessment_title text, display_time_zone text, expires_at_millis bigint, timer_remaining_milliseconds bigint) LANGUAGE sql SECURITY DEFINER SET search_path = pg_catalog, ple_private, ple_api AS $$ SELECT * FROM ple_private.read_student_assessment_attempt_context($1) $$;
