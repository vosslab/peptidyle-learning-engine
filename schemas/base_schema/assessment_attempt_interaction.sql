...
FOR EACH ROW EXECUTE FUNCTION ple_private.reject_student_work_delete();
CREATE CONSTRAINT TRIGGER question_submission_matches_assessment_attempt AFTER INSERT OR UPDATE ON ple_private.question_submission
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION ple_private.enforce_question_submission_state();

CREATE INDEX assessment_submission_assessment_attempt_idx ON ple_private.assessment_submission(assessment_attempt_id);
CREATE INDEX question_submission_assessment_attempt_idx ON ple_private.question_submission(question_attempt_id);
...
REVOKE ALL ON TABLE ple_private.question_attempt, ple_private.assessment_attempt_saved_response,
    ple_private.question_submission, ple_private.assessment_submission FROM PUBLIC;
