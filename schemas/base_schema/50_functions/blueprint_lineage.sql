...
CREATE FUNCTION ple_api.list_known_blueprint_forks(p_source_blueprint_course_id text)
RETURNS TABLE (
...
DECLARE
    v_actor text;
    v_source_blueprint_course_id text;
BEGIN
    ...
    END IF;
    SELECT source.blueprint_course_id INTO v_source_blueprint_course_id
      FROM ple_data.blueprint_course AS source
     WHERE source.blueprint_course_id = p_source_blueprint_course_id
...
CREATE FUNCTION ple_api.load_blueprint_comparison_sources(
    p_left_blueprint_course_id text, p_right_blueprint_course_id text
) RETURNS TABLE (
...
BEGIN
    IF p_left_blueprint_course_id IS NULL OR p_right_blueprint_course_id IS NULL
       OR NOT ple_private.is_canonical_prefixed_public_id(p_left_blueprint_course_id, 'BP')
       OR NOT ple_private.is_canonical_prefixed_public_id(p_right_blueprint_course_id, 'BP')
    ...
    FOR v_locked_blueprint_course_id IN
        SELECT course.blueprint_course_id FROM ple_data.blueprint_course AS course
         WHERE course.blueprint_course_id IN (p_left_blueprint_course_id, p_right_blueprint_course_id)
    ...
    SELECT course.* INTO v_fork FROM ple_data.blueprint_course AS course
     WHERE course.blueprint_course_id = p_right_blueprint_course_id;
    IF NOT FOUND THEN RETURN; END IF;
    SELECT course.* INTO v_source FROM ple_data.blueprint_course AS course
     WHERE course.blueprint_course_id = p_left_blueprint_course_id;
    IF NOT FOUND THEN RETURN; END IF;
    ...
    SELECT inputs.position, inputs.blueprint_course_id, revision.blueprint_revision_number,
      ...
      FROM (VALUES
          (0, v_source.blueprint_course_id, v_source.current_blueprint_revision_number),
          (1, v_fork.blueprint_course_id, v_fork.current_blueprint_revision_number)
      ) AS inputs(position, blueprint_course_id, revision_number)
...
BEGIN
    ...
    LOOP
        ...
        ELSE
            PERFORM 1 FROM ple_data.blueprint_course WHERE blueprint_course_id = v_locked_blueprint_course_id FOR SHARE;
        END IF;
    END LOOP;
...
DECLARE
    ...
    v_child_blueprint_course_id text;
...
BEGIN
    ...
    SELECT * INTO v_source
     ...
     WHERE source_course.blueprint_course_id = v_source_blueprint_course_id
    ...
    INSERT INTO ple_data.blueprint_course_revision (
        ...
        v_child_blueprint_course_id, v_child_revision, p_content,
        p_content_checksum, v_now
    );
    INSERT INTO ple_data.blueprint_revision_question_pin
    SELECT v_child_blueprint_course_id, v_child_revision, pin.content_path,
    ...
    INSERT INTO ple_data.blueprint_revision_event (
        ...
        v_child_blueprint_course_id, v_child_revision, v_actor, p_request_checksum, v_now
    );
    ...
    SELECT course.blueprint_course_id INTO blueprint_course_id
      FROM ple_data.blueprint_course AS course
     WHERE course.blueprint_course_id = v_child_blueprint_course_id;
