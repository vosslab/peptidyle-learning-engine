-- Functions, triggers, and views from student_assessment_landing.sql.
          ...
          SELECT count(issued.issued_question_id)::bigint AS question_count,
            ...
            LEFT JOIN LATERAL (
                SELECT count(DISTINCT question_attempt.question_attempt_id) = 1
                           AND (
                               coalesce(bool_and(
                                   question_attempt.finalized_at IS NOT NULL
                                   AND saved.question_attempt_id IS NULL
                               ), false)
                               OR (
                                   count(DISTINCT result.grading_result_id) = 1
                  ...
                  LEFT JOIN ple_audit.automated_grading_receipt AS receipt
                    ON receipt.grading_result_id = result.grading_result_id
