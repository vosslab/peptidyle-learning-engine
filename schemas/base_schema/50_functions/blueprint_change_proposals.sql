...
CREATE FUNCTION ple_api.create_blueprint_change_proposal(
    p_source_blueprint_course_id text, p_source_blueprint_revision_number bigint, p_source_blueprint_edit_number bigint,
    p_target_blueprint_course_id text, p_target_blueprint_revision_number bigint, p_target_blueprint_edit_number bigint
...
BEGIN
    ...
    IF p_target_blueprint_revision_number IS DISTINCT FROM v_target.current_blueprint_revision_number
    ...
    INSERT INTO ple_data.blueprint_change_proposal VALUES (
        v_proposal_id, v_actor, v_source.blueprint_course_id, p_source_blueprint_revision_number,
        p_source_blueprint_edit_number, v_target.blueprint_course_id, p_target_blueprint_revision_number,
...
CREATE FUNCTION ple_api.lock_blueprint_change_proposal_acceptance(
    p_proposal_id uuid, p_target_blueprint_course_id text, p_expected_blueprint_revision_number bigint,
...
BEGIN
    ...
    END IF;
    IF p_expected_blueprint_revision_number IS DISTINCT FROM v_target.current_blueprint_revision_number
       OR p_expected_blueprint_edit_number IS DISTINCT FROM v_target.blueprint_edit_number
       OR p_expected_blueprint_revision_number IS DISTINCT FROM v_proposal.target_revision_number
...
DECLARE
    ...
    v_result_blueprint_revision_number bigint;
...
BEGIN
    PERFORM ple_api.lock_blueprint_change_proposal_acceptance(
        p_proposal_id, p_target_blueprint_course_id, p_expected_blueprint_revision_number, p_expected_blueprint_edit_number);
    ...
    IF p_content = v_prior.content AND p_content_checksum = v_prior.content_checksum THEN
        IF NOT v_metadata_changed THEN
            RAISE EXCEPTION 'Blueprint Proposal decision changes no canonical content'
                USING ERRCODE = '22023';
        END IF;
        v_result_blueprint_revision_number := p_expected_blueprint_revision_number + 1;
        INSERT INTO ple_data.blueprint_course_revision VALUES (
            v_target.blueprint_course_id, v_result_blueprint_revision_number,
            v_prior.content, v_prior.content_checksum, v_now);
        INSERT INTO ple_data.blueprint_revision_question_pin
        SELECT v_target.blueprint_course_id, v_result_blueprint_revision_number, content_path,
        ...
        SELECT v_target.blueprint_course_id, v_result_blueprint_revision_number, blueprint_module_id,
        ...
        UPDATE ple_data.blueprint_course SET current_blueprint_revision_number = v_result_blueprint_revision_number
         WHERE blueprint_course_id = v_target.blueprint_course_id;
    ELSE
        ...
        SELECT resulting_blueprint_revision_number INTO STRICT v_result_blueprint_revision_number
          FROM ple_api.save_blueprint_course(p_target_blueprint_course_id, p_expected_blueprint_revision_number,
              p_request_checksum, p_content, p_content_checksum);
    END IF;
    IF v_result_blueprint_revision_number <> p_expected_blueprint_revision_number + 1 THEN
        RAISE EXCEPTION 'Blueprint Proposal acceptance needs one successor' USING ERRCODE = '22023';
    END IF;
    SELECT blueprint_edit_number INTO STRICT v_blueprint_edit_number FROM ple_api.rename_blueprint_course(
        p_target_blueprint_course_id, p_expected_blueprint_edit_number, v_short_name, v_long_name);
    IF v_source_classification THEN
        SELECT blueprint_edit_number INTO STRICT v_blueprint_edit_number FROM ple_api.update_blueprint_classification(
            p_target_blueprint_course_id, v_blueprint_edit_number, v_source.content_discipline_id, v_source.content_subject_id,
    ...
    INSERT INTO ple_data.blueprint_change_proposal_acceptance VALUES (
        ...
        v_target.blueprint_course_id, v_result_blueprint_revision_number, v_blueprint_edit_number);
END
$$;
