-- Functions, triggers, and views from question_images.sql.

SET LOCAL ROLE ple_data_owner;

CREATE CONSTRAINT TRIGGER question_image_delivery_preserves_available_owner
AFTER INSERT OR UPDATE OR DELETE ON ple_data.question_image_delivery DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION ple_data.require_exact_available_object_delivery_owner();

SET LOCAL ROLE ple_private_owner;

CREATE FUNCTION ple_private.reject_question_image_publication_change()
...
BEGIN
    IF NEW.published_question_id IS DISTINCT FROM OLD.published_question_id OR NEW.revision_number IS DISTINCT FROM OLD.revision_number
       OR NEW.question_image_asset_id IS DISTINCT FROM OLD.question_image_asset_id OR NEW.source_object_record_id IS DISTINCT FROM OLD.source_object_record_id
       ...
       OR current_setting('ple.question_image_publication_activation', true) IS DISTINCT FROM OLD.job_id::text THEN
        RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'Question Image Publication is immutable apart from Pending to Ready';
    END IF;
    RETURN NEW;
END $$;

CREATE TRIGGER question_image_publication_is_immutable
BEFORE UPDATE OR DELETE ON ple_private.question_image_publication
FOR EACH ROW EXECUTE FUNCTION ple_private.reject_question_image_publication_change();
...
CREATE TRIGGER question_image_publication_has_trusted_objects_and_job
BEFORE INSERT OR UPDATE ON ple_private.question_image_publication
FOR EACH ROW EXECUTE FUNCTION ple_private.validate_question_image_publication();

CREATE FUNCTION ple_private.require_complete_question_image_publication_delivery()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = pg_catalog, ple_data, ple_private AS $$
DECLARE target_delivery_id uuid := COALESCE(NEW.object_delivery_id, OLD.object_delivery_id); publication ple_private.question_image_publication%ROWTYPE;
BEGIN
    SELECT * INTO publication FROM ple_private.question_image_publication
     WHERE question_image_publication.object_delivery_id = target_delivery_id;
    ...
    IF NOT EXISTS (SELECT 1 FROM ple_data.object_delivery delivery WHERE delivery.object_delivery_id=publication.object_delivery_id
       ...
       OR NOT EXISTS (SELECT 1 FROM ple_data.question_image_delivery image_delivery WHERE image_delivery.object_delivery_id=publication.object_delivery_id
        ...
        RAISE EXCEPTION USING ERRCODE='23514', MESSAGE='Question Image Publication requires exact state-matched delivery';
    END IF;
    RETURN NULL;
END $$;

CREATE CONSTRAINT TRIGGER question_image_publication_requires_complete_delivery
AFTER INSERT OR UPDATE OR DELETE ON ple_private.question_image_publication DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION ple_private.require_complete_question_image_publication_delivery();

SET LOCAL ROLE ple_data_owner;

CREATE CONSTRAINT TRIGGER object_delivery_preserves_question_image_publication
AFTER INSERT OR UPDATE OR DELETE ON ple_data.object_delivery DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION ple_private.require_complete_question_image_publication_delivery();

CREATE CONSTRAINT TRIGGER question_image_delivery_preserves_question_image_publication
AFTER INSERT OR UPDATE OR DELETE ON ple_data.question_image_delivery DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION ple_private.require_complete_question_image_publication_delivery();

SET LOCAL ROLE ple_private_owner;

CREATE FUNCTION ple_private.select_ready_question_image_renditions(p_published_question_id text, p_revision_number integer)
RETURNS TABLE(question_image_asset_id uuid, question_image_checksum text, rendition_checksum text, intrinsic_width integer, intrinsic_height integer)
LANGUAGE sql SECURITY DEFINER SET search_path = pg_catalog, ple_data, ple_private AS $$
    SELECT publication.question_image_asset_id, encode(publication.source_object_checksum,'hex'), encode(publication.public_object_checksum,'hex'), publication.intrinsic_width, publication.intrinsic_height
    FROM ple_private.question_image_publication publication
    ...
    JOIN ple_data.question_image_delivery image_delivery ON image_delivery.object_delivery_id=publication.object_delivery_id AND image_delivery.question_image_asset_id=publication.question_image_asset_id
    WHERE publication.published_question_id=p_published_question_id AND publication.revision_number=p_revision_number
      AND publication.publication_state='ready' AND delivery.delivery_state='available' ORDER BY publication.question_image_asset_id
$$;

SET LOCAL ROLE ple_api_owner;

CREATE FUNCTION ple_api.select_ready_question_image_renditions(p_published_question_id text, p_revision_number integer)
RETURNS TABLE(question_image_asset_id uuid, question_image_checksum text, rendition_checksum text, intrinsic_width integer, intrinsic_height integer)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = pg_catalog, ple_api, ple_private AS $$
    SELECT * FROM ple_private.select_ready_question_image_renditions(p_published_question_id, p_revision_number)
$$;
...
CREATE FUNCTION ple_private.load_question_fork_asset(
    ...
    question_image_asset_id uuid, object_record_id uuid, object_address jsonb, sha256 bytea,
...
SET search_path = pg_catalog, ple_api, ple_data, ple_private AS $$
    SELECT publication.question_image_asset_id, record.object_record_id, record.object_address,
