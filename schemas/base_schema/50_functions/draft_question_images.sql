-- Functions, triggers, and views from draft_question_images.sql.

SET LOCAL ROLE ple_private_owner;

CREATE FUNCTION ple_private.validate_draft_question_image()
RETURNS trigger LANGUAGE plpgsql SET search_path = pg_catalog, ple_private AS $$
BEGIN
    -- ASVS 2.2.2, 5.3.2: persisted facts must use this exact server-owned semantic address.
    IF TG_OP = 'UPDATE' THEN
        RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'Draft Question Image is immutable';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM ple_private.object_record AS record
        WHERE record.object_record_id = NEW.source_object_record_id
          AND record.object_address = jsonb_build_object('kind', 'draftQuestionImage',
              'workspaceId', NEW.authoring_workspace_id, 'draftQuestionId', NEW.draft_question_id,
        ...
        RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'Draft Question Image requires exact immutable raster evidence';
    END IF;
    RETURN NEW;
END $$;

CREATE TRIGGER draft_question_image_has_exact_immutable_evidence
BEFORE INSERT OR UPDATE ON ple_private.draft_question_image
FOR EACH ROW EXECUTE FUNCTION ple_private.validate_draft_question_image();

CREATE FUNCTION ple_private.register_draft_question_image(
    p_draft_question_uuid uuid, p_expected_draft_question_edit_number bigint, p_question_image_asset_id uuid,
...
BEGIN
    IF p_draft_question_uuid IS NULL OR p_question_image_asset_id IS NULL OR p_object_record_id IS NULL OR p_sha256 IS NULL OR octet_length(p_sha256) <> 32
       ...
       OR p_width <= 0 OR p_height <= 0 OR p_width::bigint * p_height > 20000000 THEN
        RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'Draft Question Image arguments are invalid';
    END IF;
    ...
    END IF;
    expected_address := jsonb_build_object('kind', 'draftQuestionImage', 'workspaceId', draft.authoring_workspace_id,
        'draftQuestionId', draft.draft_question_id, 'questionImageAssetId', p_question_image_asset_id, 'objectId', p_object_record_id);
    IF p_object_address IS DISTINCT FROM expected_address THEN
        RAISE EXCEPTION USING ERRCODE = '22023', MESSAGE = 'Draft Question Image address is not its exact owner';
    END IF;
    ...
    INSERT INTO ple_private.draft_question_image VALUES
        (draft.draft_question_id, draft.authoring_workspace_id, p_question_image_asset_id, p_object_record_id, p_width, p_height);
END $$;

CREATE FUNCTION ple_private.load_draft_question_image(p_draft_question_uuid uuid, p_question_image_asset_id uuid)
...
CREATE FUNCTION ple_api.register_draft_question_image(
    p_draft_question_uuid uuid, p_expected_draft_question_edit_number bigint, p_question_image_asset_id uuid,
    p_object_record_id uuid, p_object_address jsonb, p_sha256 bytea, p_size_bytes bigint,
    p_media_type text, p_created_at_millis bigint, p_width integer, p_height integer
) RETURNS void LANGUAGE sql SECURITY DEFINER SET search_path = pg_catalog, ple_private AS $$
    SELECT ple_private.register_draft_question_image(p_draft_question_uuid,p_expected_draft_question_edit_number,p_question_image_asset_id,
        p_object_record_id,p_object_address,p_sha256,p_size_bytes,p_media_type,p_created_at_millis,p_width,p_height)
$$;

CREATE FUNCTION ple_api.load_draft_question_image(p_draft_question_uuid uuid, p_question_image_asset_id uuid)
RETURNS TABLE(object_record_id uuid, object_address jsonb, sha256 bytea, size_bytes bigint,
    media_type text, created_at_millis bigint, intrinsic_width integer, intrinsic_height integer)
