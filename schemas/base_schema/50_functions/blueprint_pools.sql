...
RETURNS TABLE (public_reference text, revision_number bigint, blueprint_edit_number bigint, changed boolean,
...
CREATE FUNCTION ple_api.fork_blueprint_question_pool(
    p_source_public_id text, p_child_id text, p_child_public_id text
...
BEGIN
    ...
    SELECT entry #>> '{question_pool_revision,questionPoolId}' INTO pin_id
...
BEGIN
    ...
    FOR assessment_value IN SELECT assessment FROM jsonb_array_elements(NEW.content -> 'modules') AS module,
        ...
        FOR entry_value IN SELECT entry FROM jsonb_array_elements(assessment_value #> '{content,entries}') AS entry
            ...
            END IF;
            SELECT entry #>> '{question_pool_revision,questionPoolId}' INTO prior_id
