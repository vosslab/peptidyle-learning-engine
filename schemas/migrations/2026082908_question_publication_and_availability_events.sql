...
-- the complete Question Version availability workflow.
...
CREATE TABLE ple_data.question_publication_event (
    ...
    CONSTRAINT question_publication_event_version_matches FOREIGN KEY (question_id, version_number)
        REFERENCES ple_data.published_question_version (question_id, version_number),
    CONSTRAINT question_publication_event_version_is_unique UNIQUE (question_id, version_number)
);
CREATE TABLE ple_data.question_version_availability_event (
    event_id uuid PRIMARY KEY,
    question_id text NOT NULL,
    version_number integer NOT NULL CHECK (version_number > 0),
    ...
    CONSTRAINT question_version_availability_reason_matches_event CHECK (
        (availability = 'available' AND reason IS NULL)
        OR (availability = 'archived' AND char_length(btrim(reason)) BETWEEN 1 AND 1000)
    ),
    CONSTRAINT question_version_availability_event_version_matches FOREIGN KEY (question_id, version_number)
        REFERENCES ple_data.published_question_version (question_id, version_number),
...
BEGIN
    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtextextended(NEW.question_id || ':' || NEW.version_number::text, 0)
    );
    IF NEW.availability = 'archived' AND NOT EXISTS (
        SELECT 1 FROM ple_data.question_version_availability_event AS available_event
        WHERE available_event.question_id = NEW.question_id
          AND available_event.version_number = NEW.version_number
          AND available_event.availability = 'available'
    ) THEN
        RAISE EXCEPTION USING ERRCODE = '23514',
            MESSAGE = 'a Question Version must be Available before it is Archived';
    END IF;
    RETURN NEW;
END
$$;
CREATE TRIGGER question_version_availability_event_has_valid_transition
BEFORE INSERT ON ple_data.question_version_availability_event
FOR EACH ROW EXECUTE FUNCTION ple_data.validate_question_version_availability_event();
CREATE INDEX question_version_availability_current_idx
    ON ple_data.question_version_availability_event (question_id, version_number, occurred_at DESC);
