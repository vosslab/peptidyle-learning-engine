-- WP-INST-G2 / G2-W3B: align the broker's transient rowset with PostgreSQL naming.
--
-- Migration 1872 materialized lowerCamelCase browser-shaped JSON and then decoded
-- it through snake_case SQL record columns. PostgreSQL matches jsonb_to_recordset
-- keys by exact name, so the typed result contained NULLs. Keep the transient
-- broker rowset in PostgreSQL's snake_case vocabulary; the durable audit payload
-- retains its lowerCamelCase serialized contract.

BEGIN;

CREATE OR REPLACE FUNCTION public.ple_inspect_student_work_v1(
    p_tenant_id uuid,
...
BEGIN
    ...
    SELECT jsonb_agg(jsonb_build_object(
        ...
        'score_visible', CASE witness.score_disclosure
            ...
            WHEN 'after_due' THEN
                witness.resolved_due_at IS NOT NULL
                AND v_occurred_at >= witness.resolved_due_at
            WHEN 'after_close' THEN
                witness.resolved_closes_at IS NOT NULL
