-- Verified email replacement must retire every bearer proof for the account.
-- The capability consumes the browser-bound challenge itself, so ple_auth
-- cannot invoke a user-wide session revocation with an arbitrary UserId.

CREATE FUNCTION public.ple_complete_email_change_and_revoke_sessions(
    p_token_hash bytea,
...
BEGIN
    ...
    DELETE FROM public.email_authentication_challenge
     WHERE token_hash = p_token_hash
       ...
       AND purpose_user_id = p_user_id
     RETURNING normalized_email, delivery_email, rate_limit_key_hash
    ...
    DELETE FROM public.account_authentication_session WHERE user_id = p_user_id;
    ...
    DELETE FROM public.authentication_rate_limit
     WHERE limit_scope = 'email' AND key_hash = v_rate_limit_key_hash;
...
REVOKE ALL ON FUNCTION public.ple_complete_email_change_and_revoke_sessions(
    bytea, bytea, uuid, bytea, bigint
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.ple_complete_email_change_and_revoke_sessions(
    bytea, bytea, uuid, bytea, bigint
) TO ple_auth;

-- The broker's only new authority is consumed inside the challenge-verifying
-- capability above; the API login retains no direct cross-session access.
GRANT SELECT, UPDATE ON public.ple_account TO ple_enrollment_broker;
GRANT SELECT, DELETE ON public.email_authentication_challenge TO ple_enrollment_broker;
GRANT SELECT, DELETE ON public.authentication_rate_limit TO ple_enrollment_broker;
GRANT SELECT, INSERT, DELETE ON public.account_authentication_session TO ple_enrollment_broker;
GRANT SELECT, UPDATE ON public.auth_session TO ple_enrollment_broker;
