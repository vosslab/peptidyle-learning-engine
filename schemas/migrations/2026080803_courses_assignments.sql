...
ALTER TABLE public.enrollment
    ADD CONSTRAINT enrollment_user_required_check CHECK ((user_id IS NOT NULL));
...
-- Cascade from course_group needs the parent-key prefix; the user lookup above does not provide it.
CREATE INDEX course_group_member_group_idx ON public.course_group_member
    USING btree (tenant_id, course_id, course_group_id);

CREATE INDEX assignment_policy_exception_group_idx ON public.assignment_policy_exception
    USING btree (tenant_id, course_id, course_group_id)
    WHERE (course_group_id IS NOT NULL);

CREATE INDEX assignment_selection_candidate_group_fk_idx ON public.assignment_selection_candidate
    USING btree (tenant_id, assignment_id, selection_group_id);
...
ALTER TABLE ONLY public.assignment
    ADD CONSTRAINT assignment_course_fk FOREIGN KEY (tenant_id, course_id) REFERENCES public.course(tenant_id, course_id);
