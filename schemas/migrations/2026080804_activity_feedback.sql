...
CREATE TABLE public.submission_evaluation (
    ...
    CONSTRAINT submission_evaluation_result_shape_check CHECK ((((grading_status = 'needs_manual_grading'::text) AND (credit_fraction IS NULL) AND (correct IS NULL)) OR ((grading_status = ANY (ARRAY['graded'::text, 'exempt'::text])) AND (credit_fraction IS NOT NULL) AND (correct IS NOT NULL)))),
    CONSTRAINT submission_evaluation_revision_check CHECK ((evaluation_revision > 0))
);

ALTER TABLE ONLY public.submission_evaluation FORCE ROW LEVEL SECURITY;

CREATE TABLE public.manual_grade_receipt (
    tenant_id uuid NOT NULL,
    manual_grade_action_id uuid NOT NULL,
    attempt_id uuid NOT NULL,
    actor_id uuid NOT NULL,
    request_sha256 character(64) NOT NULL,
    expected_evaluation_revision bigint NOT NULL,
    resulting_evaluation_revision bigint NOT NULL,
    scoring_generation bigint NOT NULL,
    occurred_at timestamp with time zone DEFAULT transaction_timestamp() NOT NULL,
    course_id uuid NOT NULL,
    CONSTRAINT manual_grade_receipt_expected_revision_check CHECK ((expected_evaluation_revision > 0)),
    CONSTRAINT manual_grade_receipt_resulting_revision_check CHECK ((resulting_evaluation_revision > 0)),
    CONSTRAINT manual_grade_receipt_revision_step_check CHECK ((resulting_evaluation_revision = (expected_evaluation_revision + 1))),
    CONSTRAINT manual_grade_receipt_scoring_generation_check CHECK ((scoring_generation > 0))
);

ALTER TABLE ONLY public.manual_grade_receipt FORCE ROW LEVEL SECURITY;
...
ALTER TABLE ONLY public.manual_grade_receipt
    ADD CONSTRAINT manual_grade_receipt_pkey PRIMARY KEY (tenant_id, manual_grade_action_id);
...
ALTER TABLE public.question_attempt
    ADD CONSTRAINT question_attempt_position_required_check CHECK (((assignment_position IS NOT NULL) AND (assignment_position >= 0)));
...
ALTER TABLE public.submission
    ADD CONSTRAINT submission_attempt_required_check CHECK ((attempt_id IS NOT NULL));
...
ALTER TABLE public.submission
    ADD CONSTRAINT submission_idempotency_required_check CHECK ((idempotency_key IS NOT NULL));
...
CREATE INDEX manual_grade_receipt_evaluation_idx ON public.manual_grade_receipt
    USING btree (tenant_id, attempt_id);

CREATE INDEX manual_grade_receipt_course_idx ON public.manual_grade_receipt
    USING btree (tenant_id, course_id);
...
ALTER TABLE ONLY public.manual_grade_receipt
    ADD CONSTRAINT manual_grade_receipt_evaluation_fk FOREIGN KEY (tenant_id, attempt_id) REFERENCES public.submission_evaluation(tenant_id, attempt_id) ON DELETE RESTRICT;

ALTER TABLE public.manual_grade_receipt
    ADD CONSTRAINT manual_grade_receipt_course_fk FOREIGN KEY (tenant_id, course_id) REFERENCES public.course(tenant_id, course_id);
...
ALTER TABLE public.manual_grade_receipt ENABLE ROW LEVEL SECURITY;

CREATE POLICY manual_grade_receipt_tenant ON public.manual_grade_receipt USING ((tenant_id = public.ple_current_tenant())) WITH CHECK ((tenant_id = public.ple_current_tenant()));
...
GRANT SELECT,INSERT ON TABLE public.manual_grade_receipt TO ple_app;
GRANT SELECT,DELETE ON TABLE public.manual_grade_receipt TO ple_retention_broker;
...
BEGIN
    FOR partition_number IN 0..15 LOOP
        EXECUTE format(
            'CREATE TABLE IF NOT EXISTS public.%I PARTITION OF public.submission_idempotency '
...
SELECT public.ple_ensure_activity_partitions(
    -- Fixed baseline epoch keeps development, CI, and production partition names identical.
    DATE '2026-08-01',
    26
);
