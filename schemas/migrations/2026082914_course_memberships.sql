...
CREATE TABLE ple_data.course_membership (
    ...
    CONSTRAINT course_membership_account_role_matches FOREIGN KEY (user_id, role)
...
CREATE UNIQUE INDEX course_membership_current_role_idx
    ON ple_data.course_membership (course_id, user_id, role) WHERE revoked_at IS NULL;
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        JOIN ple_data.course_membership AS membership
         ...
         AND membership.user_id = course.assigned_instructor_user_id
...
AFTER INSERT OR UPDATE OF assigned_instructor_user_id, assigned_instructor_role ON ple_data.course_instance
...
AFTER UPDATE OF user_id, role, revoked_at OR DELETE ON ple_data.course_membership
DEFERRABLE INITIALLY DEFERRED FOR EACH ROW
EXECUTE FUNCTION ple_data.assert_assigned_instructor_membership();
RESET ROLE;
SET LOCAL ROLE ple_private_owner;
CREATE TABLE ple_private.course_invitation (
    ...
    course_id uuid NOT NULL REFERENCES ple_data.course_instance (course_id),
    target_user_id uuid NOT NULL REFERENCES ple_private.account (user_id),
    ...
    revoked_at timestamp with time zone,
    CONSTRAINT course_invitation_target_role_matches FOREIGN KEY (target_user_id, membership_role)
        REFERENCES ple_private.account (user_id, role),
