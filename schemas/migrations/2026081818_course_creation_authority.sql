-- WP-PROF-T4: closed course and Base Course provisioning capability.
-- ASVS 1.2.4, 1.5.2, 2.1.1-2.1.3, 2.2.1-2.2.3, 2.3.1-2.3.4.
BEGIN;
...
BEGIN
 ...
 IF (SELECT count(*) FROM public.assignment_run)<>2 OR NOT EXISTS(SELECT 1 FROM public.assignment_run WHERE tenant_id=p_tenant AND run_id=mr AND enrollment_id=me AND run_number=1 AND completed_at IS NOT NULL AND payload_sha256~'^[0-9a-f]{64}$' AND payload->>'mode'='assigned' AND payload->>'variation'='newSeeds' AND (payload->>'score')::numeric=1) OR NOT EXISTS(SELECT 1 FROM public.assignment_run WHERE tenant_id=p_tenant AND run_id=jr AND enrollment_id=je AND run_number=1 AND completed_at IS NULL AND payload_sha256~'^[0-9a-f]{64}$' AND payload->>'mode'='assigned' AND payload->>'variation'='newSeeds' AND payload->'score'='null') OR (SELECT count(*) FROM public.assignment_run_item WHERE assignment_item_id=item AND source_position=0 AND issued_position=0 AND problem_id=pid AND version_id=vid AND selection_group_id IS NULL AND selection_seed IS NULL AND delivery_status='issued')<>2 OR (SELECT count(*) FROM public.question_attempt)<>2 OR NOT EXISTS(SELECT 1 FROM public.question_attempt WHERE tenant_id=p_tenant AND attempt_id=ma AND run_id=mr AND problem_id=pid AND version_id=vid AND attempt_status='submitted' AND submitted_at IS NOT NULL AND assignment_position=0 AND course_id=bc AND presentation_descriptor_version=1 AND octet_length(presentation_nonce)=16 AND octet_length(presentation_digest)=32 AND presentation_capability='envelope_v1' AND flat_grading_required=false AND flat_grading_payload IS NULL AND webwork_grading_required=false AND webwork_grading_payload IS NULL AND external_tool_indeterminate_at IS NULL) OR NOT EXISTS(SELECT 1 FROM public.question_attempt WHERE tenant_id=p_tenant AND attempt_id=ja AND run_id=jr AND problem_id=pid AND version_id=vid AND attempt_status='in_progress' AND submitted_at IS NULL AND assignment_position=0 AND course_id=bc AND presentation_descriptor_version=1 AND octet_length(presentation_nonce)=16 AND octet_length(presentation_digest)=32 AND presentation_capability='envelope_v1' AND flat_grading_required=false AND flat_grading_payload IS NULL AND webwork_grading_required=false AND webwork_grading_payload IS NULL AND external_tool_indeterminate_at IS NULL) THEN failure_kind:='completion_aggregate_incomplete';RETURN NEXT;RETURN; END IF;
...
GRANT EXECUTE ON FUNCTION public.ple_verify_base_course_completion_internal(uuid,uuid,text,jsonb) TO ple_base_course_install_broker,ple_base_course_completion_verification_broker;
    ...
    IF EXISTS (
        WITH expected(relation_name,policy_name,command,permissive,using_expression,check_expression) AS (
            SELECT format('%I.%I',namespace.nspname,table_row.relname),
              ...
              JOIN pg_catalog.pg_namespace AS namespace ON namespace.oid=table_row.relnamespace
             WHERE namespace.nspname='public' AND table_row.relkind IN ('r','p')
               AND table_row.relrowsecurity AND table_row.relname<>'_sqlx_migrations'
        ), actual AS (
