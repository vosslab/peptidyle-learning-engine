...
RAISE EXCEPTION 'Base Course recipe is invalid' USING ERRCODE='22023';
END;
IF bi IS NULL OR gi IS NULL OR bi=gi OR bi=ANY(ARRAY[e,m,j,a,o]) OR gi=ANY(ARRAY[e,m,j,a,o]) OR b->>'title'<>'Biochemistry: Protein Structure and Function' OR g->>'title'<>'Genetics Practice Course' OR b->>'termStart'<>'2026-01-01' OR g->>'termStart'<>'2026-01-01' OR b->>'termEnd'<>'2099-12-31' OR g->>'termEnd'<>'2099-12-31' OR b->>'timeZone'<>'America/Chicago' OR g->>'timeZone'<>'America/Chicago' OR b->>'initialInstructor' IS DISTINCT FROM e::text OR g->>'initialInstructor' IS DISTINCT FROM o::text THEN
RAISE EXCEPTION 'Base Course recipe is invalid' USING ERRCODE='22023';
END IF;
