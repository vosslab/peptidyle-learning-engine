...
- `receiptNext === null` now calls `machine.complete`, shows the summary, and
  loads the summary without a fresh Assignment Attempt screen read. The
  production completion fixture verifies zero fresh Assignment Attempt reads
  for pending, allowed, and closed summary policy.
- Historical code evidence: a nonnull receipt reached former
  `runtime.client.getRunScreen` inside `machine.advance`, so fetch failures
  entered the existing `advanceFailed` recovery path and `retryAdvance` could
  retry without resubmitting.
...
- `matchesIssuedSuccessor` compares the exact public receipt binding:
  Assignment Attempt ID, assignment position, question revision, seed,
  deadline, and rendered question hash. A fresh Assignment Attempt screen must
  pass that predicate before it can become a machine successor.
...
- The production component fixture covers `same`, wrong ID, Assignment Attempt,
  position, version, seed, deadline, and hash. Every case visibly exposes
  `Retry next question`, retains Position 1, and retains the selected radio.
  This closes both prior behavioral-test findings without inspecting answers or
  private state.

## Validation

- PASS: focused Prettier, TypeScript lint compile, ESLint, and `git diff --check`.
- Historical test paths: PASS: `node --import tsx --test tests/test_question_attempt_state.mjs tests/test_frontend_contract.mjs tests/test_run_page_recovery.mjs` (41 passed).
- Historical test paths: PASS: `npx playwright test tests/playwright/run_prefetch_route.spec.ts tests/playwright/run_completion_summary.spec.ts tests/playwright/frontend_contract.spec.ts` (32 passed).
