...
| C863-C869 | blocked future H5P chain; 0; no dispatch before this exact product answer: **Which H5P content type(s) and exact pinned library versions are supported first; for each which terminal xAPI event/score semantics are authoritative; are scoreless activities non-assessment only?** Rootless Node20 Lumi runtime uses pinned GPL manifest and private gateway/tickets; API owns auth/lifecycle/SHA256/result, runtime owns only bounded opaque interaction/xAPI and has no PLE credentials/egress. | decision -> `{C863,C864}` -> C865/C866 -> C867/C868 -> C869. No current A7 node claims H5P completion. | Ignored supply-chain, archive, ticket, xAPI, persistence, frame, and Podman proof only after decision; no permanent orchestration test. |
