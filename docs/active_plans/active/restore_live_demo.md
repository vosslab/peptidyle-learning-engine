...
The current developer showcase is not a restored destination. The first real
Instructor, Student, and Sysadmin tasks replace it as the post-selector path;
the showcase does not remain an alternate completion surface or a fallback for
an unavailable teaching capability.
...
| --- | --- | --- |
| M0 | complete | [foundation findings report](../audits/live_demo_foundation_findings.md); fresh disposable acceptance passed on 2026-09-06, but it is not browser evidence |
...
Obvious follow-ons: retain Instructor and Sysadmin laptop plus Student responsive profiles.
