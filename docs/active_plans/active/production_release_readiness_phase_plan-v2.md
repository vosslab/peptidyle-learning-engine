...
2. This plan wins on product decisions: what a user sees, what a role may do, what "done" means.
3. The detailed milestone plans (`student_decisions_and_grading_trust_plan.md`,
   `webwork_opaque_backend_plan.md`, and future Phase 3/4 plans) remain the authority for
   implementation details this plan does not mention. Leaving a technical requirement out of this
   plan does not cancel it.
...
> Submission of an Assignment Attempt produces its final grade automatically. Instructors,
...
- "Recovery" has exactly one meaning: when a timed attempt runs out, the server submits whatever
  the student had saved. Unanswered questions close at zero. That auto-submission produces a
  grade like any other submission.
...
| Assignment Attempt           | One student's one sitting of an assignment. Timed by the server.                                                                                                                                                             |
| Assignment Question Editor   | Teacher page for picking and ordering questions.                                                                                                                                                                             |
| Assignment Properties Editor | Teacher page for due dates, time limit, attempt count, late work, answer disclosure. (Code calls this "Policies".)                                                                                                           |
...
| 2   | Assignment Overview and Question Editor pages worked, but the top menu bar hid them.                                                                             | Phase 1, done                                             |
...
- 1.1 Assignment Properties Editor saves simple changes (dates, checkboxes, numbers) as soon as
...
- 1.3 Grade Settings and Teaching Operations were removed. Their URLs now give an ordinary
  not-found page.
- 1.4 Assignment Overview and Question Editor appear in the top menu bar.
...
- Attempt expiry automatically submits saved responses and closes unanswered questions at zero.
...
- When a Student submits before expiration, submission produces the grade in the same
  operation. If the backend is unavailable, the submission fails with a plain "try again"
  message, saved responses remain, and the Student may submit again while the Attempt remains
  within its existing time limit. Nothing half-submitted is recorded. `expires_at` stays the
  sole authority for whether editing or submission is still available.
- Expiry finalization happens on the next interaction with the Attempt (see below). If the
  backend is down at that moment, the interaction fails with the same plain message and the
  Attempt stays expired-but-unfinalized; it is finalized on the next interaction that succeeds.
  At no point can anyone edit it after expiry.
...
- One product lifecycle, backend-specific completion. Submit sends responses through the
  backend; when the backend answers immediately (native PLE, WeBWorK today) the Student sees the
  score in the same request. A backend that answers "accepted, ask me later" is finished by the
  background process, and the Student sees "submitted" then the score. Neither case exposes a
  grading lifecycle. Background machinery is kept only for the two responsibilities named below.
...
> interacts with the Attempt. Because expiry must submit saved responses even when nobody has a
...
- The server already owns `expires_at`. On any **Student** interaction with an Attempt (save,
  reload, submit, starting a new Attempt on the same Assignment), the server first asks "is now
...
- The requirement that justifies background execution, written down: HUMAN_GUIDANCE says "when
  an Assignment Attempt expires, its saved responses should be submitted automatically." That
...
- The background process has two responsibilities and no more:
  1. Find expired Attempts that still need automatic submission and submit them through the
     ordinary Attempt submission path, the same code a Student's Submit uses.
  2. For a Question Backend that answers "accepted, ask me later" instead of returning the
     credit fraction immediately, keep asking until the fraction arrives, then store it.
     It is not a grading daemon. It owns no public states, no queue of grading work, no retry
     policy visible to anyone. It is background execution for operations that must progress when
     no user request exists.
- One submission path for everyone:
  Student Submit or expiry job -> ordinary Attempt submission -> Question Backend -> credit
  fraction -> stored outcome.
- Backend completion mechanics are backend-specific and hidden. Native PLE returns the fraction
  at once. WeBWorK today returns it at once. A future backend may require polling. The
  backend-agnostic boundary allows both; Students and Instructors see one thing: submitted,
  then the score.
- Idempotent: the job and a Student interaction racing on the same Attempt produce one result.
  Backend down: the Attempt is picked up on the next pass. No per-Attempt timers, no states
  beyond "submitted or not" plus, internally, "waiting on backend" for polling backends.
- Keep what already works. The 09-13 code has a generic worker with an expiry sweep and grading
  ...
  worker to the two responsibilities above, removes the public grading states, the
...
1. Submit a response. The Question Backend (native PLE, WeBWorK, later iMathAS/H5P) returns a
   credit fraction: 0, 0.67, 1. PLE stores that fraction. It never changes.
2. The Assignment score = sum of (stored credit fraction x that question's current point value).
...
4. Attempt expiry submits saved responses automatically; unanswered questions close unanswered.

> Changing points may change a Student's Assignment score. The stored credit fraction for each
...
once at submission; PLE keeps the outcome.

Starting design: do not store Assignment scores at all. Store the credit fractions. The Student
...
in session 53170 with those r11 hashes. It is the ordinary reusable Blueprint: stored, Available,
...
and Assignment records, visibly restored a saved radio selection after reload and a new
...
- Scenario picker, selected moment, effective settings summary (due, close, time limit, attempt
  ...
  "Cannot start: closes at ..."), the assignment instructions, and the questions.
- Opens separately from the editing pages (HUMAN_GUIDANCE: "Assignment Preview should open
  separately from the editing surface").
...
- Submission of an Assignment Attempt produces its final grade automatically. No manual grading,
...
- The Question Backend produces the credit fraction; PLE stores it immutably; scoring uses
  stored fractions x current point values, computed on read. No stored Assignment score, no
...
- Attempt expiry is enforced on every Student interaction (correctness) and by one small
  ...
  process has two jobs: submit expired Attempts through the ordinary path, and finish backend
  interactions that require polling. It is not a grading daemon and owns no product states.
...
- Backend completion is backend-specific (immediate or polled) and hidden from users.
