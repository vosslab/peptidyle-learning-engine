...
the full screenshot corpus only after the objective interim gate under R1 passes. Audit the fresh
corpus again before resuming lower-impact UI work. Remaining Human Guidance work continues afterward.
...
Required SQL-owned work remains only in the `Open` rows of the current SQL lock ledger: Sysadmin
vocabulary lifecycle, mandatory Bloom initialization and Library admission, required Question/Pool
notification kinds. Classification, Change Proposal,
recovery, and resource-specific support receipts still leave application, connected-browser, or
global acceptance work; that work does not reopen a `Closed SQL` row. The current checklist
reconciliation below supersedes former current-count claims; it changes inventory accounting only,
...
Reconciliation on 2026-09-17 confirms that the generator and checklist agree on 1,018
occurrences: 472 verified, 498 open, and 48 N/A. Deferred product behavior is authority but is
...
retirement against concurrent new use. Final review and major-milestone actual-role SQL plus
canonical browser proof remain pending, so the owning checklist row and SQL lock row stay open.

The Library improvement-thread and impact-notice lifecycle is also settled. Its implemented source
core is recorded under C871-C875 below. Final review, major-milestone proof, and the in-progress
...
2. This approved plan is the primary source for implementation scope, sequencing, and detail where
   it agrees with HG.
3. Durable contracts in `docs/` govern their owned boundaries where they agree with HG and this
   plan. Update `docs/TERMINOLOGY_CONTRACT.md` whenever product terminology changes.
4. Current code, routes, SQL, DTOs, tests, and generated views are implementation evidence.
...
| C878 | Question-fork typed command contributor: `QuestionForkStore` and server command resolve the exact authorized Published Revision from the canonical request path, obtain the new ID only from C369's server allocator, and invoke C877 once. The request body contains no Question ID, source, attribution, authorship, or Draft payload. | C369,C877; hands C879. | Ignored Store/server authorization, malformed-path, allocator-collision, retry, and concurrent-request matrix; remove. No mock call-order test is permanent. |
| C879 | **Complete.** An active Instructor invokes the C878 command from a Published Question and reaches only the returned distinct private Draft, with own authorship, exact immutable source Revision attribution, and no Library admission before C9 publication validation. Publication derives and preserves the exact source Revision license. | C9,C878. | Accepted connected PostgreSQL/server/browser proof covered two Instructors, source pin/attribution, private cross-account denial, retry/concurrency, a distinct server-issued ID, and prevalidation-publication denial. C843-C846 supersede its former HMAC/format evidence; fresh canonical-ID proof remains required. Accepted 3-by-3 PostgreSQL proof preserved each of three compatible CC source licenses and rejected all mismatches through the approved minimal SQL. Temporary probes were removed; no new permanent orchestration test was retained. |
...
C871-C873's SQL/LDA/server/browser source core is implemented and awaits final review plus
major-milestone actual-role SQL and browser proof. C874's four source-bound event writers and
C875's private four-event Watch delivery are in progress. The Watch-notification bullets remain
`[ ]` until the repaired source and terminal proof pass. Temporary probes are removed after review;
retain a permanent test only for stable authorization, privacy, or idempotency behavior that earns
it under `docs/PYTEST_STYLE.md`.
...
| C528 / L237,L250                                   | **OPEN, decision-blocked.** The complete-or-unsaved rule cannot be declared compliant or closed until product semantics distinguish raw nonempty, trimmed nonempty, and optional text entries. `FillIn` and `MultiFillIn` define maximum length/count but no required, nonblank, or optional-text meaning. `BackendOwned` is opaque; WeBWorK accepts `[]` and empty values, and no adapter returns a trusted pre-save completeness verdict. Current source implements one structural interpretation that accepts empty or whitespace-only text and successfully captured opaque empty values; that behavior does not settle the intended semantics. The current format validator has unambiguous structural outcomes for `Numeric`, `SingleChoice`, `MultipleAnswer`, `Matching`, `Ordering`, and `Hotspot`; it does not settle the three blocked variants. The two smallest decisions are: (1) whether text-entry completeness is raw nonempty, trimmed nonempty, or optional; and (2) whether successful opaque capture is complete by itself or the adapter must return trusted completeness. | Record the source evidence only; do not run or retain the former `hg_a9_g27_response_complete` proof while the product decision is open. After both decisions, create the narrow temporary proof for the selected semantics, repair the owning boundary if needed, and remove it. No compliant closure, accepted test, or checklist completion is claimed. |
...
course-scoped roster identities stay private. `R`, `W`, `D`, `M`, and `I` short-reference concepts
are implementation drift pending audit or removal, not approved public formats. Internal non-user-facing
objects retain native UUID identifiers; a public reference exists only for a human-facing workflow that
needs one. Published Question and Question Pool `XXXX-ZXXX` identities remain unchanged.
