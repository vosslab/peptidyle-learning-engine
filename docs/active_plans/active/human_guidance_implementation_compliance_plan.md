...
Own conclusions: the checklist is generated by a script so verbatim fidelity and later HG drift
are machine-checked; audit ownership follows HG sections so each of nine parallel subagents owns
a distinct bullet range; each part gets its own spot-check so a passed part is immediately
usable for repair.
...
Gate: every owning `[ ]` bullet appears in exactly one report; every `Reason: product decision
still unclear` item appears in `unresolved_or_ambiguous_items.md` with both readings; counts in
`compliance_summary.md` equal the checklist summary block; `pytest tests/test_markdown_links.py
tests/test_ascii_compliance.py` pass; changelog entry.
...
Gate per run: every owning `[ ]` item of the part has exactly one closure-owner milestone or
...
- HG bullets closed: "Adaptability should be a focus so the software can evolve as requirements
  ...
  to Rust/WebAssembly."
- Expected behavior: these bullets are recorded as audited `N/A` guidance because neither asserts
  a specific current PLE behavior; a later measured performance issue remains eligible for a
...
- Contributes to: the predictable-purge portion of "**Student** data should be collected
  reluctantly, used deliberately, and purged predictably." The collection/use portion remains the
  explicit A2-08 product question in the gap map.
...
| C824 | C311 evidence owner: inventory every biologyproblems.org WeBWorK problem family, pin its provenance/license and canonical algorithmic author source (official PG/PGML file or generator), and map each family to its generated static QTI/PG variants. Run one independent family ledger at a time; `topic03/002-hla-genotype-2-markers-black` begins as a 199-row example traced to an author generator, not an upstream PG, so C838 must construct its target PG. It creates no Question lineage, Pool, Blueprint, archive, or historical rewrite. | `docs/TODO.md`; `content/genetics/manifest.yaml`, `content/genetics/sources/`, `content/genetics/pg/`; hands C838. | Ignored family ledger proves the canonical author source, source hash, provenance/license, and exact generated-variant coverage; remove after C838 handoff. A missing canonical source is an implementation gap to repair, not authority to retain static copies or use a Pool. |
...
| C838 | C311 canonical-source and publication owner: for one C824 family at a time, validate its pinned canonical algorithmic author source, then retain or reconstruct exactly one canonical algorithmic PG/PGML file with its provenance/license; remove generated source copies from the content shape; and create exactly one ordinary WeBWorK Question lineage from that file. | Canonical `content/genetics/pg/` source and manifest mapping; ordinary publication only: `schemas/base_schema/question_authoring_operations.sql`, `crates/server/src/question_publication.rs`; C824. | Ignored per-family source/publication fixture records exactly one canonical PG/PGML file and one lineage, without an adapter heuristic or historical Question ID/revision/pin rewrite/delete. |
| C839 | C311 canonical-source acceptance owner: before catalog mutation, verify the canonical PG/PGML source, provenance, deterministic parameter contract, and representative rendered/grading instances for one accepted family. This is source acceptance, not row-for-row equivalence to an inferior static expansion. | C838 new lineage and one affected Genetics family; hands C840. | Ignored per-family provenance/parameter/render/grade fixture is always temporary content evidence. A failure repairs the canonical source before any retirement. |
| C840 | C311 catalog-state and no-Pool closure owner: after C839, require the expected-current Genetics Blueprint Revision CAS, replace the accepted family's Pool entry and static Question placements with its one canonical algorithmic Question directly in the Assessment, and remove every biologyproblems.org WeBWorK member of that family from every Question Pool. For each affected live Pool, publish a successor Revision containing only remaining eligible members, or, if none remain, remove the Pool from current Assessments/Blueprints and mark its lineage for C841 retirement. Never create an empty Pool or singleton BiologyProblems.org Pool. | Current Question/Pool/Blueprint revision operations; C838,C839. | Ignored per-family Pool-membership inventory plus Blueprint CAS fixture proves direct Assessment placement, no canonical/replaced biologyproblems.org WeBWorK Pool member, and no empty Pool Revision. On CAS failure, retain all current catalog state; never delete evidence. |
| C841 | C311 single-source retirement closure owner: only after C839 acceptance and C840's CAS-published catalog state, archive replaced static Question lineages and every Pool lineage made empty or removed from current catalog use through ordinary availability, and remove redundant generated PG/QTI source copies, leaving exactly one canonical algorithmic PG/PGML file for that family. A revised shared Pool remains published only with no biologyproblems.org WeBWorK members. Preserve historical Blueprint and Student Work pins. | Published availability and `content/genetics` source/manifest rewrite; C840. | Ignored per-family historical-pin/revision-preservation plus source-count fixture proves one canonical source, no live static duplicate, no live prohibited Pool member, and no empty Pool Revision. Forward recovery uses ordinary availability and a later Blueprint Revision, never historical rewrite/delete. |
...
| C858 | Author-content identity-and-document contributor. First, its sole persistence subtask resolves each closed C857 library ID through the server registry at issuance and retains the exact immutable entry identity (library ID, package/version, and reviewed JS/WASM digests) with the answer-free descriptor/reproduction evidence. It accepts no author version, digest, path, or URL. That atomic repair hands C901 the only permitted asset-selection input and hands C902 the durable in-use predicate. After C901, provide one authenticated no-store HTML document route for exact Student/position authorization and that retained identity's pinned presentation reproduction. Safely encode source; no raw-source browser API/save/submit/grading/object-store/general API. Headers are exactly `Content-Security-Policy: sandbox allow-scripts; default-src 'none'; base-uri 'none'; object-src 'none'; connect-src` restricted only to C901's exact retained-identity RDKit WASM path; `img-src 'none'; media-src 'none'; font-src 'none'; frame-src 'none'; worker-src 'none'; form-action 'none'; frame-ancestors 'self'; script-src` restricted only to the retained registry JS SRI hash and server bootstrap nonce, `Content-Type: text/html; charset=utf-8`, `X-Content-Type-Options: nosniff`, `Cache-Control: no-store`, and `Referrer-Policy: no-referrer`; no author URL or broad `'self'` source is used. | C857,C900; identity handoff -> C901,C902; C901 -> document route -> C859. | Ignored issuance/immutability plus route/header/authorization matrix proves the retained identity, document, exact SRI/`locateFile` assets, and every header/CSP exclusion; remove. Retain a small real-session access/isolation outcome only if it independently passes every `PYTEST_STYLE.md` question. |
...
| C860 | Author-content connected-proof contributor: prove C859 rendering/interaction, isolation, grading independence, and C901's reviewed local RDKit manifest on the authorized pinned Student position before C304 proceeds. | C859,C901; hands C903. | Ignored connected server/browser/RDKit-manifest fixture; remove after review. No permanent renderer orchestration or mock call-order test. |
...
| C900 | Reviewed RDKit supply-chain owner: add the canonical manifest for `@rdkit/rdkit@2025.3.4-1.0.0`, official npm/rdkit-js provenance, BSD-3-Clause license, npm SHA-512 integrity, and exactly the two allowed `RDKit_minimal.js`/`.wasm` SHA-256 entries. Add one deterministic vendoring/check command that verifies tarball integrity, license, paths, bytes, and generated registry; it rejects extra runtime files and any unpinned resolver/CDN. | C899; hands C901,C902. | Ignored clean-cache download/tamper/extra-file/generation-reproducibility matrix; remove. A manifest snapshot is not permanent. |
| C901 | Server-owned local-asset/registry owner: consume only C900 generated registry and C858's retained immutable entry identity, then expose only that entry's JS/WASM pair through server-derived GET/HEAD routes. Permit no caller-selected path, redirect, directory list, object-store URL, cookie-dependent response, or CORS grant. Send exact MIME, `nosniff`, `Cross-Origin-Resource-Policy: cross-origin`, and one-year immutable cache headers; derive the SRI JS hash and exact WASM `locateFile` route from the retained entry, never current `rdkit` by name. | C900,C858 identity handoff; hands C858 document route,C860,C902,C903. | Ignored route/header/method/path traversal/retained-identity/registry-mismatch matrix, including opaque-origin frame fetch; remove. Retain only a narrow stable immutable-asset authorization/byte-integrity outcome if it independently satisfies every `PYTEST_STYLE.md` criterion. |
| C902 | Reviewed dependency update-workflow owner: after C858 persists immutable entry identity and C901 selects it, document and enforce one deliberate manifest-update command that can add a new immutable entry only after fresh official provenance/license/integrity/file-hash/browser-compatibility review and isolated-frame proof. It never replaces an existing path; retirement first proves no retained C858 identity references that entry. The command and registry therefore support multiple immutable entries rather than treating the current `rdkit` library ID as a mutable singleton. | C858 identity handoff,C900,C901; hands C903. | Ignored clean-worktree add-entry/reject-replacement/retire-in-use matrix; remove. No release-script call-order or manifest inventory test is permanent. |
| C903 | External-dependency closure owner: on an authorized pinned Student position, prove the closed `rdkit` declaration yields only C901 local reviewed JS/WASM, successful isolated chemistry rendering, no network/CDN/API fallback, exact CSP/SRI/`locateFile` confinement, and unchanged server grading. This closes the three external-dependency occurrences only after C858-C860 also pass. | C858,C859,C860,C900,C901,C902; hands C304. | Ignored connected server/browser/offline-network-denial proof; remove after review. Retain a permanent test only if one small, stable user-visible local-runtime or security outcome independently passes every `PYTEST_STYLE.md` question. |
| C910 | WeBWorK opaque-feedback contributor: carry the renderer's bounded backend-owned feedback through one typed grading outcome into protected Student Work evidence, finalization, and Student-authorized history disclosure. It neither parses PG controls/HTML nor translates feedback into native Question semantics; only the configured feedback-release policy may disclose the opaque backend result. The current renderer client normalizes only `problem_result.score`, and history supplies default feedback, so this contributor is required before C331 or C362 can claim the backend owns feedback. | C308,C358,C359,C361,C831; hands C331,C362. | Ignored real-renderer/fresh-PostgreSQL matrix proves feedback is retained without a native parse, withheld before policy release, delivered only to the authorized Student after release, and concealed from other Accounts. Remove it after review; retain no renderer-call-order or serialization-shape test. |
| C904 | terminal product question; 0; **For an independently reusable Question Pool, does the number of Questions selected belong to the immutable Pool Revision, to each Assessment Pool entry, or may an Assessment override a Pool Revision default? If override is allowed, what validation and exact delivered-evidence rule applies?** HG requires an Instructor choice but does not place it. | Blocks C353 and C905-C909. | Record this exact question in the fresh unresolved report. Keep the five Pool behavior occurrences `[ ]` with `Reason: product decision still unclear`; create no schema, API, or placeholder. |
| C905 | blocked Pool-count schema contributor: after C904 only, define the decision-approved selected-count field/override, positive bounded validation, and exact immutable Pool Revision or Assessment-entry provenance. It neither changes Pool member pins nor invents a default/override. | C313,C885,C904; hands C906. | Ignored fresh-schema validation/provenance matrix after decision; remove. No speculative schema or permanent inventory test. |
| C906 | blocked Pool-count typed-LDA contributor: after C905 only, carry the exact Pool Revision and decision-approved selected-count source without client-selected revision, backend behavior, or arbitrary selection policy. | C905; hands C353,C907. | Ignored typed authorization/provenance codec matrix after decision; remove. No Store call-order test is permanent. |
| C907 | blocked Pool-selection server contributor: after C906 only, the authorized server/Assessment operation writes the selected-count source, calls C353's backend-neutral exact-Pool-Revision selection, and C315 persists only resulting exact selected Question Revision evidence. It exposes no generic selection endpoint or backend-native randomization. | C353,C315,C906,C887; hands C908. | Ignored server/Assessment authorization, fresh-Attempt, resume, and persistence matrix after decision; remove. No orchestration test is permanent. |
| C908 | blocked Pool-selection client contributor: after C907 only, the Instructor Assessment editor and typed browser client submit only the decision-approved count and exact existing Pool Revision reference, then show a whole validation outcome. They cannot provide selected Question IDs, Pool-ID issuance, backend behavior, or a silent override. | C907; hands C909. | Ignored connected client/editor boundary proof after decision; remove. No component snapshot is permanent. |
| C909 | blocked Pool-selection closure owner: after C908 only, connected Instructor/Student proof closes C355's five Pool behaviors: approved count configuration, new-Attempt backend-neutral selection from exact pinned Pool Revision, resume preservation, and exact Pool/Question Revision evidence. | C355,C907,C908; hands C314,C334. | Ignored fresh schema/server/browser proof after decision; remove. Retain only a narrow stable selection-evidence or authorization outcome if every `PYTEST_STYLE.md` criterion passes. |
...
algorithmic source replaces static variants directly and never through a Question Pool. `C842 -> C843`; `{C318, C341, C842, C843} -> C369`; `{C842, C369} -> C844`;
...
`{C313,C885,C904} -> C905 -> C906 -> C353 -> C315 -> C907 -> C908 -> C909 -> {C314,C334}`;
`{C313,C887,C904} -> C355 -> C909`.
...
C304, C323, C332, C309, and C910. `{C308,C358,C359,C361,C831} -> C910 -> {C331,C362}`;
C362 retains its other declared backend-boundary prerequisites. C306 has no H5P mapping. `C206 -> C847 ->
...
contributors; `C302 -> C857`; `{C301,C302} -> C899 -> C900`; `{C857,C900} ->
C858(identity) -> C901(selection) -> C858(document) -> C859 -> C860`; and
`{C858(identity),C900,C901} -> C902`; then
...
C300-C375 is the sole ordinary A7 dispatch range, disjoint from A5's C91+ range and A6's C200-C216
range. C825-C831 is the explicitly reserved cross-boundary native-reproduction correction chain
for C300; it does not renumber or duplicate an A7 owner.
`docs/active_plans/audits/human_guidance_gap_map.md`, section "A7 Questions - canonical
dispatch map", is the one authoritative row set, ownership ledger, and dependency DAG. Do not
dispatch from prior A7 drafts or abbreviated DAGs.

Accounting is exact: 76 A7 requirements are accounted for. Of them, 71 have a closure owner:
62 in the ordinary C300-C375 range, two in C879's Question-fork closure, one in C887's
Pool-creation closure, one in blocked C875's Watch-notification closure, and five in C859's
approved author-isolation correction. C904 is the one exact product question blocking the five
unowned C909 Pool-selection occurrences; they remain `[ ]` with a product-decision reason. The exact duplicate
...
C400-C425 is the sole ordinary A8 dispatch range, disjoint from C300-C375 (A7) and the accepted cross-boundary C800-C862 range. The authoritative A8 ownership ledger, exact 64-occurrence accounting, shared-writer handoffs, contract-to-recipient DAG, temporary-proof lifecycle, and browser closure rules are in docs/active_plans/audits/human_guidance_gap_map.md, section "C400-C425: Complete A8 Courses gaps."

C400-C425 has 22 closure owners for 63 owning A8 audit records and exactly four contributors (C421 and C423-C425); C856 is the one accepted cross-boundary owner for the remaining Blueprint Star identity occurrence, for 64 total A8 records. C400-C407 are contract-only: they close nothing until their designated C6/C7/C49/C72/C73/C52/C19/C47 recipient gates return. A8 owns only new isolated modules/tables named in the map: blueprint_stewardship.sql, blueprint_update_offers.sql, blueprint_lineage.sql, blueprint_change_proposals.sql, and course_blueprint_publication.sql. No A8 worker concurrently edits the C6/C7/C19/C46-C55/C72/C73/C206/C216 boundaries.
...
C500-C536 is the sole A9 implementation range, disjoint from C400-C425 and C800-C862. This
ledger is authoritative; `docs/active_plans/audits/human_guidance_gap_map.md` is a concise,
one-way index to it. It accounts for 109 raw opens as **104 closure owners** plus exactly five
pointers: `L99->L7`, `L104->A8-L165`, `L152->L111`, `L254->A7-L126`, and `L304->A7-L122`.
C502 is a C216 contributor and closes no A9 occurrence. Every row below has one owner, one
...
schema/domain/API/Blueprint JSON/routes directly, with no compatibility SQL view, dual DTO, or
import path. Preserve public A-reference/UUID values and Type enums. For one announced
compatibility release only, legacy browser GET/HEAD has one fixed same-origin 308 transform;
legacy APIs are payload-free nonenumerating 410. There is no mixed-version deployment: rollback
restores both previous code and its matching resettable base together.
...
**Per-row dependencies.** This index is part of the ledger (not a substitute for it): an empty
entry means the prerequisite is DD-A9-01 or no unclosed prerequisite. It keeps source ownership
separate from the dependencies rather than silently broadening a row's boundary.
...
decoders/client -> T-A9-4 routes/link emitters -> T-A9-5 finite legacy compatibility -> T-A9-6
...
`node tests/_temp/hg_a9_t5_redirect/probe.mjs`; T-A9-6 has
...
T-A9 probes remove by default; only one stable external canonical API/redirect contract may be
...
Legacy `/assignments` browser GET/HEAD receives only a fixed same-origin 308 carrying declared-safe
query parameters; no caller-selected target or authorization decision occurs there. Legacy API
endpoints return payload-free nonenumerating 410, never redirects; the canonical route authorizes.
