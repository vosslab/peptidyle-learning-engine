...
the old `pg/genetics/` layout; 43 canonical registrations now use
`content/genetics/pg/topicNN/`, while the earlier acceptance covered 42 sources.
The remaining 76 unreplaced banks still name 13,434 removed generated PG files;
their canonical-source migration is open, and the redundant expansions must not
be restored. This source cleanup is not catalog
...
| C61 | Assessments ribbon/Due Soon/Templates use Assessment labels and dense state rows. | `src/ribbon/ribbon_catalog.ts`, `src/pages/assessments_due_soon_page.tsx`, `src/pages/assessment_templates_page.tsx` | Closed 5 of G-A4-17's 6 rows with accepted Ribbon/Templates evidence plus private actual-server and exact-main Due Soon evidence. The Course list now renders readable Account-zone local Due values, including at 720px in a browser configured to a different zone; broad representative cross-list scanability remains open. | Representative production proof across multiple Course lists. | `node --test tests/test_assignments_due_soon_client.mjs`; accepted private PostgreSQL 17, actual-server, and full-app browser proof. Empty/error states, release workflow, WASM runtime, and deployment gateway were not claimed. |
