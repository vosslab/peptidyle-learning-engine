...
second-Instructor fixture proof passed. The next rebuild needs compiled-main two-Instructor
...
The current SQL lock ledger has no open SQL-owned rows. Fresh actual-role evidence closes the
Sysadmin vocabulary lifecycle, Bloom storage and Library admission, and all required Question/Pool
notification kinds. Classification, Change Proposal, recovery, and resource-specific support
receipts still leave application, connected-browser, or global acceptance work; that work does not
reopen a `Closed SQL` row. The current checklist reconciliation below supersedes former
current-count claims; it changes inventory accounting only, not product acceptance.
...
## Historical preview delivery defect receipt
...
capture is pending. At this point, the affected Instructor-inspection row remained open pending
separately authorized resource-policy work, not a blanket compliance block. The earlier isolated
`window.frameElement.id` failure also still needed retesting after resource delivery worked. The
current closure receipt below supersedes both defects. Connected Library probe 56322 passed only its narrow search receipt;
...
no such activity is claimed here. At the time of this 2026-09-16 receipt, the generated checklist
had 1,009 occurrences: 450 verified, 514 open, and 45 N/A; those numbers are historical and this
receipt changed no row or status. Receipt:
...
remain pending. At the time, this did not close the broad Blueprint search rows or alter that
receipt's 450/503/43 inventory. Separate later Question Library classification and Blueprint return-state receipts have
their own bounded isolated evidence; Tags, sorting, and richer result metadata remain open. Receipt:
...
cross-Discipline Subject flag preserve their strict chain/cursor contracts; Question matching uses
current lineage classification, while Pool matching uses Pool-owned lineage metadata before
ordering, `LIMIT`, lookahead, and continuation. Root `cargo tsgen` generated 370 types; five
model tests passed in session 44587, and `cargo check -p server_core -p project-tools --tests`
passed in 10.48 seconds before 13 Question and two Pool Rust tests passed in session 75888. A
fresh network-none PostgreSQL 17 installation ran as `ple_migrator`; its subsequent isolated proof
used product calls as `ple_auth` and `ple_app` for cross-only matching, member independence,
unchanged pins, and role denial. Its exact labelled disposable container was removed. Root
`cargo check -p server_core -p project-tools
--tests` later passed in 3.87 seconds with no warnings; five model, 13 Question Rust, two Pool
Rust, 24 focused Node, and strict TypeScript gates passed. Pytest session 48126 passed 7,532 tests
in 5.58 seconds after initial-Search hierarchy recovery and Pool wiring. Question classification and malformed-
...
need no mixed engine or shared cursor. At the time of this historical receipt, the inventory had
999 occurrences: 450 verified, 504 open, and 45 N/A. Receipt:
...
fresh replay; unsent controls remain separate. The continuation-navigation blocker was corrected
and independently re-reviewed. This does not establish sorting, Tags, richer result metadata,
connected HTTP/authorization, deployed behavior, or a broad Human Guidance closure. Receipt:
`/private/tmp/ple-library-ui-batch-evidence-receipt-20260916.md`.
...
Nine narrow owning rows closed in that receipt: four independent Pool metadata/Title/Description/first-member/
nonduplicated-metadata obligations and five Course-wide/exactly-one-Discipline/optional-hierarchy/
Tags/Instance-independence obligations. Its generated inventory had 999 occurrences: 450 verified,
504 open, and 45 N/A. The excluded How-to-use and Product vocabulary/glossary metadata
...
classified as a whole. At the time, this preserved the 450 verified, 504 open, and 45 N/A inventory;
...
Reconciliation on 2026-09-17 confirms that the generator and checklist agree on 1,058
occurrences: 513 verified, 497 open, and 48 N/A. Deferred product behavior is authority but is
excluded from current implementation-compliance counting. Use the generated checklist for current status;
all counts attached to earlier receipts are dated historical snapshots.

Public-ID planning uses seven random Crockford Base32 characters plus one final calculated checksum;
Question and Pool IDs retain their distinct `XXXX-ZXXX` shape.

## Current lifecycle source receipt

The Discipline lifecycle decision is settled and implemented in current source: Sysadmins create,
rename, retire, and restore stable UUID identities; no delete transition exists. Retirement removes
a Discipline from new choices while existing exact references remain visible and discoverable with
retired status. Exact inherited or copied references may remain, and shared row locks serialize
retirement against concurrent new use. Fresh PostgreSQL 17 actual-role proof closes its SQL lock
row. Canonical browser proof remains an application gate.

The Library improvement-thread and impact-notice lifecycle is also settled. Its implemented source
core is recorded under C871-C875 below. Fresh PostgreSQL 17 actual-role proof closes the four-event
private Watch-delivery SQL boundary. Browser proof remains open.

## Evidence cadence

Treat tests as liabilities as well as protection. Each bounded slice runs its narrow gate and removes
temporary probes. Full Podman and `source source_me.sh && ./launchers/all_test.sh` acceptance run at
major milestones, not after every small source or documentation change.
...
checklist then agreed on the historical snapshot of 1,038 occurrences: 441 verified, 547 open, and
50 N/A. The first-owner inventory is 432 verified, 533 open, and 50 N/A across 1,015 distinct
identities; 23 are later duplicates. Sixty-nine new or changed requirements remain explicitly
...
Search PNG remains absent. The 75-entry manifest contains no _new_ published PNG; no stale or
...
1. `docs/HUMAN_GUIDANCE.md` alone controls product behavior and scope. Its Deferred product
   behavior section overrides other implementation language until an item leaves that section.
2. Vendored `docs/*_STYLE.md` files govern repository and language practice.
3. Durable contracts in `docs/` govern their owned implementation boundaries only where they agree
   with Human Guidance. Update `docs/TERMINOLOGY_CONTRACT.md` when settled terminology changes.
4. This plan organizes implementation, sequencing, and evidence. It does not create product
   behavior or override Human Guidance.
5. Current code, routes, SQL, DTOs, tests, and generated views are implementation evidence.
...
- [x] Every Account has exactly one Product Role: **Student**, **Instructor**, or **Sysadmin**.
  - Evidence (source): `schemas/base_schema/50_functions/accounts.sql` `accounts.product_role` CHECK constraint.
...
- [ ] Students may use multiple passkeys across their devices.
  - Mismatch: `schemas/base_schema/20_tables/authentication.sql` holds email-code ceremonies; a passkey
...
second gateway or alternate Live Demo path. `containers/Caddyfile` is the only Live Demo Caddy
configuration. Use
synthetic records and dates through existing E2E fixtures or a
...
2. Spot-check, risk-based: one fresh reviewer subagent re-verifies, using the cited evidence as
   its sole input, every `[x]` in a runtime-required category plus the remaining claims whose
   authorization, persistence, privacy, grading, or user-visible behavior makes regression costly.
   An unsupported `[x]` flips to `[ ]` with a
...
- Owned boundary: Blueprint schema and adoption operations,
  `schemas/base_schema/50_functions/blueprints.sql` and `schemas/base_schema/50_functions/course_blueprint_adoption.sql`.
...
- Expected behavior: Course Instance creation selects the explicit `Empty | Adopted` Course root.
  ...
  tagged direct-versus-adopted Assessment origin is now in `schemas/base_schema/50_functions/assessments.sql`,
...
- Current bounded contribution: the Rust/browser creation wire now uses strict `Empty | Adopted`,
  ...
  closure.
- Owned boundary: Course schema family, `schemas/base_schema/50_functions/course_core.sql`,
...
- Owned boundary: Question authoring schema contract,
  `schemas/base_schema/50_functions/question_authoring_state.sql`,
  `schemas/base_schema/50_functions/question_authoring_operations.sql`, and related
...
- Owned boundary: Accounts authorization schema and API, `schemas/base_schema/50_functions/accounts.sql`.
...
- Owned boundary: account lifecycle schema, `schemas/base_schema/50_functions/accounts.sql`.
...
- Owned boundary: account identity schema, `schemas/base_schema/50_functions/accounts.sql`.
...
- Owned boundary: C808 audits existing `schemas/base_schema/50_functions/course_core.sql`,
...
- Owned boundary: roster relationship operations, `schemas/base_schema/50_functions/course_operations.sql`.
...
- Owned boundary: Sysadmin authorization schema, `schemas/base_schema/50_functions/authorization.sql`.
...
- Owned boundary: Ribbon browser evidence, `tests/playwright/ui_corpus_manifest.ts` and `tests/playwright/ribbon_responsive_evidence.mjs`. Dependencies: none.
- Focused gates: `node tests/playwright/ribbon_responsive_evidence.mjs`; temporary self-starting `bash tests/_temp/hg_a3_c27_sysadmin_desktop_workflows.sh`; `bash tests/e2e/e2e_live_demo_instructor_accounts.sh`; `bash tests/e2e/e2e_live_demo_support_capability.sh`; `source source_me.sh && ./launchers/run_fast_checks.sh`.
...
- Focused gates: `node --import tsx --test tests/test_course_theme_scope.mjs`; `node tests/playwright/ribbon_density_evidence.mjs`; `source source_me.sh && ./launchers/run_fast_checks.sh`.
...
- Focused gates: temporary `node --import tsx tests/_temp/hg_a3_c34_role_home_routes.mjs`; `node tests/playwright/ribbon_responsive_evidence.mjs`; `./check_codebase.sh`; `source source_me.sh && ./launchers/run_fast_checks.sh`.
...
- Focused gates: temporary `node tests/_temp/hg_a3_c35_three_role_profile_affordance.mjs`; `node tests/playwright/ribbon_responsive_evidence.mjs`; `./check_codebase.sh`; `source source_me.sh && ./launchers/run_fast_checks.sh`.
...
- Contributes to, but does not close: the Profile menu contains Profile and Sign Out, and the
  account-action portion of "Avoid scattering related actions across page headers, menus,
...
- Expected behavior: Profile opens an accessible menu and Sign Out is not standalone in the main
  top bar. C819-C823 supply the self-only Profile preference; no separate Account Settings route is
  part of the menu.
- Owned boundary: `src/ribbon/app_ribbon.tsx`, `src/ribbon/app_ribbon.css`, and existing shell action seam. Dependencies: C35.
- Focused gates: `node --test tests/test_ribbon_contract.mjs`; `node tests/playwright/ribbon_shell_evidence.mjs`; `./check_codebase.sh`; `source source_me.sh && ./launchers/run_fast_checks.sh`.
...
- Expected behavior: `docs/DESIGN_DECISIONS.md` "Account avatars are one role-neutral aggregate" requires `account_avatar` in `schemas/base_schema/20_tables/profile_media.sql` to store closed `ProvidedAvatarId` or self-owned image through one discriminated shape.
- Owned boundary: profile-media schema family, `schemas/base_schema/20_tables/profile_media.sql`. Dependencies: the durable design decision named above.
...
- Focused gates: temporary `node tests/_temp/hg_a3_c43_breadcrumb_route_matrix.mjs`; `node tests/playwright/ribbon_shell_evidence.mjs`; `./check_codebase.sh`; `source source_me.sh && ./launchers/run_fast_checks.sh`.
...
| C812 | C38 prerequisite: one typed ProfileImage object bridge and no second live ProfileThumbnail path.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | new model `ProfileImageId`; `ObjectAddress::ProfileImage`, ProfileImage class, private content, `profiles/images/{image}/{object_id}`; remove old thumbnail address/store/router.                                                                                                            | Retain exactly typed private/signed/serde/path coverage against exposure/wrong physical address and legacy `profileThumbnail` JSON rejection against a second current-image path. Remove the temporary bridge proof after C38 acceptance; failure restores ProfileImage-only privacy/address/signing and repairs the owning migration, never legacy deserialization.                                                   |
...
| C818 | C36 narrow interaction owner: avatar opens menu and Sign Out is relocated; contributes, not closes, contents/no-scattering.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Ribbon component/CSS/contract; C35.                                                                                                                                                                                                                                                                 | Ribbon contract plus `ribbon_shell_evidence.mjs`; retain pointer/keyboard/focus/relocation behavior.                                                                                                                                                                                                                                                                                                               |
...
| C821 | Self-only time-zone read/write is a Profile preference. The API derives the Account from the signed-in session and exposes no credential lifecycle. HG-required Student/Instructor passwordless authentication and multiple Student passkeys remain Accounts-and-roles milestone-owned.                                                                                                                                                                                                                                                                                | `docs/DESIGN_DECISIONS.md` "Profile is the one time-zone preference page"; no credential-lifecycle implementation.                                                                                                                                                                                   | Decision is satisfied; no implementation evidence or HG closure is implied.                                                                                                                                                                                                                                                                                                                                            |
| C822 | The all-role `/profile` page consumes C821's self-only time-zone behavior; invitations remain separate. No separate Account Settings route is used.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | route contract/routes/Profile page/API; C821.                                                                                                                                                                                                                                                        | Closed-shape self-only behavior plus Ribbon route/contract tests; no fake/disabled controls.                                                                                                                                                                                                                                                                                                                           |
| C823 | Final Profile menu composition: `/profile` and Sign Out; no Account Settings route.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Ribbon/route contract; C818, C820, C822.                                                                                                                                                                                                                                                             | Ribbon route/contract and M10 shell tests; lease-gated capture.                                                                                                                                                                                                                                                                                                                                                        |
| C824 | C311 evidence owner: inventory every biologyproblems.org WeBWorK problem family, record its provenance/license, canonical algorithmic author source, and closed `source_format` (`pg` or `pgml`), and map each family to its generated static QTI/PG variants. `pgml` is allowed only for fully PGML-compliant source and uses `.pgml`; traditional or mixed source is `pg` and uses `.pg`. This is import metadata, not a runtime parser or separate backend. The active Fall 2026 selection contains 41 canonical PGML sources (40 official biologyproblems-website sources plus HLA); the BIOL 301 and Chargaff variants are explicitly excluded. | `docs/TODO.md`; `content/genetics/manifest.yaml`, `content/genetics/sources/`, `content/genetics/pg/`; hands C838.                                                                                                                                                                                  | Historical temporary evidence accepted the prior 42-source selection. Revalidate the current selection's source-format/path, local and upstream SHA, license, legacy-bank mapping, and representative render/grading behavior before treating this selection as accepted. A missing canonical source is an implementation gap to repair, not authority to retain static copies.                                         |
...
dropped.
| C842 | Question-ID foundation contributor: make `crates/question_model/src/question_library.rs` the one Rust contract input and add `devel/generate_question_id_contract.py`, which generates browser-safe constants and the canonical `XXXX-ZXXX` syntax/checksum contract at `generated/api/QuestionIdSyntaxContract.ts`. Wire its check command, `source source_me.sh && python3 devel/generate_question_id_contract.py --check`, into the existing generation front door. `src/question_id.ts` consumes that output; no hand-maintained second alphabet, length, grouping, permitted human-entry normalization map, or regex remains. `X` is a random Crockford character and `Z` is the embedded public SHA-256 checksum character; no browser secret or HMAC implementation. | DD "Public Question and Pool IDs"; `docs/QUESTION_ID_SPEC.md`; hands C843,C369,C844. This owns model input, generator, generated output, and browser-safe syntax contract only: it does not allocate IDs or resolve them on the server. | Ignored model/generator/output conformance probe covers canonical `XXXX-ZXXX`, accepted closed human-input normalization, and invalid-form rejection; run the generation check; remove after handoff. No generated snapshot becomes permanent. |
| C843 | Public-ID schema/live-demo contributor: make the fresh base schema and Live Demo fixtures use exact canonical public IDs only. Install one append-only global public-ID registry that reserves every full issued value across every public-ID type; Published Questions and Question Pools share its `XXXX-ZXXX` namespace. The fresh preproduction schema has no legacy conversion, compact storage, dual reader, or rewrite path. | C842; fresh preproduction schema only; hands C369,C845. | Ignored fresh-PG17 fixture proves registry collision refusal across Question/Pool and prefixed types, exact canonical storage, and atomic reservation with creation; remove after handoff. |
| C844 | Question-ID browser-codec contributor: use C842's generated syntax contract at browser entry/copy/display seams. Normalize only the closed human-entry forms before validation and lookup; transmit, copy, and display only exact canonical `XXXX-ZXXX` IDs. | C842,C369; hands C845,C846. This excludes the active/accepted C358 `crates/server/src/question_library.rs` boundary and its tests. | Ignored browser codec matrix proves canonical output, accepted closed human-input normalization, and invalid-form failure before request; remove after handoff. Retain no implementation-coupled decoder snapshot. |
...
| C846 | Question-ID integrated proof contributor: on a fresh schema, prove mint, exact canonical `XXXX-ZXXX` storage/display/serde, closed human-input normalization before validation and lookup, public SHA-256 checksum validation, and malformed/nonmatching denial. Prove Question and Pool use the shared namespace and all issued public IDs reserve globally through C843's registry. | C369,C844,C845; hands C319,C342,C343,C354,C885. | Ignored fresh-PG17/server/browser proof; repair the named failed boundary and rerun. Consider only one stable public canonical-ID contract under every `PYTEST_STYLE.md` question; otherwise remove. |
...
| C851 | C209e orchestration contributor: after C215 plus C847-C850, a late run attempts required earlier notices in C847 `(due_at,id)` order through the typed boundary, then dispatches `mark_inactive`, archive, and delete after a successfully recorded pre-acceptance failure. It stops the notice lane only for an unknown typed Store state and always continues retention transitions; it never calculates dates or policy. A crash before send leaves the receipt leased until expiry; a crash after provider invocation reuses C847's durable idempotency key on an eligible reclaim. | C215,C848,C849,C850; hands C209. | Pending one temporary connected PostgreSQL/typed-Store/worker proof for one evaluated-at claim, due order, lease/crash reclaim, acceptance no-resend, deterministic failure backoff through cap, action-no-longer-due exclusion, repeated-run idempotency, and recorded-failure or unknown Store state with continued `mark_inactive`, archive, and delete transitions. Remove it after acceptance. No new permanent test. |
...
| C879 | **Complete.** An active Instructor invokes the C878 command from a Published Question and reaches only the returned distinct private Draft, with own authorship, exact immutable source Revision attribution, and no Library admission before C9 publication validation. The Draft retains private UUID identity. A successful C9 publication creates the new Published lineage, mints its public Question ID exactly once, and derives and preserves the exact source Revision license. | C9,C878. | Historical connected PostgreSQL/server/browser proof covered two Instructors, source pin/attribution, private cross-account denial, retry/concurrency, and prevalidation-publication denial. Its former server-issued-ID and HMAC/format evidence is superseded by the current lifecycle; fresh canonical publication-ID proof remains required. Accepted 3-by-3 PostgreSQL proof preserved each of three compatible CC source licenses and rejected all mismatches through the approved minimal SQL. Temporary probes were removed; no new permanent orchestration test was retained. |
| C885 | Pool schema/create-operation contributor: `question_pools.sql` stores the exact canonical Pool ID reserved through C843's global registry, immutable sequential Pool Revisions, and each Revision's ordered exact Published Question Revision members. Its trusted create/append operations accept a server-issued typed ID only at Revision 1, never a browser/client grant, and are neither allocator nor unused coordinator. | C312,C313,C846,C354; hands C886,C342,C905. | Ignored uniqueness/revision/member-pin/RLS matrix proves no direct client path; remove. No schema call-shape test is permanent. |
| C886 | Pool creation command contributor: typed `QuestionPoolCreationStore` and an active-Instructor server route mint every Pool ID only through C369's server-side random checksum allocator and atomically create Revision 1 from a bounded ordered nonempty distinct list of exact Question Revision Tuples plus the Instructor's interchangeability attestation. The browser supplies only that content and attestation, never an ID, owner, stored revision number, or backend behavior; a registry collision obtains a new ID and retries the one creation transaction. | C369,C885; hands C887. | Ignored Store/server authorization, member validation, client-ID refusal, backend-mix, forced collision/retry, and atomic-create matrix; remove. No mock allocator/call-order test is permanent. |
| C887 | Pool creation closure owner: the authorized Instructor Pool workflow creates a reusable Published Pool with a new canonical `XXXX-ZXXX` ID, Revision 1, ordered pinned Published Question members, and interchangeability attestation through C886. C342 projects that ID; C905 owns Assessment-owned fork provenance and selection evidence. | C886; hands C342,C355,C905. | Ignored connected PostgreSQL/server/browser proof covers active-Instructor creation, denied non-Instructor/client-selected ID, collision retry, globally unique canonical ID, Revision 1, exact member pins, and C342 handoff. Retain only a narrow real-session authorization or issuance outcome if every `PYTEST_STYLE.md` criterion passes. |
...
### C871-C875: Question Watch notification completion

C346 remains a partial contributor, not a closure. The product lifecycle is now settled. Active
vetted Instructors may create, read, and reply to text-only improvement threads for available
Questions and Pools and may edit only their own retained posts. Question owners and Sysadmins may
resolve or reopen Question threads and create, update, or cancel retained Question impact notices;
Pool administration is Sysadmin-only. Threads record their creation Revision; impact notices may
name an affected Revision; resolved and cancelled records remain visible. Sysadmins use a read-only
Library mode outside these explicit administration actions.

C871-C875's SQL/LDA source core is implemented. Fresh PostgreSQL 17 actual-role proof covers the
four source-bound event writers and private Watch delivery. The Watch-notification bullets remain
`[ ]` until browser proof passes. Temporary probes are removed after review; retain a permanent test
only for stable authorization, privacy, or idempotency behavior that earns it under
...
| C46      | Milestone 2 source projection is implemented: Courses separates the authorized stored `active` and `inactive` states. `/instructor` is My Active Courses; `/instructor/courses/inactive` is My Inactive Courses. The two Ribbon targets and list routes are source-backed. Inactive Courses retain their ordinary detail route and metadata independently of Student-data archive/delete. C47 owns Public Blueprint search; upcoming-Assessment/activity work remains separate. | Milestone 1 remains owned by C206/C208/C215/C851/C209: `course_retention.sql`, `course_retention_transitions.sql`, retention LDA/PostgreSQL adapter and executor attestation, and `course_retention_worker.rs`. C46 owns the lifecycle projection in `course_operations.sql`, Course-instance LDA/PostgreSQL adapter, `src/api/course_instance.ts`, strict `src/api/decoders/course_instance.ts`, `course_list_page.tsx`, `role_home_pages.tsx`, route contract/routes, and Ribbon catalog/capability/contract. | Contributes to A4-03 (5); its rows remain unchecked until the real restricted-role transition and canonical browser journey have actual receipts. A backed Ribbon target records the compiled source/client/route chain, not connected acceptance. | C7; C12; C206/C208/C215/C851/C209 must supply the restricted-role transition receipt. C47 public search and C55 Course-workspace work remain separate; neither blocks the active/inactive routes. | First, temporary connected PostgreSQL/typed-Store/worker proof: cutoff boundary, repeat/concurrent executor behavior, least-privilege attestation, failed/unknown notice state with continued `mark_inactive`, and retained data. Then run `node --import tsx --test tests/test_course_instance_summary.mjs tests/test_ribbon_catalog.mjs tests/test_ribbon_capability_registry.mjs tests/test_ribbon_route_contract.mjs`, both TypeScript no-emit checks, and one lease-owned connected two-co-Instructor journey: Active -> due sweep -> fresh Active/Inactive lists -> retained Course detail; include nonmember/revoked denial, empty/deep-link/keyboard/narrow states, and metadata after independent Student-data deletion. |
...
| C49      | Blueprint schema permits only Private/Public/Archived and records lifecycle/adoption predicates, including Public-only eligibility for adopted Course creation.                                                                                    | `schemas/base_schema/50_functions/blueprints.sql`                                                                                                                                                                                                                                                                                                                                                                              | Contributes to A4-06 (8); flips none.                                                                                                                                                                                                                                                                                                                                                                                                                 | C6.                                                                                                                          | `bash tests/e2e/e2e_live_demo_blueprint_course.sh --service`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| C54      | Centered responsive Course Banner accepts valid still 5:1 sources within independent safety bounds, preserves 5:1 delivery without crop, and treats 1280x256 as HG's recommended authoring size rather than a required minimum or exact rendition. | C814 inventories `crates/question_model/src/course_appearance.rs`, `crates/server/src/course_appearance.rs`, `crates/learning-data-access/src/course_banner.rs`, `crates/learning-data-access/src/postgres/course_banner.rs`, `crates/objects/{bucket,image_validation}.rs`, `schemas/base_schema/20_tables/course_media.sql`, frontend, and test consumers; C815 owns normalization/delivery; C813 owns centered frontend. | Closes G-A4-10 (5).                                                                                                                                                                                                                                                                                                                                                                                                                                   | C814 -> C815 -> C813. Exact fixed rendition dimensions remain an unlocked design requiring a recorded decision.              | ignored small-valid-5:1, 1280x256, 2560x512, non-5:1 fixtures; `bash tests/e2e/e2e_live_demo_course_appearance.sh`; `npx playwright test tests/playwright/e2e/course_appearance_propagation.spec.ts`                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| C63      | Question editor shows order, inspection-before-add, Search/Browse paths.                                                                                                                                                                           | `src/pages/assessment_workspace/assessment_workspace_questions_page.tsx`                                                                                                                                                                                                                                                                                                                                          | Closed G-A4-19's visible-order and direct Search/Browse rows with accepted actual-server and browser evidence. Exact-revision inspection resolves the authorized private source and checksum, then renders through the opaque WeBWorK adapter in a hardened iframe. The canonical HTTPS closure receipt proves prompt, form, focusable control, PLE CSS/bridge, renderer scripts, and reviewed font delivery without the former `frameElement` error. | C60, C62.                                                                                                                    | Accepted private PostgreSQL 17/MinIO and unchanged-renderer HTTP evidence returned preview 200 with hardened headers and concealed missing Revision, Student, and anonymous requests; exact-main browser evidence showed the prompt and five choices. The isolated preview path left all five Student Work counts at zero before and after: Assessment Attempts, Question Attempts, saved responses, submissions, and grading results. Do not loosen the iframe sandbox or rewrite the sibling renderer HTML. Explicit return without losing Assessment state and the fresh screenshot-corpus layout review remain pending. |
| C64      | Persist the selected Assessment Question-order rule with the immutable Attempt, and shuffle the complete fixed-and-Pool issued vector only when that rule is `shuffled`; Question presentation remains Question-owned.                             | `schemas/base_schema/50_functions/assessment_attempt_operations.sql`; `crates/learning-data-access/src/postgres/assessment_delivery_start.rs`                                                                                                                                                                                                                                                                                  | Contributes to G-A4-20 (2); flips none.                                                                                                                                                                                                                                                                                                                                                                                                               | C62; source, compile, private SQL, and actual Student HTTP start/resume gates pass; C65 browser integration remains pending. | Accepted private PostgreSQL 17 actual-API and authenticated Student HTTP proof covered authored order, shuffled complete-vector persistence, concurrent Instructor-write locking, exact pins, immutable resume, and outsider/anonymous concealment.                                                                                                                                                                                                                                                                                                                                                                         |
...
- Focused gates: temporary `bash tests/_temp/hg_a3_c69_hci_walkthrough.sh`; `node tests/playwright/ribbon_responsive_evidence.mjs`; `./check_codebase.sh`; `source source_me.sh && ./launchers/run_fast_checks.sh`.
...
- Focused gates: temporary `bash tests/_temp/hg_a3_c71_precision_field_console_walkthrough.sh`; `node tests/playwright/ribbon_density_evidence.mjs`; `./check_codebase.sh`; `source source_me.sh && ./launchers/run_fast_checks.sh`.
...
- Owned boundary: `schemas/base_schema/50_functions/course_blueprint_adoption.sql`; do not change server
...
- Priority note: current production Question Backends are PLE and WeBWorK. Human Guidance defers
  iMathAS, so this row owns only current PLE/WeBWorK Student View acceptance. Existing iMathAS source
  is not a current implementation requirement or a closure dependency.
...
#### C91: Student invitation context

- HG bullet: Course invitations show Course name, relevant Instructor, and term before acceptance.
- Owned boundary: the Student invitation index/detail pages and their closed invitation decoder.
- Current receipt: source implementation and independent actual-component review are accepted.
- Current projection: self-only Course name, verified assigned Instructor display name, and
  inclusive term. It omits invitation, Account, and membership identities.
- Concealment: loading, unavailable, missing, foreign, expired, and already-accepted states show
  no invitation context.
- Required gates: actual-role connected PostgreSQL proof covers self versus foreign, expired,
  accepted, and already-enrolled invitations. Authenticated canonical-browser acceptance covers
  index/detail context before `Accept invitation` and the concealed transient states.
- Handoff: failures repair this projection or presentation boundary. Keep A5-S14 unchecked until
  both gates pass. This plan owner records implementation and evidence; Human Guidance remains
  product authority.

### C200-C216: Complete A6 Data and history gaps

C91 and later are reserved for the dynamic page-specific A5 dispatches. C91 is the bounded
Student-invitation context owner; it records implementation/review evidence but does not override
Human Guidance or close A5-S14 before its remaining gates. A6 therefore uses the disjoint explicit
C200-C216 range; C205 is the recorded product question, not a code milestone. This allocation does
not reuse C1-C90 or reserve a dynamic A5 identifier.
...
| C206     | D13-D14, D16-D17, D20, D25, D27. Store the operational policy schedule and due actions consuming the audited existing Course-core facts; clock calculation has no action side effect.                                                                                                                                                                                                                         | `schemas/base_schema/50_functions/course_retention.sql`: policy schedule and due actions `mark_inactive`, `warn_inactive`, `notify_archive`, `archive`, `delete`, and archive marker. It consumes, rather than duplicates, current `course_core.sql` facts. No generic recovery state/queue/snapshot service.                                                                                                                                                                                                                                                                                              | Pending one temporary connected PostgreSQL/typed-Store/worker proof for the due schedule and dispatched transition; remove it after acceptance. No permanent test.                                                                                                                                                                                                                                  |
| C207     | D15, D19. Assessment release/save synchronizes `latest_assessment_due_at` and rejects `due_at` past existing `active_until_at`, atomically under concurrent Course/Assessment edits.                                                                                                                                                                                                                          | `schemas/base_schema/50_functions/course_core.sql`; `schemas/base_schema/50_functions/assessments.sql`; private `schemas/base_schema/50_functions/assessment_deadline_sync.sql` `ple_data.synchronize_course_assessment_deadline`; exact retention readers, installation order, and canonical Live Demo seed/oracle; depends on C206 and C809.                                                                                                                                                                                                                                                                                        | Accepted independent PostgreSQL 17 actual-API proofs covered all three save APIs, release, current MAX/no-Due fallback, immutable Active cap, stale CAS, wrong authorization, cap rollback, deterministic two-Assessment concurrency, archive race/freeze, helper ACL, and seeded current MAX; the ignored proof was removed after acceptance. |
| C208     | Complete for D06-D07, D10-D11, D24 archive/delete behavior. The separate `mark_inactive` procedure persists only the due Course lifecycle state; it does not archive or delete Student records. Archive protects Course Student records; delete at expiry removes identifiable records while preserving Account, Course metadata, definitions, Questions, settings, unrelated Course membership, and existing aggregates. | `schemas/base_schema/50_functions/course_retention_transitions.sql` transition procedures; depends on C206. Statistics disclosure is C205, not a C208 prerequisite.                                                                                                                                                                                                                                                                                                                                                                                                                                         | Archive/delete: accepted one-time disposable receipt dated 2026-09-16; the large schema-shaped proof did not earn permanent-test status. `mark_inactive` needs the pending temporary connected PostgreSQL/typed-Store/worker proof; no permanent test.                                                                                         |
| C215     | Contributor to C209. Store interface/adapter returns stored due actions and commits `mark_inactive`, archive, and delete transitions under its least-privilege database capability; flips no HG bullet.                                                                                                                                                                                                      | `crates/learning-data-access/src/retention.rs`, `crates/learning-data-access/src/postgres/retention.rs`, and the executor attestation; depends on C206, C208.                                                                                                                                                                                                                                                                                                                                                                                                                                   | Narrow compile gate: `cargo check -p learning-data-access --no-default-features --features postgres --tests`. The temporary connected Store/worker proof, including `mark_inactive`, remains pending; no permanent test.                                                                                                                          |
| C209     | D18, D21-D22, D26, D28-D30. Worker consumes C215 stored due actions, invokes only C851's retention-notification boundary, then dispatches `mark_inactive`, archive, and delete. Late and repeated runs are equivalent. Provider failure never blocks a lifecycle transition.                                                                                                                                    | `crates/server/src/course_retention_worker.rs`; depends on C215,C851.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Pending temporary connected PostgreSQL/typed-Store/worker proof, including notice failure and unknown notice state with continued `mark_inactive`, archive, and delete dispatch; no permanent test.                                                                                                                                                 |
| C210     | D23. Normal reads exclude archived Work while a protected pre-delete read remains available.                                                                                                                                                                                                                                                                                                                  | Complete. Current owners are `schemas/base_schema/50_functions/assessment_attempt_history.sql`, `student_assessment_landing.sql`, `grading_access.sql`, `assessment_attempt_access.sql`, and the explicit `archived_student_work_recovery.sql` path; depends on C208. Fresh PostgreSQL 17.11 actual-role and production-server proof covered submitted Work across the complete ordinary Student route inventory and Instructor Gradebook before/after archive, current-Instructor-only recovery, expiry concealment, and deletion. No general recovery workflow/state machine or permanent test was added. | Accepted one-time receipt: `/private/tmp/ple-c210-archived-visibility-20260916.md`. Canonical browser presentation remains part of final connected acceptance, not a second SQL mechanism.                                                                                                                                                     |
| C211     | D31. Revision creation remains conservative: source/answer/grading/feedback/assets revise; title/description/tags/subject/topic do not.                                                                                                                                                                                                                                                                       | `schemas/base_schema/50_functions/question_authoring_operations.sql`; it connects the already implemented A7 Published Question rule and has no unclosed A7 dependency.                                                                                                                                                                                                                                                                                                                                                                                                                                     | `source source_me.sh && python3 tests/_temp/hg_a6_c211_revision_probe.py`                                                                                                                                                                                                                                                                      |
...
- Gate order: focused Rust, Node, Python, SQL, or temporary browser proof for the touched area;
  then `devel/run_playwright_tests.sh` when visible connected behavior changed; then
  `./launchers/run_fast_checks.sh` as the offline aggregate gate; then the coder reports flips and
  a changelog bullet to the manager.
...
Current C420 source/client receipt (2026-09-17): `CourseInstancePage` now exposes Create
Blueprint from Course Instance in Course tools. Its compact dialog pre-fills only the new
Blueprint's short name, long name, and shared classification; it states that reusable structure is
copied, the source Course Instance remains unchanged, and it becomes the first Adoption. The
strict same-origin client sends only generated `CreateBlueprintFromCourseInstanceInput` metadata to
the canonical source-Course route, retains one idempotency key for an uncertain retry, requires
`201` plus `no-store`, strictly decodes a new owner-visible Private Revision-1 root, and opens that
Blueprint. Focused Node transport/decoder, TypeScript, ESLint, and Prettier checks pass. A fresh
PostgreSQL 17 actual-role baseline now proves C419 authorization/no-write, stale rollback,
idempotency, exact reusable pins and Pool forks, unchanged source state, immutable source
provenance, first-Adoption/student counts, and lifecycle rollback. Accepted canonical HTTPS C420
browser evidence used exactly one child-route POST to create actor-owned Private Revision-1
Blueprint `BPJD8H28` from Course `CI0QR41X`, showed Adoption count 1, counted source Students only
in the statistic, copied no roster or delivery state, and left the source Course addressable and
unchanged;
the seven directly owned cross-direction checklist rows now close.

Current C415 SQL receipt (2026-09-17): reviewed model, Store, PostgreSQL, and server code export
the authorized current canonical Blueprint and strictly import it through ordinary atomic actor-owned
Private Revision-1 creation. The retained PostgreSQL 17 actual-role proof passed authorized
export/import/re-export semantic deep equality; fresh Blueprint, module, Assessment, and Pool
identities; exact fixed and ordered Pool-member pins; unchanged source; and unauthorized denial.
Ordered reusable structure is retained while authority, stewardship, Course, Student, adoption, and
operational state are excluded. Focused Rust, strict Clippy, formatting, and independent review
passed; C415 and the SQL audit exchange row are closed.

Current C425 browser receipt (2026-09-17): the rebuilt canonical HTTPS Live Demo exported a seeded
Blueprint through the Instructor UI, strictly rejected malformed pasted JSON, imported the exact
download as a distinct actor-owned Private root at Revision 1, and redirected to its detail page.
Exporting the imported Blueprint produced a deeply equal canonical object, including ordered
modules, Assessments, and entries, while the source Blueprint remained unchanged. The ignored
temporary Playwright probe was removed after acceptance. This closes the canonical exchange product
workflow; the separately retained actual-role PostgreSQL round trip now closes its SQL row.

Current I15 acceptance receipt (2026-09-17): canonical HTTPS evidence measured the opaque
WeBWorK preview at 444 px through the exact opaque source/origin/resize shape, kept content
reachable without horizontal overflow, made no Student Work or grade request, and verified keyboard
Star/Watch `aria-pressed` toggle and restore. This is not a screenshot refresh.
...
reader projects each visible child fork's ID, current name, current Revision, and verified
...
old-name adapter may be introduced to split it further. Completed C870 removed only dormant H5P
...
parallel after both handoffs. The manager serializes only an overlapping mutation, then uses
T-A9-1 as the first combined receipt after both cutovers land. The consumer-manifest preflight,
independent review, and non-overlapping focused gates may run in parallel.
...
| C522 / L179,L181                                   | **Complete.** Connected production server/data-access/access-page proof delivered the released Course Instance Assessment and issued its Work to the authorized Student while denying unauthorized access.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Accepted connected proof passed; the temporary `hg_a9_g21_delivery` probe was removed. No implementation-coupled permanent orchestration test was retained.                                                                                                                                                                                                                                                                                                                                                                     |
...
| C528 / L237,L250                                   | **OPEN, limited to unresolved semantics.** `FillIn` and `MultiFillIn` define maximum length/count but not required, nonblank, or optional-text meaning. `BackendOwned` is opaque; WeBWorK accepts `[]` and empty values, and no adapter returns a trusted pre-save completeness verdict. Current source accepts empty or whitespace-only text and captures opaque empty values; that evidence does not settle the intended semantics. The current validator has unambiguous structural outcomes for `Numeric`, `SingleChoice`, `MultipleAnswer`, `Matching`, `Ordering`, and `Hotspot`. Keep this row open without inferring text-entry or opaque-response behavior. Its uncertainty blocks only proofs that depend on response completeness; independent milestones continue. | Record the source evidence only; do not run or retain the former `hg_a9_g27_response_complete` proof while the product decision is open. When Human Guidance settles the affected behavior, create a narrow temporary proof, repair the owning boundary if needed, and remove it. No compliant closure, accepted test, or checklist completion is claimed. |
...
| C536 / L322,L324                                   | **Implemented and independently reviewed; acceptance remains open.** The fixed point-only CSV/TSV contract has exactly `roster_id`, `roster_name`, `assessment_id`, `assessment_title`, `status`, `points_earned`, and `points_possible`. It uses the ordinary authorized Gradebook projection, protects spreadsheet text, omits unnecessary FERPA data, and leaves Course weighting and percentages to the home LMS.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Source implementation and independent review resolve the former terminal product question. Run a temporary actual-role route proof for permitted co-Instructor and concealed unrelated Instructor, Student, expired-session, and unavailable-Course cases; then run canonical-browser acceptance for both downloads, exact attachment bytes, keyboard/busy/error feedback, and header-only export. These gates, not the implementation review, determine checklist closure; remove the temporary probes.                                                                                                                                                                                                                                                                                                                                                         |
...
| C536 | source implementation and independent review are complete; A2 authorization, A4 export UI, and security/privacy owner still gate actual-role and canonical-browser acceptance |
...
Canonical routes are `/courses/:courseInstanceId/assessments/:assessmentId`,
`/assessment-attempts/:assessmentAttemptId`,
`/assessment-attempts/:assessmentAttemptId/summary`,
`/instructor/courses/:courseInstanceId/assessments/new`, and
`/instructor/courses/:courseInstanceId/assessments/:assessmentId` with `/questions`, `/properties`,
`/student-view`, and `/delivery-check`. Parameters remain `:assessmentId` and
`:assessmentAttemptId`. Canonical JSON spellings are `assessment`, `assessmentId`,
`assessmentAttempt`, `assessmentEntry`, `assessmentStatus`, `assessments`, and
`blueprint_assessment_id`; new public JSON never emits generic Assignment/attempt names.
...
This checkpoint unlocks only when both conditions are true:

- the **Current SQL lock ledger** in `sql_human_guidance_audit.md` has no required SQL-owned row
  marked `Open`; and
- C44 has connected proof for both distinct Ribbon requirements, and every high-priority Instructor
  finding I01-I06, I13, and I14 in `UI_UX_USABILITY_AUDIT.md` has a current rendered disposition.

Then, before the remaining compliance work finishes, run one fresh replay of the canonical Live
Demo runtime that captures every current manifest
...
it creates no alternate release path. Re-audit the fresh corpus in
`UI_UX_USABILITY_AUDIT.md` and record each prior finding's current disposition before continuing.
R1 remains the final freshness-verification gate after all later corrections.
...
The plan covers every currently applicable HG bullet: the checklist, gap map, correction milestones
wherever evidence lives (`src/`, `crates/`, `schemas/`, `tests/`, `launchers/`, `local_stack.py`,
`local_stack_control/`, `containers/`), and generated evidence under `docs/`. Deferred product
behavior remains recorded as authority but is not current implementation scope. Work with no HG
bullet behind it (cloud deployment, backups, the compliance report folder rename) belongs to other
plans.

### Human-facing reference IDs

- A public ID is one canonical value across SQL, Rust, JSON, URLs, object storage, hashes, logs,
  and browser UI.
- Public IDs use uppercase ASCII and the Crockford Base32 alphabet
  `0123456789ABCDEFGHJKMNPQRSTVWXYZ`.
- Human entry accepts lowercase Crockford characters and `O`/`o` for `0`.
- Human entry accepts `I`/`i`/`L`/`l` for `1`.
- Normalize accepted human input before canonical syntax and checksum validation.
- A public-ID checksum uses unsalted SHA-256 of the uppercase ASCII canonical characters.
- The checksum input includes a type prefix and excludes only separators and the checksum position.
- The checksum is the Crockford character mapped from the high five bits of digest byte 0.
- Blueprint Courses use `BPXXXXXXXZ`; Course Instances use `CIXXXXXXXZ`.
- Assessments use `AXXXXXXXZ`; Account support references use `UXXXXXXXZ`.
- Each prefixed format has seven random characters and a final embedded checksum.
- Published Questions and Question Pools use `XXXX-ZXXX` and share one namespace.
- Human-entered Question IDs may omit the hyphen; PLE restores the canonical hyphenated form
  before validation and lookup.
- Generation enforces global uniqueness, retries random collisions, and never reuses an issued ID.
- Native UUIDs remain internal identifiers unless a human-facing workflow needs a public ID.
- A global public-ID reservation registry is current implementation evidence for uniqueness and
  non-reuse. Human Guidance requires those outcomes, not that implementation mechanism.
