...
| --- | --- | --- | --- |
| C880 | OPEN current-authority reconciliation: shared-Question-ID Assessment relationships and shared/added/removed content correspondence through names/order/structure changes. Prior contributor-verified three-snapshot internal-ID projection remains historical evidence, not the required matching model. | C412,C414. | Prior accepted temporary consumer covered internal IDs/order, module-parent/current-name changes, source-only/fork-only/overlapping changes, pins, defaults and duplicate-ID rejection. Preserve that bounded evidence; corrected matching proof remains open. No permanent fixture inventory. |
| C881 | OPEN related-pair coverage: any visible related Blueprint Courses in the same fork lineage, with ordinary visibility and nonenumeration. Prior contributor-verified direct-source/current-head read evidence remains valid only for that scope. | C412,C51,C880. | Accepted corrected-current-head build and direct-source connected proof; no permanent test. Independent earlier evidence review accepted, not proof of all related pairs. |
| C882 | OPEN corrected server review: on-request canonical JSON with Question-ID Assessment matching and related-pair coverage. Prior contributor-verified direct-source review retains valid names/current-head/ordinary-visibility/no-persistence proof. | C880,C881. | Accepted prior actual HTTP exact-current-head/name-ETag/visibility/`no-store`/zero-`ple_data`-mutation proof; fixed Questions only, Pool semantics outside receipt. No permanent test. Corrected matching/related-pair evidence remains open. |
| C883 | OPEN corrected matching/Apply reconciliation: fresh local Assessment IDs on forks and no persistent cross-Blueprint Assessment lineage. Historical contributor-verified selective-apply operation used explicit reviewed selections, coherent full-tree validation, Revision CAS and metadata ETag; it does not close corrected-contract Apply where cross-fork internal-ID equality is assumed. | C882. | Preserve actual HTTP/pure-domain proof at `/private/tmp/ple-fork-apply-http-artifacts.1e5gy8`: four `412` zero-write CAS paths, mixed changes, pinned old Pool retention, fork-only and daughter/provenance preservation, rollback, no-op and authorization denials. No permanent test; earlier backend review accepted for its recorded scope only. It does not claim Student Work or concurrent stale-after-wait behavior. |
| C884 | OPEN: the canonical workflow baseline is complete (`/private/tmp/ple-fork-workflow-baseline.log`: PASS); remaining connected Apply and Instructor workflow evidence stays open. | C883. | C882/C883 substantially cover the backend. C884 and C413 remain open for residual UI/connected workflow proof; no product-wide closure or permanent test is claimed. |
