...
- Expected behavior: C7 may now implement the explicit `Empty | Adopted` Course root and its
  independent teaching state (memberships, deadlines, releases, and Course settings). It cannot
  close the full started-empty authoring/delivery behavior until C503 supplies the tagged direct-
  versus-adopted Assessment origin; the current `assignments` schema requires a nonnull Blueprint
  triplet and cannot represent direct Course Assessment creation without a sentinel or compatibility
  path.
...
- HG bullets closed: "Themes should use biome and habitat names, such as Forest, Grassland, Ocean, and Desert."
...
| C809 | A6 C207 contributor: atomic Assignment save/release synchronizes latest due and rejects due date past active limit. | `assignments.sql`, `ple_api.save_assignment*`, `postgres/assignment_release.rs`; C206/C207. | Expanded `hg_a6_c207_deadline_probe.py` with deterministic concurrency; durable invariant only if it earns promotion. |
...
| C838 | C311 canonical-source and publication owner: for one C824 family at a time, validate its recorded canonical algorithmic author source and `source_format`, retain exactly one canonical `.pg` or `.pgml` file with its provenance/license, and create exactly one ordinary WeBWorK Question lineage from that file. `pgml` requires C824's fully-compliant classification; traditional or mixed source remains `pg`. This is no parser and no separate backend. | Canonical `content/genetics/pg/topicNN/` source and manifest mapping; ordinary publication only: `schemas/base_schema/question_authoring_operations.sql`, `crates/server/src/question_publication.rs`; C824. | The redundant static source bulk is already removed. Ignored per-family source/publication fixture records exactly one canonical source file with a matching format/extension and one lineage, without an adapter heuristic, runtime PG parser, separate backend, or historical Question ID/revision/pin rewrite/delete. |
| C839 | C311 canonical-source acceptance owner: before catalog mutation, verify the canonical PG/PGML source, provenance, deterministic parameter contract, and representative rendered/grading instances for one accepted family. The temporary 42-source acceptance passed for 41 official biologyproblems-website PGML sources plus HLA: render/lint/whitelist, repeatable/reseeded variation, matching `1`/`.83`, which-one `1`/`0`, and Poisson `1`/`0`. This is source acceptance, not row-for-row equivalence to an inferior static expansion or catalog migration. | C838 new lineage and one affected Genetics family; hands C840. | The accepted source probe was temporary and removed. C840/C841 still require ordinary publication, expected-current Blueprint CAS, and retirement proof; the normal curriculum-content runtime gate remains blocked by the current AWS Smithy dependency incompatibility. |
| C840 | C311 catalog-state closure owner: after C839, require the expected-current Genetics Blueprint Revision CAS and reconcile the accepted family's one canonical algorithmic Question with the catalog. Do not create an implicit Pool. Revise an existing Pool only to retire redundant generated variants, never create an empty Pool, and preserve every deliberate Pool of distinct Questions, substituting the canonical Question only where its former static predecessor was a member. An emptied redundant Pool retires through C841. | Current Question/Pool/Blueprint revision operations; C838,C839. | Redundant static source files have already been removed; ignored per-family Pool-purpose inventory plus Blueprint CAS fixture still must prove one canonical Question, no empty or implicit Pool, and preservation of deliberate distinct-Question Pools. On CAS failure, retain all current catalog state; never delete evidence. |
...
#### C311 current Genetics migration diagnostic

The former bundled Genetics inventory had 119 numeric same-question directories
with 20,579 static PG files. The user removed that redundant source bulk and
the old `pg/genetics/` layout; the 42 canonical sources now live under
`content/genetics/pg/topicNN/`. This source cleanup is not catalog
reconciliation evidence. C824 still records each canonical author source,
provenance/license, and generated-variant coverage; C838-C839 must publish and
accept one canonical algorithmic source; and C840 must distinguish redundant
former-variant membership from an intentional Pool of distinct algorithms
before its CAS catalog reconciliation. No lineage, Pool, Blueprint, or
historical pin may be deleted on source-removal evidence alone.
...
| C847 | C209a retention-notification schema contributor: add late `course_retention_notifications.sql` receipt/lease tables and the dedicated NOLOGIN notifier capability. The unique notification identity is exactly `(course_id, action_kind, due_at, recipient_account_id)`, with `action_kind` exactly `warn_inactive` or `notify_archive`; archive/delete never notice. A receipt starts DB-owned `next_attempt_at = due_at`. At one evaluated timestamp, claim requires action still due, `next_attempt_at <= evaluated_at`, no provider acceptance, and absent/expired lease; order claims by `(due_at, id)`, increment attempt count, and set next attempt to lease expiry. The capability derives and returns exactly one verified Instructor destination from the deduplicated assigned-Instructor plus active-Instructor-membership union; it exposes no generic lookup. Store the idempotency key before provider invocation, provider acceptance, and delivered receipt. | C206's due actions and current Course membership/assignment facts; hands C848,C849. No invitation export/Mail.app table or generic email queue. | Ignored fresh-PG17 concurrency/crash fixture proves action-kind refusal, recipient deduplication/one verified destination, one identity/receipt, evaluated-at claim predicate and `(due_at,id)` order, `SKIP LOCKED`, lease expiry, durable idempotency-key reuse after crash, terminal acceptance, and retry state; remove. A single stable one-identity/one-delivery outcome test is a candidate only after every `PYTEST_STYLE.md` criterion passes. |
| C848 | C209b typed LDA notification Store contributor: expose only typed claim, record-provider-acceptance, record-delivered callback, and unaccepted-attempt `FailureRecorded` operations through C847's NOLOGIN notifier capability. Provider acceptance is terminal for sending; callbacks update the same receipt/idempotency key. A successful `FailureRecorded` clears the lease and deterministically sets `next_attempt_at = failed_at + min(3600 seconds, 60 seconds * 2^(attempt_count - 1))`; an action no longer due cannot retry, and accepted receipts never resend. | C847; hands C850,C851. It has no invitation, Account search, Student Work, session, object, or renderer API. | Ignored typed PostgreSQL Store/capability-denial fixture proves failure backoff/cap, accepted no-resend, action-no-longer-due retry exclusion, callback same-receipt update, and crash recovery uses the durable idempotency key; remove unless a stable external outcome earns promotion. |
| C849 | C209c server delivery contributor: define provider-neutral `CourseRetentionNotificationDelivery` and disabled `NotConfigured` adapter. It accepts only a fixed redacted sign-in-only message with no Course identifier/title, raw ID, FERPA data, capability, or recovery link; it maps provider acceptance/delivery to C848 receipts. `NotConfigured` records non-send failure, sends nothing, and never returns fake success. | C847; hands C850,C851. Provider credentials are operational configuration; Live Demo `NotConfigured` is not delivery evidence. | Ignored adapter outcome matrix proves message redaction, disabled recorded failure/no-send, terminal acceptance, and delivered callback mapping; remove. No provider mock call-order test is permanent. |
...
| C851 | C209e orchestration contributor: after C215 plus C847-C850, a late run attempts required earlier notices in C847 `(due_at,id)` order through the typed boundary, then executes archive/delete after a successfully recorded `FailureRecorded`. It stops the notice lane only for an unknown typed Store state and always continues retention transitions; it never calculates dates or policy. A crash before send leaves the receipt leased until expiry; a crash after provider invocation reuses C847's durable idempotency key on an eligible reclaim. | C215,C848,C849,C850; hands C209. | Ignored connected PostgreSQL/server proof covers one evaluated-at claim, due order, lease/crash reclaim, acceptance no-resend, deterministic failure backoff through cap, action-no-longer-due exclusion, repeated-run idempotency, `FailureRecorded` nonblocking archive/delete, and unknown Store state notice-lane stop with continued retention transitions; remove. Consider one stable externally meaningful nonblocking retention outcome only after `PYTEST_STYLE.md` review. |
...
| C893 | Bulk-metadata server contributor: after C365/C367 and C338's accepted canonical distinct nonempty selection, an active vetted-Instructor route accepts only the typed closed metadata patch and bounded selection, validates the client-provided opaque idempotency key and binds it to actor/request digest, applies C365's all-or-none CAS/idempotency command, and returns only whole outcomes. It has no generic coordinator, queued job, partial result API, or arbitrary JSON patch. | C365,C367,C338; hands C366,C368. | Ignored server request matrix proves bounded closed decoding, role/target nonenumeration, idempotent repeat, stale zero-write, and same-key/different-request refusal; remove. Retain only a stable external authorization or all-or-none CAS outcome if it earns promotion. |
...
| C910 | General-feedback closure owner for the three feedback behaviors: add the smallest author-managed general-feedback metadata field and authorized Question-delivery projection. It treats backend feedback as transient unless robust preservation exists, never extracts or reconstructs transient feedback from backend source or output, and keeps PLE-managed general feedback separate from backend-generated interaction feedback. The accepted fresh-PG17 procedure created an explicit `webworkPgml` Draft/binding, saved general feedback, published Revision 1, made a feedback-only edit, published Revision 2, and read both immutable feedback values with the same format/path/checksum; the SQL `RETURNING` output-variable ambiguity was qualified and independently reviewed. This does not require historic renderer-feedback capture and does not block C331 or C362. | C307; independent of C331,C362. | The temporary database proof and workspace were removed; full TypeScript checking also passed. Retained browser metadata is explicit about format and local pickers keep unknown values `null`. The authorized Student HTTP projection/release remains unverified because the server build is blocked by the current AWS Smithy dependency incompatibility; do not close these three behaviors yet. |
...
independent and closes D05 only after its existing baseline gate passes. `C311 -> C824 -> C838 ->
C839 -> C840 -> C841` processes one biologyproblems.org WeBWorK family at a time: its canonical
algorithmic source replaces static variants while preserving intentional Pools of distinct algorithms. `C842 -> C843`; `{C318, C341, C842, C843} -> C369`; `{C842, C369} -> C844`;
...
| C58 | One parser supports ordinary words, quotes, minus, PLE field tags. | `crates/server/src/question_library.rs` | Closes G-A4-14 (7). | C8; pending A7 subject/topic model. | `source source_me.sh && cargo test -p server_core question_library`; `bash tests/e2e/e2e_live_demo_question_library.sh --api` |
...
| C61 | Assessments ribbon/Due Soon/Templates use Assessment labels and dense state rows. | `src/ribbon/ribbon_catalog.ts`, `src/pages/assignments_due_soon_page.tsx` | Closes G-A4-17 (6). | C12; pending A9 template query. | `node --test tests/test_assignments_due_soon_client.mjs`; `bash tests/e2e/e2e_live_demo_course_instance.sh --browser` |
| C62 | Assessment workspace has distinct named Question/Properties tasks. | `src/pages/assignment_workspace/assignment_workspace_live_page.tsx`, `src/pages/assignment_workspace/assignment_workspace_questions_page.tsx`, `src/pages/assignment_workspace/assignment_workspace_policies_page.tsx` | Closes G-A4-18 (5). | C12, C61. | `node --test tests/test_assignment_workspace_questions.mjs tests/test_assignment_workspace_policy_model.mjs` |
| C63 | Question editor shows order, inspection-before-add, Search/Browse paths. | `src/pages/assignment_workspace/assignment_workspace_questions_page.tsx` | Closes G-A4-19 (3). | C60, C62. | `node --test tests/test_assignment_workspace_questions.mjs`; `bash tests/e2e/e2e_live_demo_question_library.sh --browser` |
| C64 | Policy persists Assessment Question-order randomization only; choices remain Question-owned. | `crates/domain/src/effective_assignment_policy.rs` | Contributes to G-A4-20 (2); flips none. | pending A7 choice-randomization contract. | `source source_me.sh && cargo test -p domain effective_assignment_policy` |
| C65 | Properties UI presents Assessment-order and Question-owned-choice behavior. | `src/pages/assignment_workspace/assignment_workspace_policies_page.tsx` | Closes G-A4-20 (2). | C62, C64. | `node --test tests/test_assignment_workspace_policy_model.mjs` |
...
| C207 | D15, D19. Assignment release/save synchronizes `latest_assessment_due_at` and rejects `due_at` past existing `active_until_at`, atomically under concurrent Course/Assessment edits. | `schemas/base_schema/assignments.sql`; `ple_api.save_assignment*`; `crates/learning-data-access/src/postgres/assignment_release.rs`; depends on C206 and C809. | `source source_me.sh && python3 tests/_temp/hg_a6_c207_deadline_probe.py` |
...
The accepted decisions are locked in the map: in-app Watch projection for Revision/publish/archive/restore; six-month Active-to-Inactive policy with warning/deadline/FERPA-clock rules; publish-new Private Blueprint rather than Instance conversion; import into a new actor-owned Private Blueprint; subset Change-Proposal acceptance with Revision/ETag conflict handling; and C880-C884's explicit, baseline-based Blueprint fork update. Backend evidence is not a closure where the map declares an Instructor browser workflow (C411, C413, C420, C421, C423-C425) necessary.

### C880-C884: Blueprint fork status and selective-update chain

The architect-approved design in `docs/DESIGN_DECISIONS.md` is binding. A fork retains immutable
origin plus private per-selectable-unit sync baselines. Its status is exactly `up_to_date`,
`updates_available`, `updates_available_with_conflicts`, `conflicts_require_manual_review`, or
`source_unavailable`. Only the fork owner may inspect a Public or Archived source; a source made
Private becomes nonenumerating `source_unavailable`.

The only selectable units are short name, long name, a whole Blueprint Assessment identified by
stable reference (including settings, Questions, and Pools), and the ordered Assessment list.
Ownership, visibility, Star, Watch, adoption, Course, and Student state are excluded. The
base/source/fork comparator classifies each unit as `safe`, `already_applied`, `conflict`, or
`not_applicable`; only `safe` applies. The server issues the candidate identity and digests. One
candidate per request uses source/fork Revision plus metadata ETag and request-checksum
CAS/idempotency; it never overwrites automatically and advances only an applied unit baseline.

| ID | Atomic owner and outcome | Dependencies | Temporary proof and permanent-test decision |
| --- | --- | --- | --- |
| C880 | New `schemas/base_schema/blueprint_fork_sync.sql`: immutable origin and private per-unit baseline storage with source-unavailable nonenumeration data shape. | C412. | Ignored fresh-schema/RLS and baseline-invariant matrix; remove. Retain only an externally meaningful authorization or immutable-origin contract if it satisfies every `docs/PYTEST_STYLE.md` question. |
| C881 | Canonical comparator in `crates/question_model/src/blueprint_course/`: classify each allowed unit by base/source/fork as `safe`, `already_applied`, `conflict`, or `not_applicable`; never compare excluded state. | C412,C414. | Ignored deterministic comparison matrix covering every unit/status and excluded fields; remove. No permanent fixture inventory. |
| C882 | Typed `BlueprintForkSyncStore` data-access operations persist/read baselines and server-issued candidate digests without exposing a private source or changing a fork. | C880,C881,C51. | Ignored PostgreSQL authorization/idempotency/store matrix; remove. Retain only a small stable privacy or CAS contract if it earns promotion. |
| C883 | One server status/selective-apply operation authorizes only the fork owner against Public/Archived sources, returns exact statuses, validates source/fork Revision + metadata ETag + request checksum, applies exactly one `safe` candidate, and advances only that baseline. | C882. | Ignored server stale/CAS/replay/conflict/nonenumeration matrix; remove. No mock call-order test. |
| C884 | Connected proof: Public/Archived owner visibility, Private nonenumerating source, all five statuses, all four unit classes, one-safe-candidate application, no-auto-overwrite, stale/replay rejection, and applied-unit-only baseline advance. | C883. | Ignored fresh PostgreSQL/server connected proof; remove after review. Retain only a narrow stable authorization or CAS outcome if all `PYTEST_STYLE.md` criteria approve it. |

`C413` remains blocked until `{C47,C50,C884}`: it is the only Instructor UI closure for discovery,
review, and deliberate selected application. Question-level hunk selection is deliberately
unlocked and N/A: it is not required for the defined whole-unit update behavior and does not block
C880-C884 or C413.
...
| C503 / L19,L23,L107,L302 | Direct-cutover current Assignment schema/model to canonical Assessment entry model with tagged direct-versus-adopted Assessment origin; remove the current nonnull Blueprint-triplet requirement for direct Course creation without sentinels or compatibility paths. Retain `domain/scoring.rs` as the scoring boundary. | `source source_me.sh && python3 tests/_temp/hg_a9_g02_content/probe.py`; direct and adopted variants retain kind/order/points and origin; repair content model, remove. |
...
| C510 / L52,L57,L59 | Practice/Bonus scoring in `domain/scoring.rs`, policy, and schema. | `source source_me.sh && python3 tests/_temp/hg_a9_g09_bonus_practice/probe.py`; prove required score cases; repair rule, remove. |
| C511 / L63,L67 | Quiz/Exam restrictive defaults in policy/UI, while allowing stricter-than-Regular behavior. | `source source_me.sh && python3 tests/_temp/hg_a9_g10_restrictive_defaults/probe.py`; repair defaults, remove. |
...
| C518 / L263 | Mandatory time limit before Attempt creation; expiry semantics remain unchanged. | `source source_me.sh && python3 tests/_temp/hg_a9_g17_time_required/probe.py`; reject missing limit; repair validation, then remove; conditional retain. |
| C519 / L164,L166,L169,L171,L173 | `domain/validation.rs` and current `server/assignment_release.rs` become canonical Assessment release validation module with actionable invalid errors and valid control. | `source source_me.sh && python3 tests/_temp/hg_a9_g18_release_rules/probe.py`; repair validation; conditional retain compact cases. |
| C520 / L158,L160,L162,L177 | Unreleased/release gate in schema, server, and release API: fail before validation and succeed after. | `source source_me.sh && python3 tests/_temp/hg_a9_g19_release_gate/probe.py`; repair server gate; conditional retain. |
...
| C524 / L54,L189,L191,L193,L195 | Separate disclosure and feedback release in policy/summary/UI; Type-specific post-submit disclosure is distinct from feedback. | `node tests/_temp/hg_a9_g23_disclosure/probe.mjs`; repair policy/UI, remove. |
| C525 / L198,L200 | **Terminal product question; no code:** Which people complete before Quiz/Exam answers release, and how do enrollment, withdrawal, deactivation, never-started/never-completing Work, and course end change that cohort? | `source source_me.sh && python3 tests/_temp/hg_a9_g24_cohort_question/record_question.py`; fresh report records exact question. Failure blocks implementation; remove. |
...
| C525 | terminal product question |
