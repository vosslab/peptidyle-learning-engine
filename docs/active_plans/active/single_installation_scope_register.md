...
Graphify at commit `dc227871d18d` identifies `TenantContext`, `TenantId`, and `CourseId` as major
cross-area connectors. One-time direct inspection found the historical tenant-shaped identity in 465
...
The left column records pre-SD1 implementation or protocol vocabulary. Tenant terms below are retained
only to identify historical migration/source shapes; the right column is the binding target.

| Current owner or shape                         | Intended owner or shape                                                            | Package  |
| ---------------------------------------------- | ---------------------------------------------------------------------------------- | -------- |
| `TenantId`                                     | Exact `UserId`, `WorkspaceId`, `CourseId`, or immutable content identity           | SD1-B    |
| `TenantContext`                                | Server-derived `ActorContext { user_id, session_id }`, defined only in `session.rs` | SD1-B1   |
| Browser session `tenant` field                 | Account/session projection plus course memberships loaded by course APIs           | SD1-B5/F |
| `ple.tenant_id` and `ple_current_tenant()`     | Transaction-local `ple.actor_user_id` and operation-specific predicates            | SD1-C    |
| Tenant-leading keys and foreign keys           | Globally unique identity plus exact user/workspace/course parent                   | SD1-C    |
| Tenant RLS predicates                          | Current actor, membership, Student owner, workspace relation, or leased capability | SD1-C    |
...
| Tenant-scoped Student identity                 | Course membership/enrollment plus Student `UserId` where required                  | SD1-B2/C |
...
| Tenant-prefixed object key                     | Typed opaque catalog, workspace, or course-record object identity                  | SD1-B4/E |
| Tenant job scope                               | Typed course, workspace, catalog, or system lease scope                            | SD1-B4/E |
| Tenant launch/export/retention scope           | Exact course, assignment, attempt, export, or retention target                     | SD1-B4/E |

## Historical source ownership and successors

| Capability                       | Primary files and symbols                                                                                                                                                           | Successor      |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------- |
| Fundamental identity             | `crates/question_model/src/activity.rs::TenantId`; `crates/learning-data-access/src/session.rs::{SessionSubject,ActorContext}`; `crates/server/src/auth.rs::{AuthenticatedSession,AuthSessionResponse}` | SD1-B1         |
