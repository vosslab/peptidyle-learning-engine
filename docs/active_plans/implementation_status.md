...
course_appearance` passed 6 tests; `cargo tools tsgen`, `npx tsc --noEmit`,
  ...
  **Completed WN1 Presentation correction (2026-09-01):**
  ...
  `theme` is `CourseThemeId`, `revision` is `CourseAppearanceRevision`, and `banner` is nullable
  ...
  decoding rejects unknown theme IDs, noncanonical or out-of-range revisions, surplus fields, retired
...
revision_number }` is the exact immutable version within stable Question lineage. Object Storage,
  PLE JSON, WeBWorK, iMathAS Render Cache Entries, Remote Question Backend Session Authentication,
  ...
  **iMathAS Question Backend terminology cutover (2026-09-01):** RQB1 is accepted historical evidence. **In progress: `WP-SD1-A-TERM-01-RQB2`, after RQB1.** It directly renames the generic remote/backend family to exact iMathAS Session, Challenge, Authentication, Grading Context, Result Token, Result Exchange, Result, and Question Backend Transport ownership, including the `ImathasQuestionBackend`/`imathasQuestionBackend` response/control/Student Response marker. LDA and `2026090102` own schema, RLS, and procedure renames; iMathAS owns launch/result translation; browser copy uses Question Backend only when backend-agnostic. The accepted Session lifecycle, marker flow, relationships, procedures, launch flow, security invariants, and tests remain unchanged.
  `ETLS1`, `ETLC1`, `ETGC1`, and `ETPRT1` are superseded terminology evidence; the former open `ETRGR1` is absorbed.
  Migrations `2026082927` and `2026090102` belong only to RQB1 for its direct
  schema/API/generated/test/documentation cutover, removing orphaned `lti_grade_return` without an
  alias, compatibility path, or successor migration. The exact execute-only
  grading-worker capability, fresh migration/no-op/catalog/restricted-login evidence, both
  PostgreSQL Store service tests, independent least-privilege review, and complete
  `source source_me.sh && ./all_test.sh` acceptance pass.
...
Remote Question Backend and iMathAS routing-binding graph changed atomically. `GradebookSummaryRow` now
...
and Remote Question Backend handoff. Retired-name, generated-module, and source-file inventories remain
...
- **Audited Student work:** `StudentWorkInspectionStore` verifies the exact immutable submission,
  ...
  `IssuedPresentation` or verified `RemoteQuestionBackendResponseControl` `PresentationNotApplicable` evidence before
...
| `2026082927`                                                                                                                        | `WP-SD1-A-TERM-01-RQB1`                                | Direct fresh-schema Remote Question Backend cache, Session, and Result Exchange cutover. It removes the orphaned LTI Grade Return schema and carries no legacy compatibility object.                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `2026090102`                                                                                                                        | `WP-SD1-A-TERM-01-RQB1; WP-SD1-A-TERM-01-RQB2`          | Fresh-schema iMathAS Question Backend Session/Result Exchange Store; RQB2 directly renames the complete generic family and its `ImathasQuestionBackend` marker without changing tables, relationships, lifecycle, or procedures. `ple_app` has authenticated Session-only operations; `ple_worker_login` has only exact claim/commit SECURITY DEFINER execution without protected-table access. No aliases, parallel record, or LTI Grade Return boundary exists. |
