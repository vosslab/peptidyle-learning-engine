...
This is the sole mutable registry for the global current-package handoff and shared migration allocations. The [implementation plan](implementation_plan.md) and active
...
current plan. Durable product decisions remain in [Human Guidance](../HUMAN_GUIDANCE.md); package history and detailed receipts remain in the
...
- **Current scoped correction package:** `WP-SD1-A-TERM-01` aligns terminology with Human Guidance
  ...
  Workspace compilation, Question Source persistence, trusted source transfer, and generated Draft Question summaries now separate Draft Question Content from its exact Draft Question Revision Reference;
  persisted Draft Question and Question Source UUIDs stay in the server data-access boundary while summaries use opaque `D-` Draft Question References backed by bounded schema references, and PostgreSQL resolves private rows only within the authorized Authoring Workspace; the browser authoring model has no mounted Draft Question Store, editor route, or integrated publication coordinator yet. Focused Rust/browser decoders, TypeScript, links, formatting, and diff
  ...
  released Assignment Entry and Question Pool Item snapshots, exact selection outcomes, UUIDv5-backed Issued Question identity from frozen attempt content, issued scoring facts, and same-Student reuse provenance; `domain::select_question_pool_items`
  ...
  **Question Classification cutover:** PLE-owned model, PLE Question JSON, generated
...
- **Current package:** `WP-SD1-A-decisions-and-impact-contract` - establish the single-installation
  ...
  Aggregate service, PostgreSQL/RLS, and
...
| `2026082927`                                                                                                                                       | `WP-SD1-C`             | External-tool launches, External Question Provider Cache Entries, External Tool Results, and LTI Grade Returns                                                                                                                                                                                                                                     |
