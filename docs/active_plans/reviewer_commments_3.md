...
Avoid implementing separate databases per tenant initially. At 1,000 instructors, database-per-instructor would produce unnecessary operational overhead. Use one shared PostgreSQL cluster with organization and course ownership columns.
...
Shared catalog and tenant records
...
Do not copy published problems into each tenant database.
...
7. Include tenant IDs on every tenant-owned record.
