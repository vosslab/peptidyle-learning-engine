...
- Production-boundary domain `*Etag` names are forbidden by
  `tests/test_semantic_boundary_names.py`. HTTP `etag` / `ETAG` / `ETag`
  remain header spellings. SQL stale-concurrency messages name Edit Number,
  not ETag. Nested leftover identity and expected-edit JSON is rejected by
