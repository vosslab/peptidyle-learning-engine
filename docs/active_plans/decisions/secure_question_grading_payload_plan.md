...
Current local evidence is from `crates/question_model/src/{envelope,response,activity}.rs`,
`crates/server/src/run.rs`, and the browser API and run-page modules. A representative envelope is
1,095 bytes, a full learner attempt projection is 876 bytes, and the current small submit body is
62 bytes. The important reduction is removing the repeated full attempt projection; shaving a few
bytes from an already small answer is not a latency strategy.
...
server-issued `seed`, title, prompt blocks, public assets and checksums, choices, and
`ResponseDefinition` are render inputs. The browser needs the attempt ID, assignment position,
learner-visible status/timing, and feedback/next policy to manage the activity. It does not need the
attempt's tenant identifier, parameter hash, adapter/renderer/generator identities, source-artifact
object identity, full provenance, or server grading binding. Those fields remain necessary for
server reproduction, support, and audit, but not for active learner rendering.
...
`{"response":{"kind":"multipleChoice","selected":["b"]}}`. After loading the attempt, the
server already knows its learner, tenant, course, assignment, version, seed, timing state, expected
response family, and grading backend. Therefore `kind`, a question ID, version, seed, backend, and
course/assignment IDs are redundant learner assertions; v1 removes them rather than trusting or
cross-checking them.
...
`question_id`, serialized `submission`, and client-selected `technology`. ADAPT's server then reads
stored QTI JSON to infer the question type and computes scores. Its WeBWorK route uses an iframe
and signed JWT answer exchange, where renderer answer objects cross the browser boundary. The
inspected ADAPT code has no server-issued attempt binding or canonical presentation digest. PLE
adopts server grading and deterministic generation; it intentionally avoids browser-selected
technology, resource-ID authority, whole-assignment learner payloads, and public upstream result
transport.

| Boundary      | LibreTexts ADAPT observed behavior                                                                                                                                  | PLE v1 decision                                                                                                        |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| Native render | Fetches rich whole-assignment question records with seeded, student-sanitized `qti_json`, prior response, timing, media, and iframe state.                          | Fetch one active public envelope plus a minimal attempt descriptor; keep full attempt/provenance server-side.          |
...
- Implement `PresentationDescriptorV1`, rendered IDs, minimal attempt descriptors, type-free wire
  decoding, compact receipts, and browser recovery.
...
backend, `kind`, points, component score, or weight. The handler RLS-loads the attempt, checks its
state and timing, resolves the immutable response definition and backend, verifies the presentation
digest, decodes the answer according to that definition, grades server-side, and commits once.

`LearnerAttemptDescriptor` replaces active learner use of the full `QuestionAttempt`. It contains
only `id`, `run`, `assignmentPosition`, `status`, learner-visible timing, `presentationDigest`, and
policy-projected next/feedback state. `questionVersion` and `seed` appear once in the public
presentation envelope that feeds Wasm; they are not repeated in this descriptor. Full attempt projections remain
available only through authorized history/support routes. The active render route returns:

```json
{
  "attempt": { "id": "...", "presentationDigest": "pd1_..." },
  "envelope": {
    "version": "...",
    "seed": 0,
    "presentationNonce": "...",
...
rendered surface ID and unsigned normalized coordinates. Bounded parsing rejects duplicate IDs,
unknown properties, invalid hex, wrong order/cardinality, out-of-schema IDs, and coordinates outside
the issued schema. The browser may check shape but never grades. The internal `StudentResponse`
remains a rich tagged enum after this decoder; `kind` is absent from the public wire.
...
The compact receipt is `{ accepted, attempt, outcome, feedback, next }`; `outcome` is either
policy-permitted graded aggregate or `{ "state":"recorded" }`. Deferred feedback reveals neither
component scores nor correct answers.
...
- Files: `crates/server/src/run.rs`, native backend/validation modules, route tests, API fixtures,
  and generated client consumers.
- Behavior: issue minimal descriptors, decode type-free answers after attempt load, enforce digest
  and exact idempotency, grade once, and return compact policy-projected receipts.
- Success: all eight wire vectors reject invalid shape/ID, exact retry returns receipt, changed retry
  is `409` before grade, and mismatch is non-mutating.
...
- Files: browser API/decoders, run page, response widgets, Wasm bridge, Playwright scenarios.
- Behavior: compute digest through Wasm, gate asset readiness, submit the compact body, and restore
  compatible drafts after same-attempt mismatch refresh.
- Success: keyboard-only MC, MA, FIB, MULTI-FIB, NUM, MATCH, ORDER, HOTSPOT, and WeBWorK MC flows
  pass; network traces contain no private fields or browser `kind`.
...
migration and deploys server and browser together. Historical authorized full projections remain
available. Old active route/body usage returns stable `410 contract_retired`. Local synthetic stacks
may be explicitly recreated; production data is never recreated as a shortcut.
