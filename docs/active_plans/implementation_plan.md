...
`WP-SD1-A-TERM-01-RQB1` is accepted historical evidence. `WP-SD1-A-TERM-01-RQB2` supersedes its generic server-boundary naming with exact iMathAS Session, Result Exchange, Result, and Transport terms while preserving its security receipts.

`WP-SD1-A-TERM-01-RQB2` is in progress after RQB1. It directly renames the complete generic remote/backend family to exact iMathAS Session, Challenge, Authentication, Grading Context, Result Token, Result Exchange, Result, Question Backend Transport, and `ImathasQuestionBackend` response/control/Student Response marker terms, and amends fresh migration `2026090102` without aliases or compatibility support. It preserves the accepted lifecycle, relationships, procedures, browser launch flow, security invariants, and tests.
...
3. `SD1-B2` through `SD1-B4` own course/Student, Question Library/Question authoring workspace, and job/object/Remote Question Backend
...
- Keep `RemoteQuestionBackendLaunch` separate from serializable question source. Its `launchUrl` is a
