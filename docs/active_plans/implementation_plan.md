...
| WP-F2 | Add the WASM build path | `expert_coder` | WP-F1 | `wasm-bindgen` output into a gitignored staging dir; a trivial export callable from Node; toolchain pinned |
...
| WP-F4 | Containerize api, postgres, minio | `expert_coder` | WP-F1 | `containers/Containerfile.api` builds a multi-stage slim image under `podman build`; `containers/compose.yaml` brings up PostgreSQL 17 and MinIO with named volumes and creates three buckets; `/health` returns 200 only after a real `SELECT 1` and a bucket probe; credentials arrive at run time from the environment; `docs/CONTAINER.md` records the commands and `docs/MACOS_PODMAN.md` records the macOS machine setup |
