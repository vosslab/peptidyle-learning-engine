...
* ESLint config lives at `eslint.config.js` at the repo root (canonical, propagated). Lint correctness is enforced by `check_codebase.sh` step 4 (`npx eslint --max-warnings 0 '**/*.{ts,tsx,mts,cts,js,mjs,cjs}'`).
...
* Auto-fix path when `./check_codebase.sh` step 5 (`format:check`) fails: run `npx prettier --write '**/*.{ts,tsx,mts,cts,js,mjs,cjs}'` (the `npm run format:write` alias mirrors this).
* `.prettierignore` covers scratch names (`_temp*`), private lane builds (`dist_*/`), and noisy output trees (`node_modules/`, `dist/`, `dist-single/`, `_site/`, `coverage/`, `playwright-report/`, `test-results/`, `blob-report/`, `package-lock.json`). Root `generated/api/` is intentionally not ignored: the check gate regenerates those Rust-owned boundary types and requires them to pass Prettier unchanged.
