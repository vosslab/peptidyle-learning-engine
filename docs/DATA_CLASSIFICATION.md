...
| Course records                                    | Course settings, Instructor membership, Assignment Content, schedules, Course Appearance theme and banner records, and course-management history. Course Stores and server routes own authorization and lifecycle.                                                                                                                                                            | PostgreSQL under forced RLS and typed course scope. Active Instructor Course Members authorize Course-record writes. Active Instructor and Student Course Members may read the current answer-free Course Appearance, its authorized private rendition, active-member Course summary, and public-reference navigation result; this grants no broader Student Course-record access.                                                       | Send only the exact route response data. Appearance readers expose the selected theme plus the current banner's opaque reference and alternative-text metadata; authorized rendition delivery returns the selected normalized image. Student assignment presentation is delivered through Assignment Access, not general course-record access.                                                                                                                                                                                                                                                                       | Course lifecycle archives or deletes course-owned records according to [RETENTION_POLICY.md](RETENTION_POLICY.md). Shared presentation assets and private workspace drafts remain outside that deletion.                                                                                                                                                                                                                                                                                                                                                                                    | Implemented current Course Appearance, active-member summary, navigation, and authorized rendition paths require continuing focused authorization and operational-recovery evidence; no broader Student Course-record reader is implied.                   |
...
| Private Question Backend state and opaque backend records | iMathAS Session Authentication, iMathAS Launch State, iMathAS Result Tokens, launch cookies, and the exact WeBWorK backend-owned document and submitted response bytes. iMathAS-specific state remains in its backend Session; the WeBWorK adapter/server lifecycle owns the WeBWorK records. | Attempt-bound private persistence and server-held Sessions; Question Backend calls receive trusted server-built requests only. WeBWorK E1 stores no backend lifecycle state or replay mapping. | A Student receives only the authorized exact WeBWorK document for their issued Attempt and a policy-permitted result View. Upstream source, renderer configuration, renderer-reserved fields, raw score objects, iMathAS tokens, credentials, and backend state never enter browser DTOs, URLs, logs, or browser persistence. | Retain the WeBWorK document and opaque response only as Course and Student Question Attempt evidence while needed for authorized grading and record retention. Remove them with the associated Student Record unless a separate immutable Question Source rule applies. iMathAS state follows its backend Session and linked Student-record retention. | Current source proves the WeBWorK opaque document, response, and state-free E1 boundary, and the separate iMathAS session boundary. Broader typed-scope acceptance remains required. |
