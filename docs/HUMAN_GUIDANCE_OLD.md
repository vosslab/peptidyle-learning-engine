...
- Use an open question model for vetted Instructors: make every published assignment question
  visible to every vetted Instructor. Keep question drafts private until publication so unfinished
...
- Use Sysadmin, Instructor, and Student as the current live-product personas.
...
- Do not let Sysadmin status alone create or teach a course; use the explicit Instructor approval
  path before a Sysadmin creates or teaches a course.
...
- Consider a future Student Observer through an explicit, narrowly authorized relationship.
