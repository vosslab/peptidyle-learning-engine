...
| `postgres-major-guard`          | Reads an existing `PG_VERSION` before PostgreSQL starts.                                                                                   | Read-only volume, no network, and refusal when the retained volume does not match the selected image's PostgreSQL major. It never migrates or deletes data.            |
