...
- Extended the PLE Question JSON cutover through its durable pilot payload path, Wasm parity corpus and tests, issued-attempt capability, generated browser contract, strict decoder, editor accessibility identifiers, and current contracts. It now accepts a bounded optional source `questionHint`, compiles it as separate private pre-response `QuestionHint` content, and keeps it out of the public preview and private grading record; revision persistence and issued-Question delivery remain tracked. Hotspot public presentation, private Answer Key terminology, source validation, and interchange documentation consistently call the selectable geometry a Hotspot Region. Every active PLE-owned format boundary now uses PLE Question JSON rather than the retired layout term; focused Rust, fixture, generated-contract, browser, formatting, and diff gates pass.
