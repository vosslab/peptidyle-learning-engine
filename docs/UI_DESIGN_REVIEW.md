...
| Question Library                    | Uses All Questions, My Questions, My Question Drafts, Starred, and Watched as distinct interface Tasks. All Questions and My Questions query Published Questions; My Question Drafts navigates to isolated private authoring storage. It allocates the useful screen width to Question Search, facets, and results. The result viewport grows with loaded rows up to a shared cap instead of leaving a large empty bordered box. | [filtered library](screenshots/instructor/question_library_discovery/03_filtered_library_laptop.png) and the full-width/few-row geometry contract                                              |
