...
| WeBWorK     | External standalone `/render-api` client, bounded PGML projection, server-only grading, sanitized immutable render cache, and private stateless container | The licensed `RadioButtons` pilot passed live render/grade/cache/outage/browser acceptance. Broader PG controls require their own source and live evidence. |
