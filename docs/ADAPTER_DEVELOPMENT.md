...
adapter, not for defining a new learner response family. The shared public contract is
...
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Native flat | Static PLE `singleChoice` compilation, client rendering, server grading, hints, and per-question timing                                                       | Accepted v1 path. Additional flat families are planned under WP-RC5.                                                                                                                    |
...
| WeBWorK     | Pinned shipped `/render_rpc` client, one PGML `RadioButtons` projection, server-only grading, sanitized immutable render cache, and private container profile | Static and recorded contract checks are implemented. RC3 is not accepted until its required live OCI, PLE E2E, browser-boundary, and independent security gates pass.                   |
...
not generalize its one-radio path into broad OPL or generic PG support.
