...
| Browser-Wasm load proof                 | `tests/playwright/e2e/instructor_authoring.spec.ts` through the canonical owner                                                   | The instructor scenario sees the inert `data-runtime-mode="wasm"` diagnostic and completes an ordinary authoring workflow.              | Source and unit checks cannot prove that the production bundle initialized Wasm in Chromium. | Required for browser-Wasm claim.            |
