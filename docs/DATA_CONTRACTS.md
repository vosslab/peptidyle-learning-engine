...
| Student response                           | Student, only within an issued attempt                                             | Typed PLE-native response or bounded backend-owned ordered pairs | Immutable Question Submission and Question Submission Receipt                                                                            | Structural/membership failure receives no grade                                                          | [ASSESSMENT_PAYLOAD_DESIGN.md](ASSESSMENT_PAYLOAD_DESIGN.md)                   |
...
| -------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| Render         | **Implemented** foundation         | Answer-free PLE-native presentation or exact backend-owned document and safe presentation metadata | Key, rubric, Question Backend Version, policy authority, and immutable attempt state                           | [ASSESSMENT_PAYLOAD_DESIGN.md](ASSESSMENT_PAYLOAD_DESIGN.md) |
| Submit         | **Current compatibility contract** | Exact Question Attempt route and tagged `StudentResponse`, including bounded backend-owned ordered pairs | Account, Student, exact course/Assignment Attempt, version, seed, backend, policy, and author-declared Question Type | [ASSESSMENT_PAYLOAD_DESIGN.md](ASSESSMENT_PAYLOAD_DESIGN.md) |
...
`kind` selects a PLE-native Question Response Control in the current render
payload. A backend-owned document selects the generic capture component instead; its controls do
not identify an educational Question Type. The immutable author-declared Question Type belongs to
the Published Question Revision and supports PLE discovery and labeling. The reserved compact
submission contract omits `kind` because the attempt already selects the expected response format.
