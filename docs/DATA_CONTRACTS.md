...
| Draft source                               | Authorized instructor workspace                                                    | Never Student-visible; bounded Instructor result only       | Workspace, source object, revision history                                                                                               | Refuse unauthorized, unpublished, or stale revision access                                               | [QUESTION_MODEL.md](QUESTION_MODEL.md)                                         |
| Published question                         | Publication transition and immutable version                                       | Answer-free render only                                     | Published Question Revision records and immutable Question Source assets                                                                 | Refuse missing version, unsupported public shape, or altered Source Object Checksum                      | [QUESTION_MODEL.md](QUESTION_MODEL.md)                                         |
...
| Render presentation                        | Trusted backend reproducing version and seed                                       | Prompt, public assets, response shape, presentation binding | Question Attempt Reproduction Details and private replay state                                                                           | Refuse inconsistent reproduction or unsupported render                                                   | [ASSESSMENT_PAYLOAD_DESIGN.md](ASSESSMENT_PAYLOAD_DESIGN.md)                   |
...
| Private grading records                    | Private Draft Question Revision, adapter, or grader                                | Never                                                       | Protected database/object/provider state                                                                                                 | Refuse if unavailable, malformed, or not authorized for the backend                                      | [DATA_CLASSIFICATION.md](DATA_CLASSIFICATION.md)                               |
...
| Jobs, leases, and aggregates               | Worker lease and generation fence                                                  | Coarse progress or aggregate result only                    | Job, current Job Lease, aggregate, and audit records                                                                                     | Stale lease/generation cannot commit; cleanup remains retryable                                          | [FAILURE_RECOVERY.md](FAILURE_RECOVERY.md)                                     |
...
| -------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ |
| Render         | **Implemented** foundation         | Answer-free prompt, public assets, Question Response Control shape, and safe presentation metadata | Key, rubric, Question Backend Version, private replay mapping, and policy authority                           | [ASSESSMENT_PAYLOAD_DESIGN.md](ASSESSMENT_PAYLOAD_DESIGN.md) |
| Submit         | **Current compatibility contract** | Question Attempt route, idempotency key, and tagged `StudentResponse`                              | Account, Student, exact course/Assignment Attempt, version, seed, backend, policy, and expected Question Type | [ASSESSMENT_PAYLOAD_DESIGN.md](ASSESSMENT_PAYLOAD_DESIGN.md) |
| Compact submit | **Reserved** cutover               | Attempt route, idempotency key, Question Presentation Token, and Question-Type-minimal answer      | The Question Type, complete server-held Question Presentation Checksum, and all attempt-owned context         | [ASSESSMENT_PAYLOAD_DESIGN.md](ASSESSMENT_PAYLOAD_DESIGN.md) |

`kind` is needed in the current render payload so the browser can select the
correct Question Response Control. It remains on the current compatibility submission wire. The
reserved compact submission contract omits it because the attempt already
selects the expected Question Response Format.
