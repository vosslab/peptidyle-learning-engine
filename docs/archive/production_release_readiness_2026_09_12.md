...
> **Superseded grading recommendation (2026-09-13):** this report's proposed Instructor action for
> failed automated grading had no human authority and caused the grading Retry drift. Instructors do
> not grade or retry work in PLE. Recovery means saved responses are submitted automatically when an
> Assignment Attempt expires. The report remains historical evidence only; current plans permit only
> automatic pre-result Job requeue after transient infrastructure failure.
...
- [ ] Prove the user-visible status and internal unfinished-Job outcome when the renderer fails
      during grading. Assignment Attempt recovery remains expiry auto-submission.
...
evidence does not replace the teaching, feedback, renderer-outage status, reload, or Gradebook walkthrough
...
| Response and grading              | [ple_bridge.js](../../src/public/ple_bridge.js) serializes complete form data as ordered `[name, value]` pairs, including legitimate PG hidden fields. [client.rs](../../crates/adapters/webwork/src/http_renderer/client.rs) validates bounded ordered pairs and forwards them to the renderer without recognizing interaction types. `webwork_grading.rs` loads only the persisted backend-owned payload; [worker.rs](../../crates/server/src/worker.rs) reconstructs that opaque response for the one renderer grade call. |
...
| Renderer outage                   | **Superseded finding.** Current workers may internally requeue the same unfinished Job after a transient renderer failure and before any Grading Result exists. Final failure is read-only Instructor-attention status; it creates no Instructor action.                                                                                                                                                                                                                                                                                                                                                     |
...
score; and renderer failure with internal unfinished-Job requeue. Include scientific notation, subscripts, superscripts,
...
- [ ] Show answer-free final-failure status to the Student and Course Instructor. Infrastructure
      repair and unfinished-Job requeue remain internal and never grade accepted work again.
- [ ] Rework the unconditional native PLE post-lease delay while preserving shutdown and lease
      continuity
      guarantees, then measure a realistic submission burst.

The original evidence found an unconditional three-second post-lease delay and treated every
WeBWorK renderer failure as terminal. Current worker code removes the deliberate per-Job delay,
allows bounded internal requeue of the same unfinished Job after a transient dependency failure,
and keeps final failure terminal. The connected teaching walkthrough still needs fresh evidence.
...
environment. The Student should know that submission succeeded independently of grading, and the
Instructor should see when the system needs attention. Instructors do not resolve grading or
trigger another evaluation. Restoring an unavailable dependency permits only the existing
unfinished Job's internal bounded requeue before any result exists.
...
| Instructor Gradebook                                       | Backed; course grade settings and read-only automatic-grading status are different capabilities. |
