...
- The initial projection still makes exactly one `listGradebook(courseId)` request. Assignment Attempt history remains opt-in: it is requested only after the instructor activates that row's public history control. Historical evidence: its former visible label was **View run history**.
...
- Gradebook identity is the stable `(assignmentId, enrollmentId)` pair, so an overlapping server page cannot duplicate a visible student-assignment record. The first newly appended row's history control receives focus after a successful load or retry.
