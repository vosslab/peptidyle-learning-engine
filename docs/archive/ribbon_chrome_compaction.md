...
only, not a dependency: _About Face_ (compact persistent navigation; branding minimized so the
persistent bar consumes less real estate), _Practical UI_ (removing redundant chrome), _Designing
with the Mind in Mind_ (consistent control location), _Refactoring UI_ (the shell serves the
features).
...
deliberately **not** generalized -- the Ribbon keeps three _named_ rows. `docs/UI_DESIGN_GUIDE.md`
...
nothing needs and weaken a contract that carries meaning. What changes is _when a named row is
reserved_.
...
- Changing Ribbon row _size_ tokens, tab or task presentation, or the capability registry.
...
- Evidence or review, when useful: capture the transition once during implementation and report the
  ...
  header stays and the Ribbon is _added_ (0 -> 8rem); after this change a 3.25rem band is replaced
...
- **Invariant this rests on, stated for future readers.** `taskAreas.length > 0` is a reading of
  _declared route topology_, not of capability admission, because of two verified properties of
...
- Acceptance criteria:
  ...
  - Chrome above content is asserted as _the sum of reserved row tokens plus this padding_, derived
...
- [x] M1-M7 accepted with their per-patch gates
- [x] Integration gate green end to end
- [x] Canonical screenshots rebuilt under `docs/screenshots/` and verified against their manifest
- [x] Independent `reviewer` pass on the assembled diff
- [x] M9 documentation updated before the change is described as complete
...
Completed on 2026-09-08. M1-M7 shipped the structural shell frame, removed duplicate viewport
arithmetic, localized navigation styles, made the fallback header conditional, placed the brand in
the Context Row, reserved the Task Row from declared topology, and tightened shell padding. M8 and
M9 completed the offline, browser, connected-stack, screenshot, documentation, and independent-review
gates.

The implementation resolved the two open methods as follows:

- `.ple-shell-frame` owns `grid-template-rows: auto minmax(0, 1fr)`; the Ribbon variant replaces the
  first track with the component-owned composed Ribbon block size.
- Exact document height comparisons were stable, so the height evidence introduced no new
  tolerance.

Recorded deviations from the planning snapshot:

- TypeScript evidence scripts run through `node --import tsx`; bare `node` cannot load their current
  TypeScript imports.
- Current screenshot authority is `docs/SCREENSHOT_CONTRACT.md` plus
  `docs/screenshots/current_capture_manifest.json`. The workflow rebuilt those eight artifacts and
  kept `docs/UI_DESIGN_REVIEW.md` historical instead of attaching disposable `test-results` images.
- Connected acceptance exposed stale used-Course navigation/state oracles and a real accepted-response
  reload defect. The oracles now follow current role-owned surfaces, and the response page restores
  its answer-free server-authorized grading state without starting a new attempt.
- The user-supplied root plan remains staged in place because `docs/REPO_STYLE.md` protects
  pre-existing root active-plan files from relocation sweeps. Only the execution copy moved from
  `docs/active_plans/active/` to `docs/archive/`; this duplicate is deliberate.

Acceptance receipts on the completed tree include `./check_codebase.sh`, the full pytest suite
(5,945 cases before archival and 5,949 after), the production build, every named focused Ribbon
evidence script, the complete fresh-stack serial production-browser suite, the eight-image capture
workflow, and its manifest verifier.

An initial repository-wide aggregate passed the Rust, Node, pytest (5,949 cases after documentation
archival), and preceding PostgreSQL acceptance lanes, then stopped in the isolated iMathAS
statistics-commit oracle with `Forbidden`. The follow-up traced that failure to the Assignment
Attempt completion trigger running under an API owner that deliberately lacked `completed_at`
