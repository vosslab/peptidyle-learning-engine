...
- Replaced the generic `ActivityTimestamp` scalar across the Question Model, domain, adapters, Stores, generated contracts, and Wasm boundary with `Timestamp`; fact-qualified fields continue to state each event or schedule meaning.
- Renamed the immutable Blueprint Course revision JSON column from generic `definition` to `blueprint_course_content`; the fresh PostgreSQL baseline and exact fixture now use that owning record name.
- Completed the Blueprint Content public-contract cutover: exact Blueprint Assignment and Blueprint Course Content Input/View records, canonical encoding, generated API, strict decoders, browser workspace, tests, and `blueprint_assignment_content_editor.tsx` now use `assignments` and `content` rather than generic Definition records and fields.
- Completed the Assignment Content cutover: immutable released-entry storage, Issued Question wire data, rollover source evidence, retention requests and views, generated contracts, strict decoders, fixtures, browser copy, and current operational documentation now use exact Assignment Content rather than generic Assignment Definition vocabulary. Current contracts also name Assignment Status, Assignment Revision, effective Assignment policy, and Assignment Attempt state rather than a generic Assignment lifecycle.
- Renamed the Question Model's generic `definition` source module to `question_content`. Draft Question Content, Question Revision, Question Metadata, Question Backend Locator, Question Response Format, and Question Grading Rule now have one exact owning module, with direct adapter, fixture, re-export, and current documentation consumers using that boundary.
- Removed the hard-coded `PeptideBondGeometryV1` Question and generator from the PLE adapter. The product registry now contains only current PLE Question JSON implementations; the peptide fixture is static `pleQuestionJson` data with regenerated reproduction facts, and verified immutable source bytes remain the sole grading input for static Questions.
- Completed the remaining sole-current Question Presentation and Student View suffix audit. The retired V1 names occur only in the retained replacement checklist; Question Model, generated API, strict browser decoders, response translation, validation, and Student UI consumers use the complete role names.
- Closed the companion Question Presentation helper suffix audit. Current presentation implementation names and descriptor helpers are complete role names; the browser-suite's separately versioned scenario-input schema remains outside that product contract.
- Renamed the WeBWorK adapter's sole-current replay records to `WebworkUpstreamControl`, `WebworkUpstreamMatchingPrompt`, and `WebworkQuestionAttemptReplayDetails`. Documentation now distinguishes this tested private adapter boundary from the still-unmounted Store and delivery work required to persist, authorize, and redaction-test replay details.
- Corrected residual Question Pool candidate wording in the Question Model, generated declaration source, Question ID specification, browser-model test, Blueprint Course item-capacity constant, and Instructor cookbook. The shared product member is consistently a Question Pool Item; historical changelog evidence and generic selection-algorithm terminology remain contextually distinct.
- Renamed WeBWorK and iMathAS adapter execution wrappers to `ResolvedWebworkQuestionSource` and `ResolvedImathasQuestionSource`. Both now visibly derive from the shared verified immutable Question Source boundary; Question Backend and backend-specific location remain separate facts.
- Corrected active PLE Question JSON language across the editor action, browser acceptance selectors, decoder errors, registered implementation identifier, contracts, current plans, FAQ, and technical documentation. The wire contract remains explicitly schema version 2; the internal module-path migration remains tracked separately.
- Completed the PLE Question JSON adapter source-path cutover. Its parser, compiler, schema-version-2 shapes, trusted-import constructor, project-tools fixture consumer, and current documentation now use `question_json` / `schema_v2` and precise PLE Question JSON types; its server-only grading contract now lives at `grading::ple_question_json`. Adapter and project-tools tests, strict Clippy, fixture validation, and the Markdown-link gate pass.
- Completed the PLE Question JSON browser and grading cutover. The authoring package, client routes, editor styles, tests, schema-owned private grading record, and fixtures now use PLE Question JSON names; the source-specific grading contract is `grading::ple_question_json`. Focused Rust and browser tests, strict Clippy, TypeScript checks, Markdown links, fixture validation, formatting, and diff gates pass.
- Extended the PLE Question JSON cutover through its durable pilot payload path, Wasm parity corpus and tests, issued-attempt capability, generated browser contract, strict decoder, editor accessibility identifiers, and current contracts. It now accepts a bounded optional source `questionHint`, compiles it as separate private pre-response `QuestionHint` content, and keeps it out of the public preview and private grading record; revision persistence and issued-Question delivery remain tracked. Hotspot public presentation, private Answer Key terminology, source validation, and interchange documentation consistently call the selectable geometry a Hotspot Region. Every active PLE-owned format boundary now uses PLE Question JSON rather than the retired layout term, and PLE-owned backend and algorithmic-source language now uses PLE Question Backend and PLE algorithmic Question Source rather than the retired Native alias; focused Rust, fixture, generated-contract, browser, formatting, and diff gates pass. Request-integrity evidence is now the typed server-held `RequestChecksum` across Blueprint reservations, apply records, commands, and receipts, distinct from Source Object, Blueprint Content, and Question Presentation checksums. The public descriptor now serializes its truncated `QuestionPresentationToken` as `presentationToken`; the full server-held `QuestionPresentationChecksum` remains distinct through generated contracts, Wasm verification, browser checks, and submission documentation.
- Replaced the browser's product-shaped Draft Assignment identifiers with Assignment Editor State. The Questions editor, picker controller, local validation, and shared browser-test setup now describe transient unsaved state for an existing Assignment rather than a separate lifecycle; focused Node, TypeScript, formatting, retirement-search, and aggregate acceptance gates pass. Removed the unmounted Sysadmin Instructor-approval browser workflow-route, page, panel, transport methods, DTOs, strict decoders, generated declarations, tests, and stale navigation copy-so it no longer creates a conflicting post-creation Account lifecycle. The fresh schema and domain now complete that redesign: Instructor Vetting precedes Account Creation, and immutable Instructor Product Role plus Account State replace the redundant Instructor Approval event ledger. Course Membership foreign keys, Course Invitation targets, Workspace and Blueprint collaboration, Question ownership, and exact course authority use direct Instructor Account and Membership relationships; generated Blueprint access now uses `active_instructor`. Removed the unbacked Course Appearance mutation route, navigation, browser API methods, stateful editor, and client tests; retained typed contracts now use Course Banner Upload Reference and Course Banner Upload Receipt, while current docs state that Store-backed upload promotion, expiry, and current-pointer authorization remain open. The PLE Question Backend now requires trusted Source Object Reference and Source Object Checksum evidence at issuance, stores both in Question Attempt Reproduction Details, and rejects replay records missing either fact; the stored fixture requires the same exact pair for each recorded attempt. Current comments and operational contracts consistently call these an exact Question Source, Source Object Reference, and Source Object Checksum rather than treating a source object as a composite reference. `ResolvedQuestionSource` centralizes immutable source-object verification: WeBWorK delegates to it, iMathAS resolves the same object before deriving provider location from its owning Question Revision, and `ResolvedPleQuestionJsonSource` parses canonical PLE Question JSON, refuses a public-revision mismatch, then issues and grades through its source-derived private key. H5P now calls its separate archive record an H5P Package Import and its content-derived identity an H5P Package Import Fingerprint, preserving that it is an ungraded-practice import rather than Question Source. QTI now names immutable package bytes, import results, public/private mappings, combined mappings, warnings, and duplicate identity with their exact Checksum or Fingerprint roles rather than report-digest or generic-integrity names. PLE Question Backend internals and browser publication verification now use PLE-specific names rather than Native Question labels; current architecture labels use `MOD-ADP-PLE`. Question Revision acceptance is now an immutable schema fact holding the exact parent revision, Question Revision Editor, Question Revision Accepted By, accepted time, and Reason for Edit; fresh PostgreSQL acceptance proves its immutability and publication prerequisite, while Store/service history and comparison projections remain open. The fresh private schema now replaces the QTI-only Workspace Import root with format-aware `workspace_import`, its Staged/Committed state, and checksummed accepted/rejected `workspace_import_item_result` records that become immutable at commit. The unbacked browser-only QTI conversion workflow, its scenario registration, and its tests are removed; a future QTI surface must arrive with the exact Workspace Import Store and service route. The complete `all_test.sh` aggregate gate now passes Rust, Wasm, browser/Node, 4,804 Python tests, fresh PostgreSQL/RLS, and PostgreSQL/MinIO acceptance. Its closed implementation registry uses `PleQuestionJsonImplementation`, and `QuestionFormat::PleQuestionJson` serializes as `pleQuestionJson` through the fresh schema baselines, generated browser contract, strict decoder, authoring client, and fixtures. The broader publication coordinator and adapter-wide source-resolution migration remain open. Adapter tests, strict Clippy, generated-contract, and browser-codebase checks pass. Focused Rust, TypeScript, browser-contract, PostgreSQL, documentation, formatting, and diff gates pass.
- Corrected the remaining generic Group and Family wording in active contracts, design rationale, decoder commentary, and deterministic selection-stream labels. The context audit leaves only exact SQL, accessibility, operating-system, CSS, and molecular-biology uses; focused Node, Markdown, formatting, diff, and aggregate acceptance gates pass.
- Completed the static Question Variation Rule cutover: exact Question Revision contracts, backend adapters, flat-question validation, deterministic fixtures, generated TypeScript, strict browser decoders, editor preview, and WASM now use `QuestionVariationRule` / `questionVariationRule`; `AssignmentQuestionVariationRule` explicitly retains the independent later-Attempt reuse decision. Focused Rust, TypeScript, browser-contract, WASM-bridge, Markdown-link, formatting, and diff checks pass.
- Established the distinct Question credit and stewardship foundation. The fresh PostgreSQL schema now owns immutable ordered Question Authorship, exact versioned Question License, optional Question Citation, and immutable Question Ownership Event relations, with deferred publication validation and a derived current Question Owner. Browser-safe contracts, generated types, strict decoders, publication requests, Question Search, editor controls, fixtures, and current documentation now use Question Authorship and Question Author display records rather than Public Byline. Question Metadata and Flat Question source now carry only the exact `questionLicense` SPDX values `CC0-1.0`, `CC-BY-4.0`, or `CC-BY-SA-4.0`; a draft can leave that field unset, while publication requires it. Flat Question authoring now carries an optional exact `questionCitation` with bounded Citation URL and NLM-style Citation Text and excludes that source credit from the answer-free student preview. Question Metadata and Flat Question source now also require a bounded Instructor-facing `questionDescription`, which the immutable Question Revision schema derives into one indexed search column and answer-free read projection while Student previews exclude it. Account-backed authorship remains server-held; the publication-service integration that persists and preserves these records through edits, proposals, and forks remains tracked in the retained vocabulary checklist.
- Closed the Assignment Edit Number versus Assignment Revision Number terminology audit. Ordinary Assignment saves use the independent compare-and-swap edit number, while Assignment Release creates the immutable per-Assignment revision sequence that Student work pins; the remaining `Assignment Revision Definition` browser-contract prose now names the owned Assignment Revision. The availability audit also confirms that active boundaries use Assignment Entry Availability for top-level entries and Question Pool Item Availability only for pool members. Question Pool selection now has the same completed audit: count and selected Question order are the Instructor's rule, while server entropy creates one frozen selection of available Question Pool Items. `BlueprintQuestionPosition` now serializes its scoped `question_pool_item_index`, and the editor, picker, styling, and browser scenarios name exact Question Pool Items rather than generic candidates. Question Search now owns each request/filter/page contract and its validation helpers, replacing the final Catalog Search aliases in the browser path; `QuestionStatisticsAvailability` owns the distinct released-statistics filter. Forced Question Correction now names its immutable `correction_generation` in the fresh schema and correction-recalculation evidence. The flat-question authoring boundary now names the server-held instructional role of an uploaded image as Question Asset Purpose, rather than treating it as Question Source or Question Authorship provenance. Assignment Policy Sources now own the policy-preview surface through visible copy and its local style hook; the cached Question Pool position and Blueprint operation comments name their exact records. H5P, QTI, and Object Storage now name their exact import location, import binding, source points, or private import evidence rather than generic provenance. Publication is now consistently the immutable Question Library entry event, while Question Revision Availability is the distinct Available or Archived selection status across model, schema, contracts, current docs, and active plans. Current model, schema trigger, browser request, terminology contract, PostgreSQL oracle, and aggregate-suite evidence agree.
- Removed the dead `question_generator_id` and `question_generator_version` columns from the fresh Question Source baseline. The immutable Source Object Reference and Source Object Checksum remain the single stored identity for Question Source bytes; the PostgreSQL acceptance oracle now refuses a competing generator identity. The larger Seeded Question Generator source, publication, repair, issuance, and grading cutover remains an open tracked migration.
- Tightened the terminology contract around distinct meanings rather than forbidden words. Checksum now names integrity verification; Digest remains available for a demonstrated content-derived identity, fingerprint, cache discriminator, or deduplication value; Definition and Canonicalization remain technical vocabulary where they are more exact than established PLE terms. Assignment now owns its current Instructor-authored content directly, ordinary saves advance Assignment Edit Number, and Assignment Release creates immutable Assignment Revisions. The vocabulary checklist tracks the coordinated removal of separate Assignment and Question Working Copy lifecycles, integrity values still named Digest, generic PLE-owned Definition names, and misleading canonicalization helpers. The temporary terminology audit reports those categories separately as context-reviewed migrations rather than raw zero-count targets. The whole-word `group` review removes the stale grading-operation CSS hook and generic Assignment-revision prose; remaining source uses are precise SQL, accessibility, operating-system, biology, or retired-wire rejection terminology. Active PLE contracts now use exact Question Backend, Question Type, Question Response Format, and Question Asset language in place of generic family terminology; retained historical reports remain dated evidence. The fresh database schema places editable Instructor-authored Assignment content and Assignment Edit Number on `ple_data.assignment`, removes `assignment_working_copy`, and proves its release/Attempt fixture through the PostgreSQL 17 oracle. The Rust model, generated TypeScript contract, strict browser decoders, and focused browser tests now use `AssignmentAuthoredContent` directly; the separate Assignment Working Copy lifecycle is retired. Assignment Status now carries Unreleased, Released, Closed, or Archived directly; `InstructorAssignmentAvailabilityView` describes the independent server-derived availability calculation, Create Assignment posts directly to the Assignment collection, and every Assignment Attempt is pinned to the selected Released Assignment Revision. The redundant generic `authored_definition` JSON container is removed from the current Assignment and Assignment Revision schema, edit trigger, and fresh fixture: the explicit Instructor-authored fields and immutable revision snapshot now own their distinct content directly. Question Publication Review now identifies its saved Draft Question Revision with `draftQuestionRevisionNumber`, completing the PLE-owned Working Copy retirement across schema, contracts, strict decoder, and tests. A staged QTI Workspace Import now names its exact `source_package_evidence` rather than a generic source record, with the private-record catalog oracle rejecting the retired column. Issued Question Presentation bindings now pair each rendered reference and role with a typed Response Item Reference, while public descriptor rebuilds explicitly carry no durable mapping; the empty `durable_id` sentinel is retired. The queued-work schema now stores a Job rather than a worker job: Job Kind, immutable Job Target, Job State, and Job Failure Kind distinguish durable work from the Worker process; fresh PostgreSQL 17 acceptance verifies its forward-only baseline and exact RLS catalog. Gradebook selection, exact inspection routing, state management, CSS hooks, and strict browser contracts now name submitted Assignment Attempts directly and refuse retired run-shaped wire data. Question Source documentation and error text now distinguish immutable Question Source data from its Source Object Reference and Source Object Checksum, while preserving Blueprint Assignment Source Record as a separate operation-evidence concept. Course Instance reconciliation now accepts only Assignment Import Receipt evidence, so Apply Blueprint Update and Copy Assignment from Blueprint retain their operation-owned receipts while rollover, term-shift, and reconciliation receipts are impossible repair inputs. The current material tree passes `source source_me.sh && ./all_test.sh`, including Rust, Wasm, browser/unit, 4,941 Python, PostgreSQL/RLS, and PostgreSQL-MinIO acceptance. Documentation formatting, shell syntax, diff validation, and all 192 Markdown-link and guidance-format tests pass.
- Completed the clean Question Version to Question Revision cutover: the Question Model, fresh PostgreSQL schema, adapters, generated contracts, strict browser decoders, Blueprint references, Assignment and Issued Question records, deterministic fixtures, and active documentation now use `QuestionRevisionReference { question_id, revision_number }`; aggregate acceptance is green. Service Login Setup now consistently names the local-stack creation and refresh of its disposable least-privilege database login. Browser course and authoring-workspace routes now use complete `CourseInstanceId` and `AuthoringWorkspaceReference` runtime narrowing while preserving `C-` and `W-` wire locators; Gradebook navigation names its `AssignmentAttemptReference` directly. Object Address now names the typed server-created physical object location; hostile `objectKey` fixture fields remain only to prove decoder refusal. Renamed fresh migration boundaries for Authenticated Session resolution, authorization checks, and credential-authentication completion; their database functions now name exact Account, Session, workspace, and credential decisions. Browser authoring now names its answer-free comparison Question Publication Review, including its Working Copy Edit Number, summary, route, strict decoder, and test contract; a mounted server publication boundary remains open. Native adapter registration now uses one source-contract key of Question Format, Question Type, and Question Source generator reference; software release metadata remains reproduction evidence. Fork Blueprint Course, Adopt Blueprint Assignment, and Instantiate Blueprint Course now each own exact readiness, blocker, and command-error contracts through previews, server-held apply records, and generated browser types. Forced Question Corrections now store both critical-question endpoints as exact Question Revision References, with revision-named schema constraints and PostgreSQL oracle coverage. Question Change Proposal Revisions now retain their exact base Question Revision Reference and revision-named schema constraints through merge validation. Question authoring now separates Question Publication Requirements, Validation, and ordered Issues; browser validation distinguishes unavailable validation from capability issues. Generic entitlement-materialization naming is retired in favor of exact operation records and receipts. Assignment Working Copy saves now explicitly use Assignment Edit Number; Successor Assignment Revision recovery retains its separate released-history reference. Assignment release now uses Requirements, Validation, and Issues rather than a generic publication-readiness state. The retained authoring review contract is confirmed as Question Publication Review rather than a generic publication diff. Print Exam export now distinguishes borrowed Print Exam Question Input from validated answer-free Print Question output. Object Store metadata now inherits licensing from its owning Question relationship instead of accepting a duplicated generic Object License. Object Store records now derive provenance from typed ownership relationships rather than accepting free-text provenance. PLE Question Backend now uses trusted Question Asset References and Object References rather than an ambiguous Asset Object Binding; the former Native product label is replaced across its package, source and backend discriminators, generated contracts, browser selector, fixtures, and current documentation. Print Exam now accepts borrowed Question Revision and Question Backend Capabilities pairs directly, retaining Print Question as its sole answer-free export record. Object Delivery now has an exact retrieval root, owner-specific delivery relationships, and Pending, Available, or Retired delivery state. Active documentation now distinguishes Assignment Activity Rules from Student Work Records and instructor-facing activity patterns. Question Attempt Reproduction Details now names trusted server-held reproduction facts across model, schema, adapters, and strict Student disclosure boundaries. Assignment structural-change recovery now directs Instructors to the exact successor Assignment Working Copy they must create, while preserving the pinned released Assignment Revision. Human Guidance and active native-adapter contracts now name the eight supported Question Types directly instead of treating them as a generic family. Question Pool Entries now replace the former candidate model across source, schema, generated contracts, strict browser decoders, fixtures, and PostgreSQL acceptance. The implementation-status registry now names its preparatory private-organization receipt Question Folder and Question Folder Share rather than retired collection records. Assignment creation now uses Create Assignment and Assignment Working Copy language across the visible browser flow, route contract, generated documentation, tests, and current docs. Question Pool Selection now consistently names Question Pool Entries in its bounds, selected-entry evidence, Blueprint content, browser contract, editor, and validation messages. The Question Model and browser contract now use `QuestionContentBlock` for the closed shared renderable unit across Question Prompts, response fields, and policy-released Student Feedback. The multi-fill-in presentation contract now uses `PresentedTextEntrySlot` across its generated declaration, strict decoder, response validation, and browser controls. Hotspot presentation records now use `PresentedHotspotRegion` and `PresentedHotspotSurface` across their model, presentation builder, generated contracts, strict decoder, and validation; The private pre-presentation hotspot helper is now `PendingHotspotRegionGeometry`. Current construction, reproduction, verification, and binding helpers now use Question Presentation, Response Item, and current-descriptor names rather than a generation suffix; Student Attempt Descriptor and the consolidated Student Assignment Attempt Screen now use their complete generated contract names; Question Variation Presentation now owns the pre-issuance answer-free rendering input, while Question Presentation owns the issued attempt-bound public contract and Student screen field; The presentation contract now distinguishes Question Choices, Matching Prompts, Matching Choices, and Ordering Items instead of reusing a generic presented-choice record; Issued Attempt Capability now names the current protected-attempt capability enum through the model, generated contract, strict decoder, and payload design; Rendered Student Response translation and Student Response Inspection records now use their complete current names through private translation, generated browser contracts, and focused tests; Question Presentation Response Format now names the issued response shape separately from the authored Question Response Format through model, generated API, strict decoder, validation, and terminology contract; Question Asset Id and Question Asset Reference now name the exact Question-owned asset identity and authored checksum across Question content, response formats, object-rendering paths, generated API, and strict decoders; Question Asset Rendition now names the browser-safe issued rendition record and question_asset_renditions collection through the presentation model, descriptor codec, generated API, browser consumers, and focused tests; Instructor-entered Question IDs now pass through parseExactQuestionIds across Assignment editor and Question Pool entry workflows; BlueprintCourseId now names the reusable Blueprint Course locator throughout the active plan; canonicalCourseLocalDateAndTime now names the Instructor-facing Course Local Date and Time normalizer; Question Revision Substitutions now name Blueprint-operation replacement records and generated contracts; Blueprint Assignment now replaces Reusable Assignment through the complete Blueprint Course contract and Instructor editor; remaining current presentation suffixes remain tracked separately.
- Replaced the source-shaped iMathAS draft locator with `ImathasQuestionLocation`, composed from External Question Provider Reference and iMathAS Item Reference. Source bytes remain separately owned by the immutable Question Source boundary through Source Object Reference and Source Object Checksum; focused iMathAS and workspace compilation gates pass. The unused generic Curriculum Adoption title scalar and umbrella-labelled retry-token bound are removed from Rust and regenerated TypeScript. The duplicate Adopt Blueprint Assignment operation is removed from the Rust envelope, server-held record, strict client allowlist, and generated contract, leaving the six Human Guidance Blueprint operations; the client regression test retains only the retired wire tag needed to prove rejection. `CreateCourseFromBlueprint` and `create_course_from_blueprint` now name the direct Course Instance creation contract across Rust, generated TypeScript, strict client validation, active documentation, and regression tests. `CopyCourseForNewTerm`/`copy_course_for_new_term` and `ShiftCourseDates`/`shift_course_dates` now name their exact operation contracts; retired tags remain only in strict rejection assertions. `ApplyBlueprintUpdate`/`apply_blueprint_update` and `CopyAssignmentFromBlueprint`/`copy_assignment_from_blueprint` now name the remaining two operation contracts across the envelope, server-held records, commands, receipts, generated browser API, client validation, active documentation, and retired-wire rejection assertions. The shared browser envelope, client, route path, generated contracts, Question Model module, feature stylesheet, and current architecture documentation now use the technical Blueprint-operation boundary; exact operation variants and Reconcile Course Instance records replace the remaining Curriculum Adoption and generic adoption identifiers. Blueprint Revision Content and Question Presentation verification now use `BlueprintContentChecksum` and `QuestionPresentationChecksum`; persisted presentation bindings carry `checksum`, while the public truncated presentation token remains separate. Question Pool Assignment Entry now names the top-level Assignment structure, while Question Pool Item names a member; retired candidate/entry identities are absent from active code, fixture, generated-contract, and documentation boundaries.
...
- Replaced the browser and generated `sourceBackend` field with `questionBackend`: a workspace summary or Question Publication Review exposes only its selected Question Backend, while the immutable private Question Source remains separately owned by its exact revision. Renamed the tracked cross-layer fixture root and module to `published_question` / `publishedQuestionFixture`; its exact stored revision is `publishedQuestionRevision`, replacing the PLE-owned published-problem term across fixture validation, Print Exam, generated fixture tooling, browser consumers, and current operational documentation. The static PLE fixture now correctly declares server grading without an algorithmic-generation capability; rebuilt Wasm E2E, project-tools fixture validation, Rust export tests, and browser checks pass. The README now names the six exact Blueprint operations and distinguishes the mounted Authenticated Session/seeded-demo boundary from unmounted Course, Question Library, authoring, delivery, Gradebook, passkey, and email-code surfaces; its target teaching flow uses Published Question, Question Revision, Assignment Attempt, Question Submission, and Automated Grading Receipt terminology rather than published-problem, run, Alpha, or curriculum-adoption claims. PLE Question prompt derivation now uses `build_question_prompt`, `build_question_content_block`, and `DerivedPleQuestion`, replacing materialization vocabulary that hid deterministic prompt construction and private Answer Key derivation; focused domain/adapter tests, Markdown links, formatting, and diff checks pass. Current architecture, activity, and Question Model documentation now names exact Assignment revision ownership, Course Memberships, Assignment Attempts, Student Responses, and Question- or Student-scoped grading-operation metadata instead of generic Group relationships; Markdown links and diff checks pass.
...
- Replaced `AppliedAssignmentImportEvidence` with `AssignmentSourceRecord`
  and the former selected Blueprint Assignment receipt with
...
- Aligned PLE Question JSON import documentation with its existing direct
  construction boundary: `ImportedPleQuestionJson::from_imported` and
  `ImportedPleQuestionJsonError` now name trusted import construction precisely.
...
- Added the private Question Source publication-transfer boundary: it records one separately copied, checksum-matching Question Revision Source Object and exact immutable Question Source relationship, allows only identical retries, and prevents a Question Publication Event from committing without that source.
...
- Replaced the generic private Draft Question Revision `definition` column with `question_content`.
  The fresh PostgreSQL private-record fixture now uses the same exact Question Content name.
- Replaced the server-only Draft Question Revision storage-key input with the exact Draft Question Revision Reference; PostgreSQL resolves its private row only within the authorized Authoring Workspace.
- Separated editable `DraftQuestionContent` from server-only immutable `DraftQuestionRevision`, which binds content to its private `DraftQuestionUuid` and positive revision number. The browser-generated Question Model now retains only answer-free Draft Question Content and Summary contracts; `DraftQuestionUuid`, `QuestionSourceUuid`, and their exact revision binding live in data access and fresh PostgreSQL `_uuid` columns. Generated `DraftQuestionSummary`, strict browser decoding, and navigation parsing use a bounded opaque `D-` Draft Question Reference rather than exposing storage UUIDs. Current architecture documents now state that editor HTTP routes and the Draft Question Store are unmounted rather than claiming a nonexistent server implementation. Fresh PostgreSQL 17 acceptance passes. Completed the Question Library cutover for its stored fixture and browser pagination boundary: the answer-free record is `questionSummary`/`question_summary`, the page-size constant is `QUESTION_LIBRARY_PAGE_SIZE`, and PLE capability and Question ID tests use their exact owned meanings rather than catalog language. Project-tools, PLE adapter, Question Model, fixture, and browser checks pass. Named the shared PLE/Wasm JSON data the `Question Response Format fixture set`, retaining it as explicit data rather than hard-coded Rust records; native, Wasm, and browser bridge consumers all load the same checked-in JSON file. Replaced PLE-facing materialization language with the exact preview construction, deterministic example generation, Blueprint Course creation, and correction-reference update operations; background materialization remains limited to actual worker processing.
