...
| [x] | `QuestionSource::Native.family` | Stored native source discriminator | Question Format, Question Type, and optional Question Generator | Store each applicable concept explicitly. Select native handling from the resulting closed combination. |
...
| [ ] | `runRef` in browser route parameters | Public route parameter for one Student pass through an Assignment | `assignmentAttemptRef` / `assignmentAttemptReference` | Use `assignmentAttemptRef` only at the URL boundary and the complete `assignmentAttemptReference` in route-resolution contracts. |
| [ ] | `run_page.tsx`, `run_summary_page.tsx`, or `run_page_recovery.ts` | Browser implementation files for one Student pass through an Assignment | `assignment_attempt_page.tsx`, `assignment_attempt_summary_page.tsx`, and `assignment_attempt_page_recovery.ts` | Keep the page, summary, and session-recovery owners visibly tied to the Assignment Attempt boundary. |
