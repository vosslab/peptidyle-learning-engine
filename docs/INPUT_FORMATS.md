...
| H5P `.h5p` package     | Trusted private adapter/object-store boundary, not a browser upload route   | `H5P.MultiChoice` converts to an unpublished, key-free, ungraded practice payload. It has no Question Backend lifecycle and cannot be used as a server-graded assignment              | [crates/adapters/h5p/src/import.rs](../crates/adapters/h5p/src/import.rs), [CONTRACTS.md](CONTRACTS.md)                                         |
