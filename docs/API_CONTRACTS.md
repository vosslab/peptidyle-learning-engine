...
M9 Course Roster Import, M10 Assignment Workspace, release, and Due Soon read
path, M11 Student
...
| Instructor Profile thumbnail | `GET` / `POST /api/instructor-profile/thumbnail`; `POST /api/instructor-profile/thumbnails/{thumbnail}/delivery` | An active Instructor reads or replaces only their current opaque thumbnail. The replacement body contains bounded image bytes only; the server validates, center-crops, normalizes, persists, and delivers one private WebP rendition under the caller's current scope. | [instructor_profile.rs](../crates/server/src/instructor_profile.rs) |
...
| Course Assignments | `GET /api/course-instances/{course_instance_reference}/assignments` | Requires the caller's direct active Instructor Course Membership and lists every Assignment as exactly `reference`, `title`, `dueAt`, `displayTimeZone`, `status`, and `editNumber`. The projection contains no Student identity, work, response, answer, or grading state. | [assignment_release.rs](../crates/server/src/assignment_release.rs) |
| Assignments Due Soon | `GET /api/assignments/due-soon` | Requires the authenticated Instructor product role and independently rechecks current active Instructor Course Membership per row. It returns no-store `{ items, nextCursor: null, displayTimeZone }`; each item is exactly `courseReference`, `courseLongName`, `assignmentReference`, `assignmentTitle`, `assignmentStatus`, and integer `dueAtMillis`. The current implementation choice is a rolling `[statement_timestamp(), statement_timestamp() + 7 days)` window for `unreleased` and `released` Assignments, ordered by due instant then Course and Assignment reference. It contains no Student, response, Attempt, grading, or answer data. | [assignment_release.rs](../crates/server/src/assignment_release.rs) |
| Inline Assignment retime | `PUT /api/course-instances/{course_instance_reference}/assignments/{assignment_reference}/inline` | Requires direct active Instructor membership and exact quoted strong Assignment Edit Number `If-Match`. Its closed `{ title, dueAt }` body requires `dueAt`, which may be null; the raw local value is resolved in the authenticated Instructor zone. Unreleased and Released saves return the six-field row and matching ETag without a revision; an unchanged retry does not write. | [assignment_release.rs](../crates/server/src/assignment_release.rs) |
...
| Assignment Workspace | `GET`/`PUT /api/course-instances/{course_instance_reference}/assignments/{assignment_reference}` | Loads or saves current answer-free authored content only for a direct Instructor. Saves require the exact strong Assignment Edit Number `If-Match`; stale content returns `412`. An exact current full-payload retry returns the existing resource and Edit Number without a mutation. | [assignment_release.rs](../crates/server/src/assignment_release.rs) |
| Assignment Release Validation | `GET /api/course-instances/{course_instance_reference}/assignments/{assignment_reference}/release-validation` | Calculates whether the current Assignment has a positive whole-Attempt time limit and releasable Available Published Questions without creating a Revision or Student work. The closed issue set includes `timeLimitRequired` when the Instructor must save that duration. | [assignment_release.rs](../crates/server/src/assignment_release.rs) |
...
| Assignment Release | `POST /api/course-instances/{course_instance_reference}/assignments/{assignment_reference}/release` | Requires the exact strong Assignment Edit Number `If-Match`, validates the selected positive whole-Attempt duration and current Question selection, and atomically creates the next immutable Assignment Revision for later M11 delivery. | [assignment_release.rs](../crates/server/src/assignment_release.rs) |
| Student Assignment Access | `GET /api/course-instances/{course_instance_reference}/assignments/{assignment_reference}/access` | Requires the authenticated Student's exact active Student Record for the Course. At authoritative time it returns the server-calculated start decision plus answer-free title, question count, points possible, time limit, authorized active Attempt reference, and complete newest-first owned previous-Attempt history with independently disclosed current scores. | [assignment_delivery.rs](../crates/server/src/assignment_delivery.rs) |
| Student completed Attempt history | `GET /api/assignment-attempts/{assignment_attempt_reference}/history` | Requires the Student's exact owned completed Attempt and active Course membership. It returns authorized Attempt, Course, and Assignment metadata; an optional aggregate current score; and Question position/state with independently permitted recorded response, score/correctness/feedback, and native PLE safe Question Answer content. Exact pinned native PLE source reproduction never regrades; its private source mapper uses text transport for `question_attempt_id`, `source_object_id`, and `question_seed`. Unavailable source, reproduction, or asset facts omit only affected content. Raw source, replay, answer keys, response IDs, and other private identifiers remain server-only; unavailable PLE explanations and WeBWorK teaching content are omitted. | [assignment_delivery.rs](../crates/server/src/assignment_delivery.rs) |
...
The current Course-scoped workspace checks the authenticated Instructor's
...
The direct-Instructor save compares the exact Assignment Edit Number, bounds
its zone-free local due value by the inclusive Course Term dates, and resolves it
through the authenticated Instructor's IANA time zone. Release snapshots that
absolute due instant and its Late Work Rule into the immutable Assignment Revision;
later Student start uses that snapshot. The release validator also requires a
positive whole-Attempt time limit; a nullable draft remains editable until the
Instructor selects and saves its duration. New Assignments default to one-Question
delivery and shuffled Assignment Question order. Initial issuance stores that
Attempt's deterministic sequence in existing Issued Question positions, while
release Entry, Question, and revision identity remain the source/recovery key.
