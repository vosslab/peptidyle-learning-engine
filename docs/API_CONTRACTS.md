...
Assessment, Attempt, Question Revision Tuple, and Course Roster Tuple
identities. It never exposes `StudentRecordId`.

An `If-Match` header carries one quoted qualified Edit Number or Revision
Number. HTTP `ETag` and `If-Match` are header spellings only. Domain APIs
expose those Numbers; they do not use ETag as a domain term. A missing
...
| Profile time-zone preference | `GET` / `PUT /api/account/settings`                                                                                                                                                                                                                               | The Profile page is the only browser page that names or edits the exact IANA preference. The self-only request has no Account, Course, or Product Role selector. PostgreSQL derives the active Account and atomically validates and changes the exact name. A change affects date display and later Instructor wall-clock date entry, never a stored instant. |
| Instructor profile            | `GET` / `PATCH /api/instructor-profile`; `GET` / `POST /api/instructor-profile/thumbnail`; `POST /api/instructor-profile/thumbnails/{thumbnail}/delivery`                                                                                                         | An active Instructor reads or updates only their profile and private thumbnail; replacement validates and normalizes bounded image bytes before delivery. This API does not own the Account time-zone preference. |
...
| Surface                      | Route                                                                                                                                                                                                       | Contract                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Assessment list and due-soon | `GET /api/course-instances/{course_instance_id}/assessments`, `GET /api/assessments/due-soon`                                                                                                          | Returns current answer-free Assessment summaries with Assessment Type, release status, due date, and `assessmentEditNumber`; due-soon independently rechecks current Course membership.                                                                                                                                                                                                                |
| Create and edit              | `POST /api/course-instances/{course_instance_id}/assessments`; `GET` / `PUT /api/course-instances/{course_instance_id}/assessments/{assessment_id}`                                         | Creates, reads, or saves one stable current Assessment. A save requires the quoted Assessment Edit Number and stores ordered Questions and Question Pools that pin exact Question Revisions. An unchanged retry returns the existing aggregate and Assessment Edit Number without mutation.                                                                                                                     |
| Inline save                  | `PUT /api/course-instances/{course_instance_id}/assessments/{assessment_id}/inline`                                                                                                              | Updates the closed title/due projection using the quoted Assessment Edit Number.                                                                                                                                                                                                                                                                                                             |
| Assessment Properties save  | `PUT /api/course-instances/{course_instance_id}/assessments/{assessment_id}/policies`                                                                                                            | Updates whole-Assessment instructions, dates, scoring, Attempts, late work, and disclosure settings using the quoted Assessment Edit Number. Ordered Questions and Question Pools remain unchanged. The `/policies` path is the Assessment Properties save surface.                                                                                                                            |
...
| Release                      | `POST /api/course-instances/{course_instance_id}/assessments/{assessment_id}/release`                                                                                                            | Requires the quoted Assessment Edit Number and a passing automated, interactive Assessment Release Validation. Validation covers required Questions/settings, point and Attempt/time ranges, date order, and a due date at least 24 hours ahead that does not exceed the Course Instance's six-month Active limit. `200` returns the current Assessment aggregate with Released status and its new Assessment Edit Number. |
| Unrelease impact             | `GET /api/course-instances/{course_instance_id}/assessments/{assessment_id}/unrelease-impact`                                                                                                    | For a Released Assessment only, returns its current confirmation title, required Assessment Edit Number, and aggregate affected Student Work counts. It contains no Student identity, response, or grade detail.                                                                                                                                                                                       |
| Unrelease                    | `POST /api/course-instances/{course_instance_id}/assessments/{assessment_id}/unrelease`                                                                                                          | Requires an equal Course co-Instructor, Released status, exact quoted Assessment Edit Number, and `{ "confirmationTitle": "exact current title" }`. `200` returns the current Unreleased Assessment and new Assessment Edit Number. PostgreSQL atomically locks the Assessment and permanently removes its Student Work.                                                                                       |
| Assessment Blueprint Update  | `GET` / `POST /api/course-instances/{course_instance_id}/assessments/{assessment_id}/blueprint-update`; `GET /api/course-instances/{course_instance_id}/blueprint-update-review` | Review and apply use named parent `BlueprintRevisionTuple` values and the daughter Assessment Edit Number. Apply CAS is `expectedSourceBlueprintRevisionTuple` plus `expectedAssessmentEditNumber`. |
...
| Attempt reads and timing checkpoint | `GET /api/assessment-attempts/{assessment_attempt}/student-progress`, `GET /api/assessment-attempts/{assessment_attempt}/context`, `GET /api/assessment-attempts/{assessment_attempt}/student-question?position={position}`, `GET /api/assessment-attempts/{assessment_attempt}/history`, `PUT /api/assessment-attempts/{assessment_attempt}/questions/{position}/display-duration` | An owning active Student reads only their authorized progress, retained presentation, and policy-authorized history. Progress includes the nullable cumulative display duration for each issued Question. The PUT accepts a nonnegative cumulative `cumulativeDisplayDurationMs`; repeated or stale values cannot reduce the stored total, and finalization closes the write. Context returns `expiresAt`, `timerRemainingMilliseconds`, and `displayTimeZone` from one authoritative read. Display duration is approximate time shown, not attention or effort. |
...
| Question images                   | `GET /api/questions/{questionId}/revisions/{revisionNumber}/images/{questionImageAssetId}`                                                                                                                                                                                                                                       | Delivers an immutable Question Image Rendition only after the authorized retained presentation relationship.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Student landing                   | `GET /api/student/course-instances`, `GET /api/student/course-invitations`, `GET /api/course-instances/{course_instance_id}/assessment-landing`                                                                                                                                                | Returns safe Course and Coursework data. Due Soon uses the server-evaluated instant and the half-open rolling seven-day window. Completed uses the Progress projection's submitted-Attempt count, so it is distinct from perfect score and may overlap Due Soon. Dates use the selected display zone; only Profile shows its name. |
| Student Course Active Attempt     | `GET /api/student/course-instances/{course_instance_id}/active-attempt`                                                                                                                                                                                                                                                 | Returns the signed-in Student's most recently active unsubmitted Attempt ID whose server-owned expiry is in the future, plus its `startedAt` and `latestActivityAt` instants, for the authorized Course. It returns explicit `null` fields when no Attempt has a running clock. The fixed Coursework shortcut reads this projection for enrolled Courses and selects the newest activity, then start time and Attempt ID, across Courses. |
| Student Course Progress           | `GET /api/student/course-instances/{course_instance_id}/progress`                                                                                                                                                                                                                                                        | Returns every released Assessment in the signed-in Student's active Course with Attempt and submission counts, latest recorded activity, and the disclosed highest submitted Attempt score when its policy permits release. A missing score remains an explicit UI state alongside the Attempt activity; score freshness is returned only with a released score. This projection contains no cohort data or weighted Course grade. |
| Student Course Attempt History    | `GET /api/student/course-instances/{course_instance_id}/assessment-attempts?pageSize={1..100}&cursor={opaque}`                                                                                                                                                                                                           | Returns a bounded newest-first page of the signed-in Student's Attempts in the active Course. Each item carries Assessment identity, Attempt number, start/submission instants, and its point score only when grading is complete and the copied Assessment score policy permits disclosure. The opaque cursor is bound to the Course and page size. The Student Attempt History page displays one independently paginated section per enrolled Course; it does not promise one global chronology across Courses. |
| Student Course Response Stats     | `GET /api/student/course-instances/{course_instance_id}/response-stats`                                                                                                                                                                                                                                                  | Groups the signed-in Student's actual submitted outcomes within the authorized Course across eligible Coursework types (Regular and Practice Question Assignments, Bonus Assignments, Quizzes, and Exams) by exact immutable Published Question Revision. The Grades page loads a separately identified section for each enrolled Course; counts are not merged between Courses. An outcome is counted only when both the Assessment score and per-item correctness policies release it. The response contains full-credit, partial-credit, incorrect, unanswered, disclosed, and not-full-credit counts plus one relevant authorized Attempt review link. It includes average measured display duration and sample count only when recorded; missing measurements remain not recorded. It contains no cohort statistics or hidden correctness. |
| Student Latest Feedback          | `GET /api/student/latest-feedback`                                                                                                                                                                                                                                                                                    | Returns the ID of the signed-in Student's newest submitted Attempt with feedback currently released under its Assessment policy, across active Courses, or `null` when none is available. It contains no feedback content or score; the existing Attempt review applies its normal disclosure rules. |
...
The browser formats every decision instant in the supplied `displayTimeZone` and displays the
server-owned Start Decision and public reason directly. The exact time-zone name appears on Profile;
Coursework and pre-start pages do not repeat it. Browser time is never used to recompute permission.

Course-specific Student operations carry the requested `course_instance_id` and derive Student
identity from the authenticated session; the server checks active membership for that Course.
Global Coursework and Grades views compose those Course-scoped reads by enrolled Course when that
keeps the API simple. Active Attempt and Latest Feedback are cross-Course reads because each shortcut
has one target across the Student's enrollment.
...
| Student Work Recovery | `POST /api/course-instances/{course_instance_id}/student-work/recovery` | An authorized Course Instructor selects or recovers archived Student Work. Results use typed Course Instance, Assessment, Attempt, Question Revision Tuple, and Course Roster Tuple identities. They never expose `StudentRecordId`. |
