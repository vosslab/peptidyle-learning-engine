...
| Route Surface         | Routes                                                   | Boundary                                                                                                                                                                                        | Owner                                                                                               |
| --------------------- | -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
...
server-only normalized score and its LDA-derived checksum; it is distinct from the raw-token
checksum and PLE Grading Result. The current browser launch shell carries
...
Revision from immutable acceptance evidence and does not join, filter, or
authorize through Question Revision Availability. Availability therefore stays
a distinct fact; this View is not evidence that `Available` or `Archived`
controls Instructor authorization. The mounted Question Library Store and route
must define and prove the full `QuestionSummary` DTO, including its availability
behavior, before this document lists a Question Library endpoint as available.
...
| `Idempotency-Key`           | Question Submission                                                                                 | The same key and same response represent one grading request. A retry returns the committed receipt without grading twice. This is a deferred Store-backed delivery requirement, not a mounted route.       |
| Server-generated identity   | Assignment Attempts, Question Attempts, publications, export jobs, and Object Deliveries            | Browser paths name an existing opaque record but browser bodies do not mint durable identities or choose storage paths.                                                                                     |
| Bytes-first promotion       | QTI/PLE Question JSON publication, exports, and Course Banner Uploads                               | Objects may be written before the database transaction, but an unbound upload is not visible content. The database commits the authoritative public/delivery pointer atomically.                            |
