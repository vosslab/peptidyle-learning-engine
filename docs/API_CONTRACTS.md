...
| Base Assignment Policy save | `PUT /api/course-instances/{courseInstanceReference}/assignments/{assignmentReference}/policies` | Updates only instructions, schedule, delivery, activity, and feedback policy values using the Assignment Edit Number ETag. Title and normalized Entries remain unchanged. |
...
| Attempt reads | `GET /api/assignment-attempts/{assignmentAttemptReference}/student-progress`, `GET /api/assignment-attempts/{assignmentAttemptReference}/context`, `GET /api/assignment-attempts/{assignmentAttemptReference}/student-question?position={position}`, `GET /api/assignment-attempts/{assignmentAttemptReference}/history` | An owning active Student reads only their authorized progress, retained presentation, and policy-authorized history. The server returns the retained evidence for an issued position; it does not regrade or substitute current configuration. |
...
| Responses, submission, and status | `PUT /api/assignment-attempts/{assignmentAttemptReference}/responses/{position}`, `POST /api/assignment-attempts/{assignmentAttemptReference}/submission`, `GET /api/course-instances/{courseInstanceReference}/assignments/{assignmentReference}/presentations/{presentationNonce}/submissions` | An exact issued presentation accepts only its declared response format. A `backendOwned` response is at most 64 KiB of raw UTF-8 canonical payload; for WeBWorK it encodes the ordered JSON array of form `[name, value]` pairs, including duplicate names and legitimate hidden PG fields. Renderer credentials and JWT are absent from the embed document and therefore are not captured. PLE does not interpret the pairs. Whole-Attempt submission is separate; status remains bound to its nonce and exposes no answer key, raw grader input, private source, renderer state, or private feedback internals. |
