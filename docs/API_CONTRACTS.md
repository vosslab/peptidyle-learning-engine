...
| Student Course Active Attempt     | `GET /api/student/course-instances/{course_instance_id}/active-attempt`                                                                                                                                                                                                                                                 | Returns the signed-in Student's most recently active resumable Attempt ID in the active Course, or `null` when no Attempt can be resumed. The server reuses current Assessment availability, unsubmitted state, and Attempt expiry rules. Activity sorts by latest saved response, then Attempt start time and Attempt ID. The fixed Coursework shortcut is disabled when no Attempt is returned. |
