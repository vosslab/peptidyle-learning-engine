...
separates Draft Question rows, editable metadata, and Draft Question Source Registrations from
Question Revision Source Registrations and Published Question metadata. Publication copies the
...
The current `ple_private.draft_question.title`, `ple_data.question_revision.public_metadata`, and
mixed `ple_private.question_source_registration` shapes do not yet satisfy this target. The open
source-isolation migration replaces them with parallel metadata tables and two qualified source
registration relationships before a publication service or route is implemented.
