...
- Completed C831's bounded native-JSON reproduction closure. Native PLE JSON Questions now retain
  ...
  response. An invalid native seed insertion was rejected by
...
- Completed C838's first canonical-family publication prerequisite. Isolated PostgreSQL 17/MinIO
  ...
  Repeating the lineage created no Pool or Blueprint; unknown ID, hash, or path failed before
  writes. Full manifest validation remains blocked by unrelated removed static rows. C839-C841
  catalog acceptance, CAS reconciliation, and retirement are not claimed. Artifact:
  `/private/tmp/ple-canonical-family-artifacts.kp2uh2`.

- Completed the bounded reusable Published Question Pool creation-and-import path. The Instructor
  Question Library now visibly creates a Pool from ordered Published Question Revisions, requires
  interchangeability attestation, and returns a new canonical `AAAA-ZBBB` Pool ID at Revision 1.
  Accepted actual-main proof recorded POST 201/no-store, exact readback of the reordered pins, and
  the visible success state; the supporting Browse component extraction reduced
  `library_page.tsx` to 906 lines and passed TypeScript, full pytest, and 11 focused Pool/picker
  tests. Separate accepted actual-server proof imported an exact reusable source Revision into a
  direct Assessment as a distinct Assessment-owned fork, preserved both public Question ID/Revision
  pins in order, left the source's semantic identity and pins unchanged, and retained the selected
  pin on Student resume. Artifacts: `/private/tmp/ple-course-empty-artifacts.bzwXEa`,
  `/private/tmp/ple-course-empty-artifacts.BhKHDp`, and
  `/private/tmp/ple-course-empty-artifacts.bSaEPl`. Follow-up accepted actual-server proof repaired
  three stale LDA append-receipt column names, advanced the fork to Revision 2 with reversed pins,
  and reread the source unchanged at Revision 1. It also resumed Attempt 1 with its retained pin
  and nonce, then after submission started Attempt 2 with a distinct Pool selection ID and nonce.
  Artifacts: `/private/tmp/ple-course-empty-artifacts.BbKFFd` and
  `/private/tmp/ple-course-empty-artifacts.JTjOJ3`. Backend-independence, broader Pool Library
  discovery remain open. Accepted actual-main Instructor proof also imported a
  reusable Pool, changed its count from 2 to 1, attested and reordered its exact members, and
  reloaded the Assessment-owned Revision 2 while the reusable source stayed unchanged. Artifact:
  `/private/tmp/ple-course-empty-artifacts.lgyOMK`. Final gates passed: 135 model tests, 81
  PostgreSQL-feature LDA library tests, 99 server library tests, and
  `cargo check -p learning-data-access --tests --features postgres` after current opaque-reference
  and Pool-pin fixture corrections; that check compiled tests and did not run PostgreSQL integration
  tests. The full 7,155-pytest suite, 339 Node tests, TypeScript check, and checklist gates passed.
  These accepted receipts also establish the ordered Published-Question Pool definition, required
  Instructor interchangeability attestation, and canonical public `AAAA-ZBBB` IDs for Questions
  and Pools; they do not establish collision handling or backend-uniform behavior.
  Accepted sharing proof let a second vetted Instructor list and read root and child published Pools
  with exact public pins and no Course facts; nonmember mutation and Student/anonymous reads were
  denied without change. False or missing Pool interchangeability attestation returned 422 with no
  Pool created. Artifact: `/private/tmp/ple-course-empty-artifacts.hvS4KT`.
  Accepted actual-main Student proof denied three Question Library routes without Library API calls,
  while the Student Ribbon reached a Released Assessment through Coursework; the earlier native
  Attempt proof delivered its Question content there. Artifacts:
  `/private/tmp/ple-course-empty-artifacts.9s89JA` and
  `/private/tmp/ple-course-empty-artifacts.ZquNiI`.

- Accepted one real native fixed-Question Student Attempt path without changing product source.
  Private actual-server/actual-main proof created and published the checked-in Genetics PKU PLE
  Question JSON through Draft authoring, saved its exact Available Revision into a direct Practice
  Assessment, released it, and claimed a real Student roster invitation. Student HTTP issued four
  opaque-choice controls; the PKU choice saved and restored, whole-submitted as a graded 1/1
  Attempt, and permitted a distinct second unlimited Attempt despite the perfect result. Save and
  submission acknowledgements had exact completion/status fields without a score leak. Artifacts:
  `/private/tmp/ple-course-empty-artifacts.ZquNiI`. The submitted-history path initially omitted
  the saved response because `assessment_attempt_history.sql` projected the presentation checksum
  as `\\x`-prefixed bytea text; it now encodes the checksum as the bare hex required by the trusted
  presentation verifier. Fresh-schema HTTP evidence `/private/tmp/ple-course-empty-artifacts.Gz0L5V`
  restored the readable PKU response, and accepted actual-main browser evidence
  `/private/tmp/ple-course-empty-artifacts.ONrLSK` showed all six before-start facts with real
  Attempt 1 history plus the recorded response/1-of-1 summary. No Pool behavior, other backend
  grading, every Student viewport, or Regular Assignment default is claimed.

- Updated the Student-only Ribbon to use **Coursework** and **Back to Coursework**, and the
  missing-title Assessment access breadcrumb to say **Before you start**. The visible start failure
  now names the loaded individual Assessment Type. The existing focused Ribbon contract lane passed
  15 tests, and an accepted Student browser check showed the Coursework tab, breadcrumb, and
  Practice Question Assignment Type. This is a narrow terminology correction; broader Student
  interface terminology and the unlocked complete Student Ribbon task layout remain open.

- Accepted the remaining Student Attempt terminology cleanup: the Ribbon task area is **Attempt**,
  its current breadcrumbs are **Attempt** and **Attempt history**, and the history page uses
  **Previous attempt** with a neutral unavailable-score message. The existing focused 15-test
  Ribbon contract lane covers both breadcrumb terminals. Connected Student browser confirmation
  of the Attempt and Attempt history Ribbon labels remains pending.

- Cut over the bundled Pilot manifest and companion links to current Assessment names:
  `content/pilot/chapter_1_assessments.yaml` replaces the retired Assignments filename,
  two `assessment_title` keys replace `assignment_title`, and Pilot/Live Demo docs point to that
  canonical file. The existing loader was unchanged; the built `project-tools pilot-content`
  command passed with two chapters, eight Questions, and four adapted PGML sources. This is a
  current-input naming repair, not a full Pilot publication, rendering, or Student workflow proof.

- Accepted a bounded real Student member-visibility and before-start check for a Released direct
  Practice Assessment in an Empty Course. The private isolated actual-server/exact-main proof
  imported and claimed a real Course roster invitation, showed the Released Assessment to its
  member, omitted an Unreleased sibling, and denied an outsider. The browser displayed title,
  Practice Type, one Question/point, the exact one-hour time limit, and the explicit zero-previous-
  Attempts state without clicking Start. Artifacts are
  `/private/tmp/ple-course-empty-artifacts.JCFLm9`; Question rendering, nonempty prior-Attempt
  history, grading, and canonical screenshot-gallery publication were not exercised. No product
  source or broad checklist status changed for this proof.

- Corrected Instructor Course Instance creation to offer an Empty source by default or explicit
  adoption of an exact public Blueprint Revision. The Blueprint picker loads its list only when
  adoption is selected; Empty creation has no Blueprint prerequisite. The browser and Rust contracts
  now share one strict `source` union with camel-case Adopted fields and reject retired flat or
  snake-case inputs. A focused four-test browser contract gate, Rust wire test, TypeScript check,
  and server build passed. Accepted private actual HTTP plus exact-main browser proof created,
  persisted, and displayed an Empty Instructor Course with zero Blueprint-list requests, `no-store`
  responses, and Student creation denial. This closes only the exact HG Empty Instructor creation
  row. A separate accepted private actual-HTTP and exact-main browser proof created and published
  a Public Blueprint through its API, selected its exact Revision in the Instructor form, created
  a daughter Course, and read its Unreleased Practice Question Assignment with one fixed Question
  Revision, two points, finite whole-Attempt limit three, and no delivery dates. The exact Public
  Blueprint creation row and creation-only Empty-or-Blueprint composite are verified. Pool copying,
  the broader Blueprint publication workflow, direct Question/Pool authoring, and the rest of the
  Course teaching lifecycle are still pending. A follow-up accepted actual-main/HTTP proof opened
  a newly created Empty Course's **Create Assessment** task, created an Unreleased direct-origin
  Practice Assessment through the UI, observed its Course-list row, and read its direct origin and
  unset dates. A second accepted actual-main/HTTP run added/saved one available Published Question
  with its exact Revision and one-point pin, set a valid Due and positive time limit in Properties,
  passed release readiness, released the direct Assessment, and observed its Released Course row.
  This is direct authoring/release evidence, not Pool copying, renderer output, Student delivery,
  Attempt creation, or grading proof. The shared initial-state default and accepted direct/adopted
  creation observations close only the exact **Course Instance Assessments start unreleased** row;
  Template copying flows through that same direct creator. Complete validation, delivery, and
  lifecycle behavior remain open.
- Added a clear first action to the Question Library empty-results surface. It now explains the
  global Published Question discovery purpose, preserves the shorter-search/broader-topic recovery,
  and links visibly and by keyboard to the existing Instructor **My Draft Questions** route to
  create private work before publication. The canonical route helper supplies the destination;
  no publication or API action was added. A one-time source reproduction showed the earlier missing
  purpose/action, and an isolated Solid rendered Browse-empty proof passed purpose, recovery,
  accessible link, and Enter navigation. Search-empty rendering, connected production routing,
  other zero-record collections, and C44's broad checklist closure remain pending.
- Repaired the tracked Published Question fixture data for the current unversioned Question and
  Assessment contract: `questionFormat`, current Course/Assessment References, Assessment
  policy/evidence and Gradebook names, submitted timestamps, zero-based Issued Question positions,
  and `poolRevisionMember` now match the typed reader. The authored Question content and two asset
  files were not regenerated. `project-tools fixtures --check` validated all three tracked files;
  the full debug build passed, and independent review accepted the fixture cutover.
...
- Closed C368's bounded Published Question shared-metadata behavior with accepted isolated
  actual-server HTTP and private exact-main browser proof. An Instructor selected Questions,
  replaced tags and subject, cleared subject, and saw the updated search projection. Another
  authorized Instructor advanced one selected Question; the stale two-Question update returned
  all-or-none 412, and the browser refreshed current metadata without automatically retrying.
  Anonymous/Student requests were denied, successful responses were `no-store`, and source,
  Revision, and availability were unchanged. The three-Question fixture does not establish
  thousands-Question or 13k cleanup practicality, backend rendering, or canonical screenshot
  collection acceptance; C366 and practical-scale rows remain open.
- Closed C61's remaining bounded Assessment-list scan row with accepted private actual-server and
  exact-main browser proof. Due Soon showed Course, release state, and Account-zone Due per cross-
  Course row; the first owned Course retained three readable Assessment rows at 1280px and 720px
  with a Los Angeles browser and Chicago Account zone. A second owned Course showed three mixed
  Released/Unreleased rows with distinct Due values matched row-by-row to its actual 200/`no-store`
  Assessment-list response. The Course heading qualified each row. Root manager visual review and
  independent artifact review accepted this bounded scan behavior. Released rows were privileged
  fixture projection state, not release-workflow evidence. The broader spreadsheet-like Course and
  Assessment collection-density judgment, empty/error states, deployment gateway, WASM runtime,
  and connected Template delivery remain open.
...
- Corrected the Linux API image builder's narrow context for the server's reviewed embedded RDKit
  pair: `Containerfile.api` copies only the current JS/WASM inputs, and `.containerignore` admits
  only those two asset files. The Linux `server_core` compilation passed after this correction;
  image commit and screenshot-stack startup remain blocked by insufficient Podman VM image storage,
  not by a compiler error. No vendored bytes, dependency pins, runtime file surface, or service
  lifecycle changed. Updated the browser fixture's current Assessment consumers and one-based
  Student display ordinal; full Node tests passed 337/337 and TypeScript type checking passed.
