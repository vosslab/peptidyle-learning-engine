// test_flat_question_asset_client.mjs - transport and secrecy checks for protected hotspot images.
...
import {
  ...
  createFlatQuestionAssetClient,
  decodeFlatQuestionAssetDescriptor,
} from "../src/features/flat_question_authoring/flat_question_asset_client.ts";
...
test("asset descriptor decoder accepts only the safe exact browser projection", () => {
  ...
  for (const value of invalid) {
    assert.throws(() => decodeFlatQuestionAssetDescriptor(value), /must be/u);
  }
  const serialized = JSON.stringify(decodeFlatQuestionAssetDescriptor(DESCRIPTOR));
...
test("asset list preserves an empty library and reports a safe request failure", async () => {
  const empty = createFlatQuestionAssetClient({ fetch: async () => json([]) });
  assert.deepEqual(await empty.list(WORKSPACE), []);

  const unavailable = createFlatQuestionAssetClient({
    fetch: async () => json({ error: "private" }, 503),
  });
  await assert.rejects(unavailable.list(WORKSPACE), FlatQuestionAssetRequestError);
});

test("asset upload transports only opaque bytes and the two server-owned author headers", async () => {
  let captured;
  ...
  const uploaded = await client.upload(WORKSPACE, {
    image,
    displayLabel: "Cell membrane diagram",
    provenance: "Instructor-created diagram",
  });
  ...
  assert.equal(captured.url.endsWith(`/api/workspaces/${WORKSPACE}/flat-question-assets`), true);
  ...
  assert.equal(captured.headers.get("x-ple-asset-provenance"), "Instructor-created diagram");
