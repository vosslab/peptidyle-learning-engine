...
def test_live_demo_browser_owns_only_typed_tls_launch_and_evidence_receipts(
	...
	assert local_stack_control.disposable_stack_cleanup.owned_project_images(selected) == (
	...
	with pytest.raises(local_stack_control.models.ControllerError, match="generic Compose"):
		local_stack_control.disposable_stack_cleanup.compose_command(selected, ["exec", "api", "env"])
...
def test_database_baseline_profile_allows_only_its_postgres_oracle_commands(
	...
	assert environment["COMPOSE_PROJECT_NAME"] == "ple-live-demo-browser"
	argv, _ = local_stack_control.disposable_stack_cleanup.compose_command(
	...
	for arguments in (
		...
		argv, _ = local_stack_control.disposable_stack_cleanup.compose_command(selected, list(arguments))
	...
	for arguments in (
		...
		with pytest.raises(local_stack_control.models.ControllerError, match="database baseline Compose"):
			local_stack_control.disposable_stack_cleanup.compose_command(selected, arguments)
...
def test_cross_store_profile_allows_only_its_two_store_oracle_commands(
	...
	for arguments in (
		...
		argv, environment = local_stack_control.disposable_stack_cleanup.compose_command(selected, list(arguments))
		assert argv[-len(arguments):] == list(arguments)
		assert environment["COMPOSE_PROJECT_NAME"] == "ple-live-demo-browser"
	for arguments in (
		["up", "-d", "postgres"],
		...
		with pytest.raises(local_stack_control.models.ControllerError, match="cross-store Compose"):
			local_stack_control.disposable_stack_cleanup.compose_command(selected, list(arguments))
...
def test_live_demo_renderer_claim_resolves_api_and_rejects_a_worker_snapshot(
	...
	selected = disposable(tmp_path)
	argv, _environment = local_stack_control.disposable_stack_cleanup.evidence_log_command(
		selected, "renderer_delivery", evidence_snapshot(selected, "api")
	)
	assert argv[:4] == ["podman", "logs", "--tail", "5000"]
	with pytest.raises(local_stack_control.models.ControllerError, match="exactly one"):
		local_stack_control.disposable_stack_cleanup.evidence_log_command(
