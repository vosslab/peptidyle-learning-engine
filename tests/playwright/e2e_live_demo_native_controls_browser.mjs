...
try {
  ...
  const controls = page.locator(
    "section[data-route-surface='assignmentOverview'] article.question-presentation section.question-response-control",
  );
  await controls.nth(7).waitFor({ state: "visible" });
  if ((await controls.count()) !== 8)
    throw new Error("M12 browser prerequisite did not issue exactly eight native controls");

  const controlWithLegend = (name) => controls.filter({ has: page.getByRole("group", { name }) });
  const single = controlWithLegend("Choose your response").filter({
    has: page.locator("input[type='radio']"),
  });
  await single.locator("input").first().focus();
  await page.keyboard.press("Space");
  await ready(single);
  const multiple = controlWithLegend("Choose your response").filter({
    has: page.locator("input[type='checkbox']"),
  });
  await multiple.locator("input").first().focus();
  await page.keyboard.press("Space");
  await ready(multiple);
  const fill = controls.filter({ has: page.getByLabel("Short written response") });
  await fill.getByLabel("Short written response").focus();
  await page.keyboard.type("one");
  await ready(fill);
  const multi = controlWithLegend("Complete each blank");
  await multi.locator("input").first().focus();
  await page.keyboard.type("one");
  await page.keyboard.press("Tab");
  await page.keyboard.type("two");
  await ready(multi);
  const numeric = controls.filter({ has: page.getByLabel("Numeric response") });
  await numeric.getByLabel("Numeric response").focus();
  await page.keyboard.type("1");
  await ready(numeric);
  const matching = controlWithLegend("Match each prompt");
  await matching.getByRole("radio").nth(0).focus();
  await page.keyboard.press("Space");
  await matching.getByRole("radio").nth(3).focus();
  await page.keyboard.press("Space");
  await ready(matching);
  const ordering = controlWithLegend("Put the Ordering Items in order");
  await ordering.getByRole("button", { name: /Move Ordering Item 1 later/u }).focus();
  await page.keyboard.press("Space");
  await ready(ordering);
  ...
  if (hotspotImageHandle === null)
    throw new Error("fixed HOTSPOT logical asset image was unavailable");
  ...
  await hotspot.locator("input").first().focus();
  await page.keyboard.press("Space");
  await ready(hotspot);
  ...
  if (
    ...
    throw new Error("M12 format-only controls exposed an M13 control");
  if (prohibitedNetwork)
    throw new Error("M12 format-only keyboard interaction made a submission/grading request");
  if (logicalAssetRequests === 0)
    throw new Error("fixed HOTSPOT did not request its logical same-origin asset route");
} finally {
  await context.close();
  await browser.close();
}
