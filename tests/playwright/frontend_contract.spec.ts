...
async function useHttpRosterFixture(page: Page, role: "instructor" | "student"): Promise<void> {
  ...
  await page.route("**/api/auth/session", async (route): Promise<void> => {
    await route.fulfill(
      fixtureJson({
        authenticated: true,
        ...
        user: {
          ...
          displayName: role === "instructor" ? "Local Instructor" : "Local Student",
          roles: [role],
        },
      }),
    );
  });
  await page.route(`**/api/courses/${IDS.course}/assignments`, async (route): Promise<void> => {
    await route.fulfill(fixtureJson({ items: [], nextCursor: null }));
  });
}
...
test("the local roster control is absent when the server does not advertise it", async ({
  page,
}) => {
  await useHttpRosterFixture(page, "student");
  const localMemberRequests: string[] = [];
  page.on("request", (request) => {
    if (new URL(request.url()).pathname.endsWith("/local-development-members")) {
      localMemberRequests.push(request.method());
    }
  });
  await page.route(`**/api/courses/${IDS.course}/roster`, async (route): Promise<void> => {
    await route.fulfill(
      fixtureJson({
        members: [],
        pendingInvitations: [],
  ...
  await expect(page.getByRole("heading", { name: "Add local development student" })).toHaveCount(0);
  await expect(page.getByLabel("Configured learner alias")).toHaveCount(0);
  expect(localMemberRequests).toEqual([]);
});

test("a local roster alias error preserves the keyboard entry with actionable recovery", async ({
  page,
}) => {
  await useHttpRosterFixture(page, "instructor");
  await page.route(`**/api/courses/${IDS.course}/roster`, async (route): Promise<void> => {
    await route.fulfill(
      fixtureJson({
        members: [],
  ...
  await page.route(
    `**/api/courses/${IDS.course}/local-development-members`,
    async (route): Promise<void> => {
      expect(route.request().method()).toBe("POST");
      expect(route.request().postDataJSON()).toEqual({ learnerAlias: "unknown-local" });
      await route.fulfill(fixtureJson({}, 404));
    },
  );
  await page.goto("/");
  await navigateWithinSpa(page, `/instructor/courses/${IDS.course}/students`);
  const alias = page.getByLabel("Configured learner alias");
  const submit = page.getByRole("button", { name: "Add active student" });
  await tabTo(page, alias, 30);
  await page.keyboard.type("unknown-local");
  await page.keyboard.press("Tab");
  ...
  await expect(alias).toHaveValue("unknown-local");
  await expect(submit).toBeEnabled();
});

test("J12 local roster activation is keyboard-visible, pending, and idempotent", async ({
  page,
}) => {
  await useHttpRosterFixture(page, "instructor");
  let activated = false;
  ...
  const firstActivationPending = new Promise<void>((resolve) => {
  ...
  function roster(): {
    readonly members: ReadonlyArray<{
      ...
      readonly rosterEmail: null;
      readonly rosterId: null;
      readonly source: "localDevelopment";
      readonly role: "student";
      readonly status: "active";
    }>;
    readonly pendingInvitations: ReadonlyArray<never>;
    readonly allowedEmailDomains: ReadonlyArray<never>;
    readonly signupPosture: "invitationOnly";
    readonly localDevelopmentRoster: true;
    ...
    return {
      ...
      localDevelopmentRoster: true,
      nextCursor: null,
      rosterRevision: activated ? 2 : 1,
    };
  }
  await page.route(`**/api/courses/${IDS.course}/roster`, async (route): Promise<void> => {
    await route.fulfill(fixtureJson(roster()));
  });
  await page.route(
    `**/api/courses/${IDS.course}/local-development-members`,
    async (route): Promise<void> => {
      expect(route.request().method()).toBe("POST");
      expect(route.request().postDataJSON()).toEqual({ learnerAlias: "student-local" });
      activationRequests += 1;
      if (activationRequests === 1) await firstActivationPending;
      activated = true;
      await route.fulfill(
        fixtureJson({ member: roster().members[0], rosterRevision: 2 }, 200, { etag: '"2"' }),
      );
    },
  );
  await page.goto("/");
  ...
  const submit = page.getByRole("button", { name: "Add active student" });
  await tabTo(page, alias, 30);
  await page.keyboard.type("student-local");
  await page.keyboard.press("Tab");
  await expect(submit).toBeFocused();
  await page.keyboard.press("Enter");
  await expect(submit).toBeDisabled();
  releaseFirstActivation?.();
  const row = page.getByRole("row", { name: /Local Student.*Local pilot.*active/u });
  await expect(row).toBeVisible();
  await expect(row).toBeFocused();
  await expect(page.getByRole("status")).toContainText(
    "Local Student is now an active student in this course.",
  );
  await expect(row).not.toContainText("@");

  await tabTo(page, alias, 70);
  await expect(alias).toHaveValue("student-local");
  await page.keyboard.press("Tab");
  await expect(submit).toBeFocused();
  await page.keyboard.press("Enter");
  await expect.poll(() => activationRequests).toBe(2);
  await expect(row).toBeVisible();
  await expect(row).toBeFocused();
});
