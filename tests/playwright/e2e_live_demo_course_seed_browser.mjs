...
async function expectAttempt(page) {
  ...
  await expect(attempt.getByRole("heading", { level: 1, name: assignmentTitle })).toBeVisible();
  const navigation = attempt.getByRole("navigation", { name: "Assignment questions", exact: true });
  ...
  await expect(
    attempt.getByRole("heading", { name: "Finish Assignment", exact: true }),
  ).toBeVisible();
}
...
async function verifyElena(page) {
  ...
  await expectGradebookRow(page, "BIO301-JACK", "In progress", "0 of 4", "0 / 0");
...
async function verifyMary(page) {
  ...
  await enterStudentCourse(page, courseLongName);
  const card = assignmentCard(page);
  await expect(card).toHaveCount(1);
