#!/usr/bin/env node
// Refresh deterministic built-app student perspective and access-boundary evidence.
...
function captureMode() {
  ...
  throw new Error(
    "usage: node tests/playwright/capture_student_access_visuals.mjs [--verify-only]",
  );
}
...
function runCapture(root, directory) {
  return new Promise((resolve, reject) => {
    const child = spawn(
      "npx",
      ["playwright", "test", "tests/playwright/student_access_visuals.spec.ts", "--workers=1"],
      {
        cwd: root,
        env: {
          ...process.env,
          PLE_STUDENT_ACCESS_VISUALS_DIR: directory,
          PLE_UI_CORPUS_CAPTURE_OWNER: "studentMock",
        },
        stdio: "inherit",
      },
    );
...
async function main() {
  ...
  await runCorpusCapture({
    ...
    pipeline: "mock",
    mode: captureMode(),
    label: "student perspective and access-boundary visuals",
...
main().catch((error) => {
  ...
  process.stderr.write(`FAIL: student/access visual capture: ${message}\n`);
  process.exitCode = 1;
});
