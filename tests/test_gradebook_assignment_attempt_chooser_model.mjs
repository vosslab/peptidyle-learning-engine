...
import { GradebookAssignmentAttemptChooserSession } from "../src/pages/gradebook_assignment_attempt_chooser_model.ts";
...
test("chooser retry keeps the newer exact Assignment Attempt when an older read finishes last", async () => {
  ...
  const session = new GradebookAssignmentAttemptChooserSession(
    scope(),
    {
      getSubmittedAssignmentAttemptChoices: () => requests[requestIndex++].promise,
    },
    () => undefined,
  );
  ...
  assert.equal(
    session.state.kind === "ready" ? session.state.rows[0]?.assignmentAttempt : undefined,
    "R-2",
  );
});
...
test("chooser retry replaces a pending continuation and leaves the new page pageable", async () => {
  ...
  initial.resolve(pageWithAssignmentAttempts(["R-1"], "initial-cursor"));
  await initialLoad;
  const staleLoad = session.loadMore();
  const retryLoad = session.retry();
  retry.resolve(pageWithAssignmentAttempts(["R-2"], "retry-cursor"));
  await retryLoad;
  const currentLoad = session.loadMore();
  currentContinuation.resolve(pageWithAssignmentAttempts(["R-3"]));
  await currentLoad;
  staleContinuation.resolve(pageWithAssignmentAttempts(["R-stale"]));
  await staleLoad;
...
test("chooser rejects mixed and incoming duplicate Assignment Attempts atomically", async () => {
  for (const incomingAssignmentAttempts of [
    ["R-1", "R-2"],
    ["R-2", "R-2"],
  ]) {
    const initial = deferred();
    ...
    const initialLoad = session.start();
    initial.resolve(pageWithAssignmentAttempts(["R-1"], "cursor"));
    await initialLoad;
    const continuationLoad = session.loadMore();
    continuation.resolve(pageWithAssignmentAttempts(incomingAssignmentAttempts));
...
test("chooser rejects duplicate Assignment Attempts on its initial page before ready publication", async () => {
  const states = [];
  const session = new GradebookAssignmentAttemptChooserSession(
    scope(),
    {
      getSubmittedAssignmentAttemptChoices: async () => pageWithAssignmentAttempts(["R-1", "R-1"]),
    },
...
test("chooser retains rows on a current continuation error and disposes pending retry or continuation", async () => {
  ...
  const retryStates = [];
  let retryRequest = 0;
  const retrySession = new GradebookAssignmentAttemptChooserSession(
    scope(),
    {
      getSubmittedAssignmentAttemptChoices: () => {
        retryRequest += 1;
  ...
  const pendingInitial = Promise.resolve(pageWithAssignmentAttempts(["R-1"], "cursor"));
  const continuationRequest = deferred();
  const continuationSessionWithPending = new GradebookAssignmentAttemptChooserSession(
    scope(),
    {
      getSubmittedAssignmentAttemptChoices: (...[, , , query]) =>
  ...
  assert.deepEqual(
    continuationSessionWithPending.state.kind === "ready"
      ? continuationSessionWithPending.state.rows.map((row) => row.assignmentAttempt)
      : [],
    ["R-1"],
  );
});
...
test("disposed chooser completions cannot publish into a later chooser instance", async () => {
  ...
  const firstStates = [];
  const first = new GradebookAssignmentAttemptChooserSession(
    scope(),
    { getSubmittedAssignmentAttemptChoices: () => pending.promise },
    (state) => firstStates.push(state),
  );
  const firstLoad = first.start();
  first.dispose();

  const second = new GradebookAssignmentAttemptChooserSession(
    { ...scope(), membership: "M-2" },
    { getSubmittedAssignmentAttemptChoices: async () => page("R-2") },
    () => undefined,
  );
  await second.start();
  ...
  assert.equal(
    second.state.kind === "ready" ? second.state.rows[0]?.assignmentAttempt : undefined,
    "R-2",
  );
});
