// ribbon_shell_loader.ts - compiles the real application-shell composition
// for browser evidence.

import { build, stop } from "esbuild";
import { solidPlugin } from "esbuild-plugin-solid";

export interface RibbonShellBundle {
  readonly javascript: Uint8Array;
  readonly stylesheet: string;
}

export async function bundleRibbonShellHarness(): Promise<RibbonShellBundle> {
  try {
    const result = await build({
      ...
      format: "iife",
      globalName: "PleRibbonShellHarness",
      outfile: "ribbon_shell_harness.js",
      platform: "browser",
