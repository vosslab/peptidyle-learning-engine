// ribbon_integration_loader.ts - compiles the real AppRibbon harness for browser evidence.

import { build } from "esbuild";
import { solidPlugin } from "esbuild-plugin-solid";

export interface RibbonIntegrationBundle {
  readonly javascript: Uint8Array;
  readonly stylesheet: string;
}

/** Uses the production Solid DOM transform rather than a hand-authored HTML substitute. */
export async function bundleRibbonIntegrationHarness(): Promise<RibbonIntegrationBundle> {
  const result = await build({
    bundle: true,
    entryPoints: [new URL("./ribbon_integration_harness.tsx", import.meta.url).pathname],
    format: "esm",
    outfile: "ribbon_integration_harness.js",
    platform: "browser",
