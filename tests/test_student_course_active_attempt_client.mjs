...
import { decodeStudentCourseActiveAttempt } from "../src/api/decoders/live_student_course_landing.ts";
...
test("Active Attempt decoder accepts an ID or explicit no-active state", () => {
  assert.deepEqual(decodeStudentCourseActiveAttempt({ assessmentAttemptId: ATTEMPT_ID }), {
    assessmentAttemptId: ATTEMPT_ID,
  });
  assert.deepEqual(decodeStudentCourseActiveAttempt({ assessmentAttemptId: null }), {
...
test("Active Attempt client reads the no-store Course-scoped shortcut projection", async () => {
  ...
  assert.deepEqual(result, { assessmentAttemptId: ATTEMPT_ID });
  assert.equal(
    requests[0].url,
