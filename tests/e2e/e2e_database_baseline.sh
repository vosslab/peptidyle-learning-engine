...
# PostgreSQL Migration Acceptance Runtime runner. Keeping this public entry point lets the aggregate
# E2E lane run that canonical baseline directly.

set -euo pipefail

script_directory="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
exec bash "$script_directory/e2e_postgres_migration_acceptance.sh" "$@"
