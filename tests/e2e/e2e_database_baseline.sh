...
[ "$#" -eq 2 ] && [ "$1" = "--runtime-manifest" ] && [ "$2" = "runtime.yaml" ] || {
	echo "database baseline E2E: private child requires the owner-created runtime manifest" >&2
	exit 2
}
RUNTIME_MANIFEST="$2"
WORKSPACE="$(pwd -P)"
RUNTIME_MANIFEST_PATH="$WORKSPACE/$RUNTIME_MANIFEST"
readonly RUNTIME_MANIFEST WORKSPACE RUNTIME_MANIFEST_PATH
...
run_project_tools() {
	(
		cd "$WORKSPACE"
		cargo run --manifest-path "$REPO_ROOT/Cargo.toml" --quiet -p project-tools -- database "$@" --acceptance-runtime
	)
}
...
run_live_cargo_test "passwordless account, roster, and role separation" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "derived entitlement, materialized receipt, and forced RLS" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "family-filtered concurrent worker claims" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "ranked catalog Store cursor behavior" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "ranked catalog continuation snapshot visibility" cargo test -p learning-data-access --features postgres \
	--test postgres_catalog_search_live \
...
run_live_cargo_test "ranked catalog operator capability" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "course appearance revision, role, and current-pointer policy" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "mandatory course term constraints, round-trip, and RLS" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "typed public references, RLS, and immutable bylines" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "normalized effective policy, provenance, and RLS" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "assignment disclosure policy, current S3 timing, and RLS" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "concurrent prefetch preserves immutable submission replay" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "immutable private flat-question image registry" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "QTI partial import, provenance, and tenant isolation" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "recognized QTI profile full authoring and grading path" cargo test -p server_core \
...
run_live_cargo_test "QTI-profile-to-flat atomic conversion and private provenance" cargo test -p learning-data-access --features postgres \
	--test postgres_flat_import_provenance_live \
	-- --ignored --test-threads=1

echo "database baseline E2E: flat-question private grading boundary"
run_live_cargo_test "flat-question private grading boundary" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "course-grade upgrade backfill and retention wrapper" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "assignment teaching lifecycle, policy, receipt, and RLS" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "T2 teaching-operations upgrade and schema" cargo test -p learning-data-access --features postgres \
	--test postgres_teaching_operations_upgrade_live \
	-- --ignored --test-threads=1

echo "database baseline E2E: T2 course groups and current policy receipts"
run_live_cargo_test "T2 course groups and current policy receipts" cargo test -p learning-data-access --features postgres \
...
run_live_cargo_test "T2 teaching authority, approval, invitation, and concurrency" cargo test -p learning-data-access --features postgres \
	--test postgres_teaching_authority_live \
	-- --ignored --test-threads=1

echo "database baseline E2E: LD3 assignment mutation brokers and atomic revision authority"
run_live_cargo_test "LD3 assignment mutation brokers and atomic revision authority" cargo test -p learning-data-access --features postgres \
	--test postgres_assignment_mutation_capabilities_live \
	-- --ignored --test-threads=1

echo "database baseline E2E: LD3 group-policy session authority and affected-assignment witness"
run_live_cargo_test "LD3 group-policy session authority and affected-assignment witness" cargo test -p learning-data-access --features postgres \
	--test postgres_course_group_policy_authority_live \
	-- --ignored --test-threads=1

echo "database baseline E2E: LD3 issued-attempt route, lifecycle, and private-grader boundary"
run_live_cargo_test "LD3 issued-attempt route, lifecycle, and private-grader boundary" cargo test -p learning-data-access --features postgres \
	--test postgres_issued_attempt_read_live \
	postgres_issued_attempt_read_is_broker_first_route_bound_and_lifecycle_aware \
	-- --ignored --exact --test-threads=1

echo "database baseline E2E: T3 preview plane authorization, atomic audit, and identity-free subject"
run_live_cargo_test "T3 preview plane authorization, atomic audit, and identity-free subject" cargo test -p learning-data-access --features postgres \
