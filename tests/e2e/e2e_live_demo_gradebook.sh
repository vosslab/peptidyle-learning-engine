...
podman exec "$postgres" sh -lc 'psql -X -v ON_ERROR_STOP=1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -At -c "DO \$\$ BEGIN IF has_table_privilege('\''ple_app'\'', '\''ple_private.grading_result'\'', '\''SELECT'\'') OR has_table_privilege('\''ple_app'\'', '\''ple_private.assessment_attempt_saved_response'\'', '\''SELECT'\'') THEN RAISE EXCEPTION '\''direct Gradebook evidence access widened'\''; END IF; END \$\$; SELECT '\''gradebook_catalog_authority'\'';"' | rg -qx 'gradebook_catalog_authority' || { echo "Gradebook least-privilege evidence failed" >&2; exit 1; }
