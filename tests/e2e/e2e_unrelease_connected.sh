...
race_assessment_id="$(psql_admin -qAt -c "SELECT assessment.assessment_id FROM ple_data.assessment AS assessment JOIN ple_data.assessment_policy_snapshot AS policy ON policy.assessment_policy_snapshot_id = assessment.assessment_policy_snapshot_id WHERE policy.assessment_title = 'Unrelease lock race'")"
race_course_instance_id="$(psql_admin -qAt -c "SELECT course_instance_id FROM ple_data.course_instance WHERE course_short_name = 'UNR-1'")"
...
if [[ ! "$race_assessment_id" =~ ^A[0-9ABCDEFGHJKMNPQRSTVWXYZ]{8}$ ]] \
	|| [[ ! "$race_course_instance_id" =~ ^CI[0-9ABCDEFGHJKMNPQRSTVWXYZ]{8}$ ]]; then
	echo "Unrelease connected acceptance: race public ID is invalid" >&2
	exit 1
fi
...

{
	printf '%s\n' \
		...
		"SELECT 1 FROM ple_data.assessment WHERE assessment_id = '$race_assessment_id' FOR UPDATE;" \
...
wait "$locker_pid"
wait "$unrelease_pid"
grep -qx "$race_assessment_id|Unrelease lock race|unreleased|2|0|0|0|0" "$race_workspace/unrelease.out" || {
...
BEGIN
    IF EXISTS (
        ...
        WHERE assessment_id = '$race_assessment_id'
    ) THEN
