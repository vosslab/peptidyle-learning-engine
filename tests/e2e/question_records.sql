...
BEGIN
    ...
    IF NOT EXISTS (
       ...
       OR NOT EXISTS (
           ...
           SELECT 1 FROM pg_constraint
            WHERE conrelid IN (
                'ple_private.draft_question_source_binding'::regclass,
                'ple_private.question_revision_source_binding'::regclass
...
INSERT INTO ple_private.draft_question_metadata (
    ...
    '00000000-0000-0000-0000-000000000905', 'Object-backed source',
    'Draft source fixture', '2026-08-31T00:00:00Z', '2026-08-31T00:00:00Z'
);
...
DO $$
BEGIN
    PERFORM ple_api.bind_draft_question_source(
        '00000000-0000-0000-0000-000000000905', 1,
        '00000000-0000-0000-0000-000000000902', 'webwork', 'webworkPg',
        'set/object-backed.pg', NULL, NULL, NULL,
        '00000000-0000-0000-0000-000000000903', repeat('ab', 32));
    RAISE EXCEPTION 'Draft Question Source Binding accepted stale mismatched facts';
...
BEGIN
    PERFORM ple_api.bind_draft_question_source(
        ...
        NULL, NULL, NULL, NULL,
        '00000000-0000-0000-0000-000000000903', repeat('ab', 32));
    RAISE EXCEPTION 'Draft Question Source Binding accepted an unauthorized workspace';
...
BEGIN
    IF (SELECT draft_question_edit_number FROM ple_private.draft_question
       ...
       OR (SELECT count(*) FROM ple_private.draft_question_source_binding
            WHERE draft_question_uuid = '00000000-0000-0000-0000-000000000905') <> 1 THEN
        RAISE EXCEPTION 'Draft Question Source Binding CAS did not apply exactly once per changed facts';
    END IF;
END $$;
...
INSERT INTO ple_data.published_question_metadata (
    ...
    'SRC-0001', 'Published object-backed source', 'Published object-backed source',
    '2026-08-31T00:00:00Z', '2026-08-31T00:00:00Z'
);
