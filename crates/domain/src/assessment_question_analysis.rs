//! Current, course-local Assessment Question Analysis projections.
//!
...
use question_model::{
    AssessmentEntryId, AssessmentId, CourseInstanceId, PublishedQuestionRevisionTuple,
...
/// One current aggregate Question row delivered by an Assessment.
...
pub struct AssessmentQuestionAnalysis {
    ...
    /// Pearson correlation between Question credit and other Question credit in the Assessment Attempt.
    pub question_discrimination: Option<f64>,
    pub question_outcome_distribution: QuestionOutcomeDistribution,
    /// Mean elapsed milliseconds from the terminal Student submission for Assessment Attempts
    /// that delivered this Question.
    pub average_completion_time_millis: Option<u64>,
}

/// Current report header and all Question rows for one Course Instance Assessment.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub struct AssessmentQuestionAnalysisReport {
    ...
    pub in_progress_assessment_attempt_count: u32,
    /// A terminal submitted attempt lacks a coherent automated score.
    /// Score-derived Assessment metrics remain suppressed while this is true.
    ...
    pub average_completion_time_millis: Option<u64>,
    pub question_analyses: Vec<AssessmentQuestionAnalysis>,
}

/// Private aggregate inputs stripped of all Student and response identity.
#[derive(Debug, Clone, Default, PartialEq)]
pub struct AssessmentQuestionAnalysisMetricInput {
    pub graded_credits: Vec<f64>,
    pub graded_correct: Vec<bool>,
    pub rest_of_assessment_credits: Vec<f64>,
    pub unanswered_attempt_count: u32,
...
/// Pure computed metrics used by storage-specific Assessment Question Analysis builders.
#[derive(Debug, Clone, PartialEq)]
pub struct AssessmentQuestionAnalysisMetrics {
    pub graded_attempt_count: u32,
...
/// Invalid aggregate input rejected before it can corrupt an Assessment Question Analysis report.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AssessmentQuestionAnalysisMetricError {
    UnpairedCredits,
    UnpairedCorrectness,
    InvalidCredit,
    InvalidRestOfAssessmentCredit,
}

impl std::fmt::Display for AssessmentQuestionAnalysisMetricError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(match self {
            Self::UnpairedCredits => {
                "Assessment Question Analysis credits must remain paired with other Assessment Attempt Question credits"
            }
            Self::UnpairedCorrectness => {
                "Assessment Question Analysis credits must remain paired with correctness"
            }
            Self::InvalidCredit => {
                "Assessment Question Analysis credit must be finite and between -1000 and 1000"
            }
            Self::InvalidRestOfAssessmentCredit => {
                "Assessment Question Analysis other Assessment Attempt Question credit must be finite"
            }
        })
    }
}

impl std::error::Error for AssessmentQuestionAnalysisMetricError {}

/// Calculates current-only Assessment Question Analysis metrics from identity-free aggregate inputs.
///
...
pub fn calculate_assessment_question_analysis_metrics(
    input: &AssessmentQuestionAnalysisMetricInput,
) -> Result<AssessmentQuestionAnalysisMetrics, AssessmentQuestionAnalysisMetricError> {
    if input.graded_credits.len() != input.rest_of_assessment_credits.len() {
        return Err(AssessmentQuestionAnalysisMetricError::UnpairedCredits);
    }
    if input.graded_credits.len() != input.graded_correct.len() {
        return Err(AssessmentQuestionAnalysisMetricError::UnpairedCorrectness);
    }
    if input
        .graded_credits
        .iter()
    ...
    {
        return Err(AssessmentQuestionAnalysisMetricError::InvalidCredit);
    }
    if input
        .rest_of_assessment_credits
        .iter()
        .any(|credit| !credit.is_finite())
    {
        return Err(AssessmentQuestionAnalysisMetricError::InvalidRestOfAssessmentCredit);
    }
    ...
    let question_discrimination = pearson_correlation(credits, &input.rest_of_assessment_credits);
    ...
    .unwrap_or(u32::MAX);
    Ok(AssessmentQuestionAnalysisMetrics {
...
mod tests {
    ...
    #[test]
    fn computes_assessment_question_metrics_and_safe_outcomes() {
        let metrics = calculate_assessment_question_analysis_metrics(
            &AssessmentQuestionAnalysisMetricInput {
                graded_credits: vec![1.0, 0.5, 0.0],
                graded_correct: vec![true, false, false],
                rest_of_assessment_credits: vec![1.0, 0.5, 0.0],
    ...
    fn leaves_undefined_math_unavailable() {
        let metrics = calculate_assessment_question_analysis_metrics(
            &AssessmentQuestionAnalysisMetricInput {
                ...
                rest_of_assessment_credits: vec![0.5],
                ..AssessmentQuestionAnalysisMetricInput::default()
            },
        )
        .expect("valid metrics");
        ...
        let metrics = calculate_assessment_question_analysis_metrics(
            &AssessmentQuestionAnalysisMetricInput {
                ...
                rest_of_assessment_credits: vec![0.5, 0.5],
                ..AssessmentQuestionAnalysisMetricInput::default()
            },
        )
        .expect("valid metrics");
    ...
    fn correctness_controls_difficulty_independently_of_credit() {
        let metrics = calculate_assessment_question_analysis_metrics(
            &AssessmentQuestionAnalysisMetricInput {
                ...
                graded_correct: vec![true, false, false],
                rest_of_assessment_credits: vec![0.0, 2.0, -0.5],
                ..AssessmentQuestionAnalysisMetricInput::default()
            },
        )
