...
pub fn write(exam: &PrintExam, layout: PrintLayout) -> ExportArtifact {
    ...
    let mut media = Vec::<(QuestionImageAssetId, Vec<u8>)>::new();
...
fn document_relationships(media: &[(QuestionImageAssetId, Vec<u8>)]) -> String {
...
fn document_xml(flow: &[Vec<FlowBlock>], media: &[(QuestionImageAssetId, Vec<u8>)]) -> String {
