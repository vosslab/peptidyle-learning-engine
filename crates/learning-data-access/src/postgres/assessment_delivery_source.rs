...
pub(super) fn presentation_evidence_from_row(
    ...
    let author_content = row
        .try_get::<Option<serde_json::Value>, _>("author_content")
        .map_err(map_sqlx_error)?
        .map(serde_json::from_value)
        .transpose()
        .map_err(|_| StoreError::InvalidRecord("Author content evidence is invalid".to_string()))?;
...
pub(super) fn optional_presentation_evidence_from_row(
    row: &sqlx::postgres::PgRow,
) -> Result<Option<StudentAssessmentAttemptPresentationEvidence>, StoreError> {
...
pub(super) fn source_from_row(
    row: &sqlx::postgres::PgRow,
) -> Result<StudentAssessmentAttemptPresentationSource, StoreError> {
    ...
    match backend.as_str() {
        "ple" => Ok(StudentAssessmentAttemptPresentationSource::Ple {
            question_attempt,
            source: NativePleIssuanceSource {
                ...
                source_object_checksum: row
                    ...
                    .map_err(map_sqlx_error)?,
                reproduction: issuance_reproduction_from_row(row, "ple")?,
...
/// Decodes the physical nullable pair as one exhaustive logical state.
///
/// PostgreSQL's check constraint rejects partial pairs.  Preserve that same
/// invariant at the Rust boundary so a malformed procedure result cannot turn
/// into an invented static source or an incomplete seeded source.
pub(super) fn reproduction_from_row(
    row: &sqlx::postgres::PgRow,
) -> Result<QuestionReproduction, StoreError> {
    let question_seed = row
        .try_get::<Option<String>, _>("question_seed")
        .map_err(map_sqlx_error)?;
    let generated_parameter_sha256 = row
        .try_get::<Option<String>, _>("generated_parameter_sha256")
        .map_err(map_sqlx_error)?;
    match (question_seed, generated_parameter_sha256) {
        (None, None) => Ok(QuestionReproduction::Static),
        (Some(question_seed), Some(generated_parameter_sha256)) => question_seed
            .parse::<u64>()
            .map(QuestionSeed::new)
            .map(|question_seed| QuestionReproduction::Seeded {
                question_seed,
                generated_parameter_sha256,
            })
            .map_err(|_| StoreError::InvalidRecord("Question Seed is invalid".to_string())),
        _ => Err(StoreError::InvalidRecord(
            "Question reproduction seed and parameter checksum must be paired".to_string(),
        )),
    }
}

fn require_seeded_reproduction(
    reproduction: QuestionReproduction,
) -> Result<QuestionReproduction, StoreError> {
    if matches!(reproduction, QuestionReproduction::Seeded { .. }) {
        Ok(reproduction)
    } else {
        Err(StoreError::InvalidRecord(
            "Renderer-backed Question reproduction must be seeded".to_string(),
        ))
    }
}

/// Decodes an issuance-only input before the renderer has produced its
/// generated-parameter checksum.  This cannot be used for committed evidence.
pub(super) fn issuance_reproduction_from_row(
    row: &sqlx::postgres::PgRow,
    backend: &str,
) -> Result<QuestionIssuanceReproductionInput, StoreError> {
    let question_seed = row
        .try_get::<Option<String>, _>("question_seed")
        .map_err(map_sqlx_error)?;
    match (backend, question_seed) {
        ("ple", None) => Ok(QuestionIssuanceReproductionInput::Static),
