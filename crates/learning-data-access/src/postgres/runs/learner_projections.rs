//! Read-only authorization for learner run projections.
...
pub(super) async fn active_learner_run_for_read(
...
mod tests {
    ...
    fn active_learner_run_projection_does_not_request_source_locks() {
        let source = include_str!("learner_projections.rs");
        let helper = source
            .split("pub(super) async fn active_learner_run_for_read")
            .nth(1)
