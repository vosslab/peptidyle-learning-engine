...
#[derive(Clone)]
pub struct PostgresAssessmentStudentTimeAccommodationStore { pool: Pool }
...
impl AssessmentStudentTimeAccommodationStore for PostgresAssessmentStudentTimeAccommodationStore {
    async fn student_time_configuration(
        &self, token: SessionTokenHash, course: CourseInstanceReference,
        assessment: AssessmentReference, roster_id: String,
        ...
        if roster_id.is_empty() || roster_id.len() > 64
            || !roster_id.bytes().all(|value| value.is_ascii_alphanumeric() || b"._-".contains(&value)) {
        ...
        let expected = match save.as_ref().and_then(|value| value.expected_edit_number.as_deref()) {
            None => 0,
            Some(value) if value.bytes().all(|byte| byte.is_ascii_digit())
                && !value.starts_with('0') => value.parse::<i64>().ok().filter(|number| *number > 0).ok_or_else(invalid)?,
            Some(_) => return Err(invalid()),
        };
        let mut transaction = self.pool.begin().await.map_err(map_sqlx_error)?;
        sqlx::query("SET LOCAL ROLE ple_auth").execute(&mut *transaction).await.map_err(map_sqlx_error)?;
        let session = sqlx::query("SELECT session_id FROM ple_api.resolve_and_install_session(decode($1, 'hex'))")
            .bind(token.to_string()).fetch_optional(&mut *transaction).await.map_err(map_sqlx_error)?;
        if session.is_none() { return Err(StoreError::Forbidden); }
        sqlx::query("SET LOCAL ROLE ple_app").execute(&mut *transaction).await.map_err(map_sqlx_error)?;
        ...
        let stored_multiplier: Option<f64> = row.try_get("time_multiplier").map_err(map_sqlx_error)?;
        let edit: Option<i64> = row.try_get("accommodation_edit_number").map_err(map_sqlx_error)?;
        let base: Option<i32> = row.try_get("base_duration_seconds").map_err(map_sqlx_error)?;
        let effective: Option<i32> = row.try_get("effective_duration_seconds").map_err(map_sqlx_error)?;
        ...
        let projection = AssessmentStudentTimeAccommodation {
            ...
            time_multiplier: stored_multiplier, edit_number: edit.map(|value| value.to_string()),
        ...
        if projection.roster_id != roster_id { return Err(invalid()); }
        transaction.commit().await.map_err(map_sqlx_error)?;
        Ok(projection)
    }
}

fn invalid() -> StoreError { StoreError::InvalidRecord("Student time configuration is invalid".into()) }
