//! Student-safe class-statistics read over the current item-analysis report.
...
pub(super) async fn student_class_statistics(
    store: &PostgresStore,
    ...
    // snapshot. The Student read needs a coherent authorization decision, not
    ...
    let result = StudentClassStatistics::from_current_analysis(
