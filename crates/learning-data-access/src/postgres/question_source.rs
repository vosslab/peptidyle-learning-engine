...
use crate::{
    DraftQuestionSourceInput, DraftQuestionSourceStore, QuestionSourceId, SessionTokenHash,
    StoreError,
};
...
impl DraftQuestionSourceStore for PostgresDraftQuestionSourceStore {
    async fn register_draft_question_source(
        ...
        input.validate()?;
        let question_source_id = crate::random_uuid::random_uuid_v4(|error| {
        ...
        let row = sqlx::query(
            "SELECT ple_api.register_draft_question_source(\
             ...
             ) AS question_source_id",
        )
        .bind(question_source_id)
        .bind(input.draft_question_revision.as_uuid())
        ...
        .map_err(map_sqlx_error)?;
        let source =
            QuestionSourceId::from_uuid(row.try_get("question_source_id").map_err(map_sqlx_error)?);
