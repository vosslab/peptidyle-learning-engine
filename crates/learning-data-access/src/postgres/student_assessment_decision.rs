//! Shared PostgreSQL row decoder for the browser-owned Assessment decision.

use browser_api_contract::student_assessment_decision::{
...
pub(super) fn decode(
...
) -> Result<StudentAssessmentDecisionSummary, StoreError> {
    ...
    Ok(StudentAssessmentDecisionSummary {
...
fn start_decision(value: &str) -> Result<AssessmentStartDecision, StoreError> {
    match value {
        "may_start" => Ok(AssessmentStartDecision::MayStart),
        "not_yet_available" => Ok(AssessmentStartDecision::NotYetAvailable),
        "closed" => Ok(AssessmentStartDecision::Closed),
        "attempt_limit_reached" => Ok(AssessmentStartDecision::AttemptLimitReached),
        "late_work_refused" => Ok(AssessmentStartDecision::LateWorkRefused),
        _ => Err(invalid("Assessment Start Decision")),
    }
}
