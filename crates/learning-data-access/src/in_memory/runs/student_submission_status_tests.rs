...
use crate::{
    ...
    JobLeaseDuration, StoreError, StudentSubmissionStatusRead, StudentSubmissionStatusStore,
    StudentWorkRoutingBinding, WorkerId, canonical_attempt_result_json,
};
use question_model::{
...
fn binding() -> StudentWorkRoutingBinding {
...
fn accepts_status_capability(_: &dyn StudentSubmissionStatusStore) {}

#[test]
fn student_submission_status_capability_is_object_safe() {
    accepts_status_capability(&MemoryStore::default());
}

#[tokio::test]
async fn student_submission_status_binds_the_exact_route_and_actor() {
    ...
    assert!(matches!(
        store
            .student_submission_status(context, actor, binding(), attempt)
            .await,
        Ok(StudentSubmissionStatusRead::AcceptedPending(_))
    ));
    for wrong_binding in [
        ...
        assert_eq!(
            store
                .student_submission_status(context, actor, wrong_binding, attempt)
                .await,
...
#[tokio::test]
async fn student_submission_status_projects_completed_attention_and_closed_contradictions() {
    let store = MemoryStore::default();
    let (tenant, actor, attempt, _) = seed_complete_issued_execution(&store);
    let context = TenantContext::from_authenticated_session(tenant);
    {
    ...
    assert!(matches!(
        store
            .student_submission_status(context, actor, binding(), attempt)
            .await,
        Ok(StudentSubmissionStatusRead::InstructorAttention(_))
    ));
    {
        let mut state = store
            .write_state()
