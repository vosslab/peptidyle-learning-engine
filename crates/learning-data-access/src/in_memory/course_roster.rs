...
use super::{MemoryStore, State, page_records};
use crate::{
    ActorContext, ClaimCourseInvitation, ClaimedCourseMembership, CommitCourseRosterImport,
    ...
    SessionTokenHash, StageCourseRosterImport, StoreError, UpsertCourseMember,
};
...
impl CourseRosterStore for MemoryStore {
    async fn list_course_roster(
        ...
        require_roster_actor_authority(
            &mut state,
            actor,
            course,
        ...
        for ((profile_course, membership), profile) in &state.roster_profiles {
            if *profile_course == course {
                records.push((
        ...
        records.extend(
            state
                .course_invitations
                .iter()
                .filter(|((record_course, _), _)| *record_course == course)
                .filter(|(_, stored)| {
                    public_invitation(&state, stored).status == CourseInvitationStatus::Pending
                })
                .map(|((_, invitation), stored)| {
                    (
        ...
        Ok(CourseRosterPage {
            ...
            policy: roster_policy(&state, course),
        })
    }
    ...
    async fn create_course_invitation(
        ...
        let mut state = self.write_state()?;
        let invited_by = require_roster_actor_authority(
            &mut state,
            actor,
            command.course,
            CourseRosterSupportAction::CreateInvitation,
        )?;
        let policy = roster_policy(&state, command.course);
        ...
        if state.invitation_by_hash.contains_key(&command.token_hash)
            || roster_id_in_use(&state, command.course, &command.roster_id)
            || invitation_email_in_use(&state, command.course, &command.email)
        {
        ...
        state
            .invitation_by_hash
            .insert(command.token_hash, (command.course, invitation.id));
        state
            .invitation_idempotency
            .insert(receipt_key, (invitation.id, command.token_hash));
        state
            .course_invitations
            .insert((command.course, invitation.id), stored);
        create_pending(&mut state, command.course, invitation.id)?;
        bump_roster_revision(&mut state, command.course, None)?;
        Ok(invitation)
    }
    ...
    async fn replace_course_enrollment_policy(
        ...
        state
            .roster_policies
            .insert(command.course, updated.clone());
        Ok(updated)
    }
    ...
    async fn claim_course_invitation(
        ...
        let mut state = self.write_state()?;
        let (course, invitation_id) = state
            .invitation_by_hash
        ...
        if stored.record.expires_at <= state.authoritative_time {
            if let Some(expired) = state.course_invitations.get_mut(&(course, invitation_id)) {
        ...
        if stored.record.status == CourseInvitationStatus::Claimed {
            ...
            return claimed_existing_member(&state, course, command.user);
        }
        ...
        let result = claim_locked(&mut state, course, invitation_id, stored, command);
        if result.is_err() {
            *state = snapshot;
        }
        result
    ...
    async fn upsert_course_member(
        ...
        let mut state = self.write_state()?;
        require_direct_course_instructor_actor(&state, command.course, actor.user_id())?;
        let rollback = state.clone();
        let result = upsert_member_locked(&mut state, command);
        if result.is_err() {
            *state = rollback;
        }
        result
    ...
    async fn revoke_course_member(
        ...
        state
            ...
            .remove(&(command.course, user));
        if let Some(roster_id) = roster_id {
            state
                .roster_member_by_roster_id
                .remove(&(command.course, roster_id));
        }
        for record in state
            .course_groups
            .values_mut()
        ...
        bump_roster_revision(&mut state, command.course, Some(command.expected_revision))
    }
    ...
    async fn revoke_course_invitation(
        ...
        let invitation = state
            .course_invitations
            .get_mut(&(command.course, command.invitation))
            .ok_or(StoreError::NotFound)?;
        ...
        cancel_for_invitation(&mut state, command.course, command.invitation);
        bump_roster_revision(&mut state, command.course, Some(command.expected_revision))
    }
    ...
    async fn stage_course_roster_import(
        ...
        let mut state = self.write_state()?;
        let imported_by = require_roster_actor_authority(
            &mut state,
            actor,
            command.course,
            CourseRosterSupportAction::StageImport,
        )?;
        import::stage(&mut state, imported_by, command)
    }
    ...
    async fn commit_course_roster_import(
        ...
        let rollback = state.clone();
        let result = import::commit(&mut state, imported_by, command);
        if result.is_err() {
            *state = rollback;
        }
        result
...
fn require_direct_course_instructor_actor(
    ...
    match current_course_membership_role(state, course, actor) {
...
fn current_course_membership_role(
    state: &State,
    course: question_model::CourseId,
    actor: question_model::UserId,
    ...
    state
        ...
        .filter(|membership| {
            membership.course == course && membership.status == CourseMemberStatus::Active
        })
        .map(|membership| membership.role)
}

fn require_roster_actor_authority(
    state: &mut State,
    actor: ActorContext,
    course: question_model::CourseId,
    ...
    if current_course_membership_role(state, course, user) == Some(CourseMembershipRole::Instructor)
    {
        return Ok(user);
    }
    if actor.role() != UserRole::Sysadmin {
        return match current_course_membership_role(state, course, user) {
            Some(CourseMembershipRole::Student) => Err(StoreError::Forbidden),
            None => Err(StoreError::NotFound),
...
fn claim_locked(
    ...
    if let Some(existing_member) = state
        .roster_member_by_roster_id
        .get(&(course, stored.record.roster_id.clone()))
        .copied()
        && state
            .course_memberships
            .get(&existing_member)
    ...
    let student = student_identity(state, command.user)?;
    let membership =
        super::entitlement::ensure_course_membership_id(state, course, command.user, student)?;
    let canonical = state
        .course_memberships
        .get_mut(&membership)
    ...
    state
        .roster_member_by_roster_id
        .insert((course, stored.record.roster_id.clone()), membership);
    state
        .roster_profiles
        .insert((course, membership), profile.clone());
    ...
    cancel_for_invitation(state, course, invitation_id);
    let roster_revision = bump_roster_revision(state, course, None)?;
...
fn upsert_member_locked(
    ...
    if let Some(existing_membership) = state
        .active_course_membership_by_user
        .get(&(command.course, command.user))
        .copied()
        && let Some(profile) = state
            .roster_profiles
            .get(&(command.course, existing_membership))
    {
        let existing = public_member(state, profile)?;
        return Ok(ClaimedCourseMembership {
            course: command.course,
            member: existing,
            roster_revision: roster_policy(state, command.course).revision,
        });
    }
    ...
    if let Some(roster_id) = roster_id
        && let Some(existing) = state
            .roster_member_by_roster_id
            .insert((command.course, roster_id), membership)
        && existing != membership
    {
        return Err(StoreError::Conflict);
    }
    state
        .roster_profiles
        .insert((command.course, membership), profile.clone());
    let member = public_member(state, &profile)?;
    let roster_revision = bump_roster_revision(state, command.course, None)?;
    Ok(ClaimedCourseMembership {
        course: command.course,
        member,
        roster_revision,
...
fn student_identity(
    ...
    if let Some(student) = state.student_by_user.get(&user).copied() {
        return Ok(student);
    }
    ...
    state.student_by_user.insert(user, student);
    Ok(student)
}
...
pub(super) fn require_roster_authority(
    ...
    let is_sysadmin = subject.role() == UserRole::Sysadmin;
    state.courses.get(&course).ok_or(StoreError::NotFound)?;
    if super::entitlement::current_course_role(state, course, actor)
        == Some(CourseMembershipRole::Instructor)
    {
        return Ok(actor);
    }
    if !is_sysadmin {
        return match super::entitlement::current_course_role(state, course, actor) {
            Some(CourseMembershipRole::Student) => Err(StoreError::Forbidden),
            None => Err(StoreError::NotFound),
            Some(CourseMembershipRole::Instructor) => {
                unreachable!("direct Instructor returned above")
...
pub(super) fn require_roster_read_authority(
    ...
    let subject = active_subject(state, context, session).ok_or(StoreError::NotFound)?;
    state.courses.get(&course).ok_or(StoreError::NotFound)?;
    if super::entitlement::current_course_role(state, course, subject.user())
        == Some(CourseMembershipRole::Instructor)
        || subject.role() == UserRole::Sysadmin
    {
        Ok(())
    } else if super::entitlement::current_course_role(state, course, subject.user()).is_some() {
...
fn roster_id_in_use(
    ...
    state
        .roster_member_by_roster_id
        .contains_key(&(course, roster_id.clone()))
...
fn claimed_existing_member(
    ...
    Ok(ClaimedCourseMembership {
        course,
        member,
        roster_revision: roster_policy(state, course).revision,
    })
}
