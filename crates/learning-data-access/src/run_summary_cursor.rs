...
const VERSION: u8 = 1;
const PAYLOAD_LENGTH: usize = 53;
const RAW_LENGTH: usize = PAYLOAD_LENGTH + 32;
const ENCODED_LENGTH: usize = 114;
...
impl RunSummaryCursor {
    /// Binds the cursor to its tenant and exact run before adding an integrity
    /// digest. A continuation copied to another run is rejected before either
    /// store queries outcomes.
    pub(crate) fn encode(self, tenant: Uuid, run: Uuid) -> Cursor {
        let mut payload = [0_u8; PAYLOAD_LENGTH];
        payload[0] = VERSION;
        payload[1..17].copy_from_slice(tenant.as_bytes());
        payload[17..33].copy_from_slice(run.as_bytes());
        payload[33..37].copy_from_slice(&self.assignment_position.to_be_bytes());
        payload[37..].copy_from_slice(self.attempt.as_bytes());
    ...
    pub(crate) fn decode(cursor: &Cursor, tenant: Uuid, run: Uuid) -> Result<Self, StoreError> {
        ...
        if bytes.len() != RAW_LENGTH
            || bytes[0] != VERSION
            || bytes[1..17] != *tenant.as_bytes()
            || bytes[17..33] != *run.as_bytes()
        ...
        let assignment_position =
            u32::from_be_bytes(bytes[33..37].try_into().map_err(|_| invalid_cursor())?);
        let attempt = Uuid::from_slice(&bytes[37..53]).map_err(|_| invalid_cursor())?;
        let decoded = Self {
            assignment_position,
            attempt,
        };
        if decoded.encode(tenant, run).as_str() != token {
            return Err(invalid_cursor());
        }
        Ok(decoded)
    }
...
mod tests {
    ...
    fn cursor_round_trips_the_exact_run_bound_keyset_tuple() {
        ...
        let token = original.encode(TENANT, RUN);
        assert_eq!(token.as_str().len(), ENCODED_LENGTH);
        assert_eq!(RunSummaryCursor::decode(&token, TENANT, RUN), Ok(original));
    }
    ...
    fn tampered_or_cross_run_cursor_never_becomes_a_query_key() {
        ...
        for candidate in [tampered, token.clone()] {
            assert!(matches!(
                RunSummaryCursor::decode(&candidate, TENANT, Uuid::from_u128(3)),
        ...
        assert!(matches!(
            RunSummaryCursor::decode(&malformed, TENANT, RUN),
            Err(StoreError::InvalidRecord(message)) if message == "invalid run summary cursor"
        ));
    }
    ...
    fn fifty_one_keyset_rows_continue_without_duplicates() {
        let keys = (0_u32..51)
            ...
            .collect::<Vec<_>>();
        let first = &keys[..50];
        let after = RunSummaryCursor::decode(
            &first.last().expect("first page").encode(TENANT, RUN),
            TENANT,
            RUN,
        )
