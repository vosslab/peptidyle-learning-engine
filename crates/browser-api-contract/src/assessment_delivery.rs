...
///
/// `question_revision_tuple` is the exact immutable revision retained by the Issued
...
mod tests {
    ...
    fn selected_presentation_keeps_the_exact_revision_for_asset_urls() {
        ...
        assert_eq!(
            wire["questionRevisionTuple"]["questionId"],
            question_id.to_string()
        );
        assert_eq!(wire["questionRevisionTuple"]["revisionNumber"], 3);
    }
}
