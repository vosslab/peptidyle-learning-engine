...
/// Summary printed by the xtask command.
pub struct Report {
...
pub fn run(fixture_dir: &Path, typescript_path: &Path, mode: Mode) -> Result<Report> {
    ...
    let typescript = generated_typescript(&corpus)?;
    write_artifact(typescript_path, typescript.as_bytes())?;
...
fn generated_typescript(corpus: &FixtureCorpus) -> Result<String> {
    let corpus_json =
        serde_json::to_string_pretty(corpus).context("serializing TypeScript data")?;
    let asset_bodies = BTreeMap::from([
        (corpus.assets[0].id.to_string(), PEPTIDE_BOND_SVG),
        (corpus.assets[1].id.to_string(), PEPTIDE_PLANE_SVG),
    ]);
    let bodies_json =
        serde_json::to_string_pretty(&asset_bodies).context("serializing fixture assets")?;

    Ok(format!(
        "// Generated by `cargo run -p xtask -- fixtures`; do not edit.\n\n\
import type {{ MockFixtureCorpus }} from \"../../src/api/mock/fixture_contract\";\n\n\
export const publishedProblemFixture = {corpus_json} satisfies MockFixtureCorpus;\n\n\
export const publishedProblemAssetBodies: Readonly<Record<string, string>> = {bodies_json};\n"
    ))
}
...
fn check_artifact(path: &Path, expected: &[u8]) -> Result<()> {
    let actual = fs::read(path).with_context(|| {
        format!(
            "reading {}; refresh with `cargo run -p xtask -- fixtures --write`",
            path.display()
        )
    })?;
    if actual != expected {
        bail!(
            "fixture drift in {}; refresh deliberately with `cargo run -p xtask -- fixtures --write`",
            path.display()
        );
    }
    Ok(())
