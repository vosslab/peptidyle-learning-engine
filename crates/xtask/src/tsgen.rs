...
fn join_union(members: &[String]) -> String {
    let single_line = members.join(" | ");
    // "export type " + name + " = " + body + ";" must fit; 24 is a safe
    // allowance for the longest type name in the model plus the fixed text.
    let multiline_needed = single_line.len() + 24 > PRINT_WIDTH || single_line.contains('\n');
    if !multiline_needed {
        return single_line;
    }
...
fn render(type_definition: &Generated) -> String {
    let mut out = String::new();
    out.push_str("// Generated by crates/xtask from the Rust question model.\n");
    out.push_str("// Do not edit by hand; run ./build.sh (or cargo run -p xtask -- tsgen).\n");
