...
use store::{
    ...
    IssueQuestionAttemptCommand, JobLeaseDuration, JobPayload, JobStore, PageRequest, PageSize,
    PublishDraftCommand, PutCourseGroupCommand, SetAssignmentPolicyExceptionCommand, Store,
    StoreError, SubmissionIdempotencyKey, SubmitQuestionAttemptCommand, TenantContext,
    UpdateAssignmentTimingCommand,
};
use uuid::Uuid;

const USAGE: &str = "usage: xtask e2e-seed --database-url <URL> --tenant <UUID> (--instructor <UUID>|--user <UUID>) --student <UUID> [--exercise-scoring] [--exercise-timing]";
...
async fn exercise_assignment_timing(
    store: &store::postgres::PostgresStore,
    ...
    let due = store
        .claim_next_job(JobLeaseDuration::from_seconds(30)?)
        .await
...
async fn exercise_scoring_generation(
    ...
    let claimed = store
        .claim_next_job(JobLeaseDuration::from_seconds(60)?)
        .await
