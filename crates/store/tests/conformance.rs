...
#[cfg(feature = "postgres")]
use store::{
    InstitutionRetentionPolicy, RetentionDays, RetentionDispatchBatch, RetentionScheduleStore,
    RetentionStore,
};
use uuid::Uuid;
...
#[test]
fn worker_queue_migration_keeps_the_broker_narrow_and_usable() {
    let migration = include_str!("../../../schemas/migrations/20260808000200_worker_jobs.sql");
    for function in [
        "ple_claim_worker_job(uuid, integer)",
        "ple_complete_worker_job(uuid, uuid)",
        "ple_fail_worker_job(uuid, uuid, text)",
        "ple_ready_worker_queue_depth()",
    ] {
        assert!(
            migration.contains(&format!(
                "ALTER FUNCTION {function} OWNER TO ple_queue_broker;"
            )),
            "queue broker must own {function}"
        );
    }
    for required in [
        "SECURITY DEFINER",
        ...
        assert!(
            ...
            "missing queue boundary: {required}"
        );
    }
}

#[test]
fn student_export_migration_keeps_artifacts_tenant_scoped_and_delivery_private() {
    let migration = include_str!("../../../schemas/migrations/20260808001900_student_exports.sql");

    for table in ["student_export_request", "student_export_artifact"] {
        assert!(
            migration.contains(&format!("ALTER TABLE {table} FORCE ROW LEVEL SECURITY;")),
            "student export table must force RLS even for its owner: {table}"
        );
    }
    assert!(
        migration.contains(
            "FROM student_export_request r WHERE r.export_id = student_export_artifact.export_id\n                  AND r.tenant_id = ple_current_tenant()"
        ),
        "artifact RLS must use a qualified outer artifact reference before following the request tenant"
    );

    for function in [
        "ple_mark_failed_export_job()",
        "ple_commit_export_job(uuid, uuid, uuid, uuid, jsonb)",
    ] {
        assert!(
            migration.contains(&format!("REVOKE ALL ON FUNCTION {function} FROM PUBLIC;")),
            "export broker function must not be executable through PUBLIC: {function}"
        );
    }
    ...
    assert!(
        !migration.contains("GRANT EXECUTE ON FUNCTION ple_mark_failed_export_job()"),
        "the trigger-only failure transition must not have a callable SQL grant"
    );
    ...
    for token in [
        ...
        assert!(
            ...
            "the export commit must retain the frozen-requester delivery ACL contract token: {token}"
        );
    }
}

#[test]
fn qti_grader_uses_a_dedicated_least_privilege_connection() {
    let postgres = include_str!("../src/postgres.rs");
    let migration =
    ...
    assert!(postgres.contains("pub async fn connect(database_url: &str)"));
    assert!(!postgres.contains("PostgresQtiGraderStore::new("));
    assert!(!postgres.contains("SET LOCAL ROLE ple_grader"));
    assert!(migration.contains("CREATE ROLE ple_qti_grader LOGIN"));
    assert!(migration.contains("REVOKE ple_app FROM ple_qti_grader;"));
    assert!(migration.contains("GRANT INSERT ON workspace_qti_import_grading TO ple_app;"));
    assert!(staging.contains("REVOKE SELECT ON workspace_qti_import_grading FROM ple_qti_grader;"));
    assert!(staging.contains("ple_read_committed_qti_grading"));
    assert!(staging.contains("TO ple_qti_grader;"));
    for child_policy in [
        "workspace_qti_import_item_app_prepared_insert",
        "workspace_qti_import_asset_app_prepared_insert",
        "workspace_qti_import_unsupported_app_prepared_insert",
        "workspace_qti_import_grading_app_prepared_insert",
    ] {
        assert!(staging.contains(child_policy));
    }
    assert!(staging.contains("ple_qti_import_is_prepared(tenant_id, workspace_id, import_id)"));
    assert!(staging.contains("p_tenant = public.ple_current_tenant() AND EXISTS"));
    assert!(published.contains("CREATE TABLE published_qti_grading"));
    assert!(published.contains("CREATE FUNCTION ple_promote_qti_grading"));
    assert!(published.contains("SECURITY DEFINER"));
    assert!(published.contains("CREATE FUNCTION ple_read_published_qti_grading"));
    assert!(published.contains("TO ple_qti_grader;"));
    assert!(published.contains("version_row.publication_scope = 'public'"));
    assert!(published.contains("grant_row.tenant_id = p_tenant"));
    assert!(!published.contains("TO ple_app;\nGRANT SELECT ON published_qti_grading"));
}

#[test]
fn workspace_draft_privileges_keep_full_authoring_payload_from_non_app_roles() {
    let migration =
    ...
    assert!(
        migration.contains("GRANT SELECT, INSERT, UPDATE, DELETE ON workspace_draft TO ple_app;")
    );
    assert!(
        !migration.contains("GRANT SELECT ON workspace_draft TO ple_student"),
        "student role must not regain direct draft-payload access"
    );
}

#[test]
fn external_tool_broker_migration_keeps_secrets_private_and_rows_tenant_scoped() {
    let migration = include_str!("../../../schemas/migrations/20260808000400_imathas_broker.sql");
    let verifier = include_str!(
    ...
    for required in [
        ...
        "CREATE POLICY external_tool_exchange_tenant",
        "CREATE POLICY external_tool_launch_session_tenant",
        ...
        assert!(
            migration.contains(required),
            "missing broker storage boundary: {required}"
        );
    }
    for forbidden in [
        "\n    provider_url",
        ...
        assert!(
            ...
            "broker schema must not persist browser-readable provider credentials or URLs: {forbidden}"
        );
    }
    assert!(
        verifier.contains("verification_token_sha256 bytea"),
        "a staged verdict must retain only a verifier-lease digest"
    );
}

#[cfg(feature = "postgres")]
#[test]
fn postgres_migration_registry_keeps_every_broker_migration_in_forward_order() {
    let registry = include_str!("../src/postgres.rs");
    let broker = registry
        .find("20260808000400_imathas_broker.sql")
        .expect("broker migration must be registered");
    let verifier = registry
        .find("20260808000500_imathas_broker_verification_token.sql")
        .expect("verifier-token migration must be registered");
    let qti = registry
        .find("20260808000600_workspace_qti_import.sql")
        .expect("later migration must remain registered");
    let workspace_privileges = registry
        .find("20260808001300_workspace_draft_privileges.sql")
        .expect("workspace draft privilege migration must be registered");
    let published_qti = registry
        .find("20260808001600_published_qti_grading.sql")
        .expect("published QTI grading migration must be registered");
    assert!(
        broker < verifier
            && verifier < qti
            && qti < workspace_privileges
            && workspace_privileges < published_qti,
        "embedded migrations must be complete and forward ordered"
    );
}

#[cfg(feature = "postgres")]
async fn exercise_postgres_queue_privileges_and_payload_backstops(pool: &sqlx::PgPool) {
    let broker_can_operate: bool = sqlx::query_scalar(
        "SELECT has_table_privilege('ple_queue_broker', 'public.worker_job', 'SELECT, UPDATE')",
    )
    .fetch_one(pool)
    .await
    .expect("queue broker table privileges should be inspectable");
    assert!(
        broker_can_operate,
        "the SECURITY DEFINER owner must have only the privileges its broker SQL needs"
    );
    let app_can_update: bool =
        sqlx::query_scalar("SELECT has_table_privilege('ple_app', 'public.worker_job', 'UPDATE')")
            .fetch_one(pool)
            .await
            .expect("app table privileges should be inspectable");
    assert!(
        !app_can_update,
        "runtime application code must not mutate queue leases directly"
    );
    let app_can_claim: bool = sqlx::query_scalar(
        "SELECT has_function_privilege(\
         'ple_app', 'ple_claim_worker_job(uuid, integer)', 'EXECUTE')",
    )
    .fetch_one(pool)
    .await
    .expect("broker function privilege should be inspectable");
    assert!(
        app_can_claim,
        "ple_app reaches cross-tenant claims only through the broker"
    );

    let tenant = TenantId::from_uuid(uuid(9_200));
    let valid_reference = serde_json::json!({
        "problem": "00000000-0000-0000-0000-000000009201",
        "version": "00000000-0000-0000-0000-000000009202"
    });
    let hostile_payloads = [
        serde_json::json!({
            "kind": "render", "reference": valid_reference.clone(), "seed": 1,
        ...
        serde_json::json!({
            "kind": "render", "reference": valid_reference, "seed": "1"
        }),
        serde_json::json!({
            "kind": "import", "sourceObject": "00000000-0000-0000-0000-000000009203",
            "rawBytes": "must-not-persist"
        }),
    ];
    for (offset, payload) in hostile_payloads.into_iter().enumerate() {
        let mut transaction = pool.begin().await.expect("hostile insert transaction");
        sqlx::query("SET LOCAL ROLE ple_app")
            .execute(&mut *transaction)
            .await
            .expect("migration login should assume ple_app");
        ...
        .bind(uuid(
            9_210 + u128::try_from(offset).expect("fixture offset"),
        ))
        .bind(tenant.as_uuid())
        .bind(payload)
        .execute(&mut *transaction)
        .await;
        let error = result.expect_err("hostile durable queue JSON must fail its SQL CHECK");
        let sqlx::Error::Database(database_error) = error else {
            panic!("expected PostgreSQL CHECK failure, received {error}");
        };
        assert_eq!(database_error.code().as_deref(), Some("23514"));
        transaction
            .rollback()
            .await
            .expect("hostile insert rollback");
    }
}
...
#[cfg(feature = "postgres")]
async fn exercise_postgres_statistics_broker_integrity(pool: &sqlx::PgPool) {
    let tenant = uuid(88_001);
    let enrollment = uuid(88_002);
    let assignment = uuid(88_003);
    let first_run = uuid(88_004);
    let later_run = uuid(88_005);
    let target_problem = uuid(88_006);
    let target_version = uuid(88_007);
    let other_problem = uuid(88_008);
    let other_version = uuid(88_009);
    let target_attempt = uuid(88_010);
    let trigger_attempt = uuid(88_011);
    let later_target_attempt = uuid(88_012);
    let later_trigger_attempt = uuid(88_013);
    let practice_enrollment = uuid(88_019);
    let practice_run = uuid(88_020);
    let practice_attempt = uuid(88_021);
    let digest = "0".repeat(64);

    for (problem, version, workspace) in [
        (target_problem, target_version, uuid(88_014)),
        (other_problem, other_version, uuid(88_015)),
    ] {
        sqlx::query("INSERT INTO problem (problem_id) VALUES ($1)")
            .bind(problem)
            .execute(pool)
            .await
            .expect("statistics fixture problem inserts");
        sqlx::query(
            "INSERT INTO problem_version (problem_id, version_id, workspace_id, title, authors) \
             VALUES ($1, $2, $3, 'statistics fixture', $4)",
        )
        .bind(problem)
        .bind(version)
        .bind(workspace)
        .bind(serde_json::json!([uuid(88_016).to_string()]))
        .execute(pool)
        .await
        .expect("statistics fixture version inserts");
        sqlx::query(
        ...
        .execute(pool)
        .await
        .expect("statistics fixture catalog grant inserts");
    }
    sqlx::query(
        "INSERT INTO assignment (tenant_id, assignment_id, payload, payload_sha256) \
         VALUES ($1, $2, '{}'::jsonb, $3)",
    )
    .bind(tenant)
    .bind(assignment)
    ...
    .execute(pool)
    .await
    .expect("statistics fixture assignment inserts");
    sqlx::query(
        "INSERT INTO enrollment (tenant_id, enrollment_id, assignment_id, student_id, payload, payload_sha256, user_id) \
    ...
    .execute(pool)
    .await
    .expect("statistics fixture enrollment inserts");
    sqlx::query(
        "INSERT INTO enrollment (tenant_id, enrollment_id, assignment_id, student_id, payload, payload_sha256, user_id) \
         VALUES ($1, $2, $3, $4, '{}'::jsonb, $5, $6)",
    )
    .bind(tenant)
    .bind(practice_enrollment)
    .bind(assignment)
    .bind(uuid(88_022))
    .bind(&digest)
    .bind(uuid(88_023))
    .execute(pool)
    .await
    .expect("statistics practice enrollment inserts");
    for (run, run_enrollment, number, completed_at, mode) in [
        (
            first_run,
            enrollment,
            1_i64,
        ...
        sqlx::query(
            "INSERT INTO assignment_run (tenant_id, run_id, enrollment_id, run_number, started_at, completed_at, payload, payload_sha256) \
             VALUES ($1, $2, $3, $4, $5::timestamptz, $5::timestamptz, $6, $7)",
        )
        .bind(tenant)
        .bind(run)
        .bind(run_enrollment)
        ...
        .execute(pool)
        .await
        .expect("statistics fixture completed run inserts");
    }
    for (attempt, run, problem, version) in [
        (target_attempt, first_run, target_problem, target_version),
        (trigger_attempt, first_run, other_problem, other_version),
        (
            later_target_attempt,
            later_run,
            target_problem,
            target_version,
        ),
        (
            later_trigger_attempt,
            later_run,
            other_problem,
            other_version,
        ),
        ...
        let payload = serde_json::json!({
            ...
            "questionVersion": version.to_string(),
        });
        sqlx::query(
            "INSERT INTO question_attempt (tenant_id, attempt_id, run_id, occurred_at, assignment_position, payload, payload_sha256) \
             VALUES ($1, $2, $3, transaction_timestamp(), 0, $4, $5)",
        )
        .bind(tenant)
        .bind(attempt)
        .bind(run)
        ...
        .execute(pool)
        .await
        .expect("statistics fixture attempt inserts");
        sqlx::query(
            "INSERT INTO submission_idempotency (tenant_id, attempt_id, idempotency_key, response_sha256, payload, payload_sha256) \
             VALUES ($1, $2, 'statistics-fixture', $3, $4, $5)",
        )
        .bind(tenant)
        .bind(attempt)
        .bind(&digest)
        ...
        .execute(pool)
        .await
        .expect("statistics fixture submission inserts");
    }

    let mut transaction = pool.begin().await.expect("statistics app transaction");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *transaction)
        .await
        .expect("statistics app role");
    sqlx::query("SELECT set_config('ple.tenant_id', $1, true)")
        .bind(tenant.to_string())
        .execute(&mut *transaction)
        .await
        .expect("statistics tenant context");
    let rejected = sqlx::query_scalar::<_, bool>(
        "SELECT ple_record_question_statistics($1, $2, $3, $4, $5, $6, 0.5, 1, 30, NULL, $7)",
    )
    .bind(tenant)
    .bind(enrollment)
    .bind(later_run)
    .bind(later_trigger_attempt)
    .bind(target_problem)
    .bind(target_version)
    .bind(vec![1_u8; 32])
    .fetch_one(&mut *transaction)
    .await;
    assert!(rejected.is_err(), "a later completed run must be rejected");
    transaction
        .rollback()
        .await
        .expect("rejected broker rolls back");
    ...
    assert!(
        rejected.is_err(),
        "a completed practice run must be rejected"
    );
    transaction
        .rollback()
        .await
        .expect("rejected practice broker rolls back");
    let receipts: i64 =
        sqlx::query_scalar("SELECT count(*) FROM question_statistics_contribution_receipt")
            .fetch_one(pool)
            .await
            .expect("receipt count after rejected broker call");
    let aggregates: i64 = sqlx::query_scalar("SELECT count(*) FROM question_statistics_aggregate")
        .fetch_one(pool)
        .await
        .expect("aggregate count after rejected broker call");
    assert_eq!((receipts, aggregates), (0, 0));
    ...
    .bind(enrollment)
    .bind(first_run)
    .bind(trigger_attempt)
    .bind(target_problem)
    .bind(target_version)
    .bind(vec![2_u8; 32])
    .fetch_one(&mut *transaction)
    .await
    .expect("one trigger may aggregate another version from its first completed run");
    assert!(recorded);
    let second_recorded: bool = sqlx::query_scalar(
        "SELECT ple_record_question_statistics($1, $2, $3, $4, $5, $6, 0.5, 1, 30, NULL, $7)",
    )
    .bind(tenant)
    .bind(enrollment)
    ...
    .fetch_one(&mut *transaction)
    .await
    .expect("one trigger may aggregate each distinct version in its first completed run");
    assert!(second_recorded);
    transaction
        .commit()
        .await
        .expect("statistics broker commits");
    let receipts: i64 =
        sqlx::query_scalar("SELECT count(*) FROM question_statistics_contribution_receipt")
            .fetch_one(pool)
            .await
            .expect("two version receipts persist");
    assert_eq!(receipts, 2);
}

#[cfg(feature = "postgres")]
#[tokio::test]
#[ignore = "requires PLE_POSTGRES_TEST_URL with migration-role privileges and a dedicated empty database"]
async fn postgres_retention_broker_enforces_roles_and_exact_course_end_replay() {
    ...
    let pool = lazy_pool(&database_url).expect("valid PostgreSQL test URL");
    apply_migrations(&pool).await.expect("migrations apply");
    let store = PostgresStore::new(pool.clone());
    let tenant = TenantId::from_uuid(Uuid::from_u128(990_001));
    let context = TenantContext::from_authenticated_session(tenant);
    let instructor = UserId::from_uuid(Uuid::from_u128(990_002));
    let student = UserId::from_uuid(Uuid::from_u128(990_003));
    let administrator = UserId::from_uuid(Uuid::from_u128(990_004));
    let outsider = UserId::from_uuid(Uuid::from_u128(990_005));
    let course = CourseId::from_uuid(Uuid::from_u128(990_006));
    let concurrent_course = CourseId::from_uuid(Uuid::from_u128(990_007));
    store
        .upsert_course(
            context,
            CourseRecord {
                id: course,
                tenant,
                title: "Retention live fixture".to_string(),
                members: vec![
                    CourseMembership {
                        user: instructor,
                        role: CourseMembershipRole::Instructor,
    ...
    store
        .upsert_course(
            context,
            CourseRecord {
                id: concurrent_course,
                tenant,
                title: "Retention concurrent fixture".to_string(),
                members: vec![CourseMembership {
                    user: instructor,
                    role: CourseMembershipRole::Instructor,
                }],
        ...
        .await
        .expect("concurrent course");
    for (byte, user, roles) in [
        (1_u8, instructor, vec![UserRole::Instructor]),
        (2, student, vec![UserRole::Student]),
        (3, administrator, vec![UserRole::Administrator]),
        (4, outsider, vec![UserRole::Instructor]),
    ] {
        store
            .create_session(
                SessionTokenHash::compute(&[byte; 32]),
                SessionSubject::new(tenant, user, "Retention live", roles).unwrap(),
                SessionLifetime::from_seconds(3600).unwrap(),
            )
            .await
            .expect("session");
    }
    ...
    .unwrap();
    store
        .configure_retention_policy(context, SessionTokenHash::compute(&[3; 32]), policy)
        .await
        .expect("admin config");
    assert!(matches!(
        store
            .configure_retention_policy(context, SessionTokenHash::compute(&[1; 32]), policy)
            .await,
        Err(StoreError::Forbidden)
    ));
    let first = store
        .end_course_retention(context, SessionTokenHash::compute(&[1; 32]), course)
        .await
        .expect("instructor end");
    let second = store
        .end_course_retention(context, SessionTokenHash::compute(&[1; 32]), course)
        .await
        .expect("replay end");
    ...
    let instructor_session = SessionTokenHash::compute(&[1; 32]);
    let (left, right) = tokio::join!(
        store.end_course_retention(context, instructor_session, concurrent_course),
        replica.end_course_retention(context, instructor_session, concurrent_course)
    );
    assert_eq!(
        left.expect("first concurrent end"),
        right.expect("second concurrent end")
    );
    let current_stages: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM course_retention_stage \
         WHERE tenant_id = $1 AND course_id = $2 AND generation = 1",
    )
    .bind(tenant.as_uuid())
    .bind(concurrent_course.as_uuid())
    .fetch_one(&pool)
    .await
    .expect("current generation stages");
    assert_eq!(current_stages, 3);
    assert_eq!(
        store
            .course_retention(context, SessionTokenHash::compute(&[2; 32]), course)
            .await
            .expect("student nonenumeration"),
        None
    );
    let foreign_tenant = TenantId::from_uuid(Uuid::from_u128(990_008));
    let foreign_context = TenantContext::from_authenticated_session(foreign_tenant);
    let foreign_session = SessionTokenHash::compute(&[5; 32]);
    store
        .create_session(
            foreign_session,
            SessionSubject::new(
    ...
    assert_eq!(
        store
            .course_retention(foreign_context, foreign_session, course)
            .await
            .expect("foreign nonenumeration"),
        None
    );
    assert_eq!(
        store
            .course_retention(context, SessionTokenHash::compute(&[4; 32]), course)
            .await
            .expect("nonmember nonenumeration"),
        None
    );
    let mut app = pool.begin().await.expect("app transaction");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *app)
        .await
        .expect("app role");
    sqlx::query("SELECT * FROM course_retention")
        .fetch_all(&mut *app)
        .await
        .expect_err("raw retention read denied");
    app.rollback()
        .await
        .expect("close denied retention read transaction");
    let mut app = pool.begin().await.expect("app auth-read transaction");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *app)
        .await
        .expect("app role");
    sqlx::query("SELECT * FROM auth_session")
        .fetch_all(&mut *app)
        .await
        .expect_err("raw auth read denied");
    app.rollback()
        .await
        .expect("close denied auth read transaction");
    let mut app = pool.begin().await.expect("app insert transaction");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *app)
        .await
        .expect("app role");
    sqlx::query(
        "INSERT INTO institution_retention_policy \
         (tenant_id, notify_days, archive_days, delete_days, assignment_disposition) \
         VALUES ($1, 30, 100, 365, 'retain')",
    )
    .bind(tenant.as_uuid())
    .execute(&mut *app)
    .await
    .expect_err("raw retention insert denied");
    app.rollback()
        .await
        .expect("close denied insert transaction");
    let mut app = pool.begin().await.expect("app update transaction");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *app)
        .await
        .expect("app role");
    sqlx::query("UPDATE course_retention SET lifecycle = 'deleted' WHERE tenant_id = $1")
        .bind(tenant.as_uuid())
        .execute(&mut *app)
        .await
        .expect_err("raw retention update denied");
    app.rollback()
        .await
        .expect("close denied update transaction");
}

#[cfg(feature = "postgres")]
#[tokio::test]
#[ignore = "requires PLE_POSTGRES_TEST_URL with migration-role privileges and a dedicated empty database"]
async fn postgres_retention_r3_broker_rls_prepare_commit_smoke() {
    // This is intentionally a dedicated empty-DB gate. It proves the broker
    // function/RLS capability boundary without weakening production SQL; the
    // broader lifecycle fixture above owns course-end replay setup.
    let database_url = std::env::var("PLE_POSTGRES_TEST_URL")
        .expect("set PLE_POSTGRES_TEST_URL to a dedicated empty PostgreSQL database");
    let pool = lazy_pool(&database_url).expect("valid PostgreSQL test URL");
    apply_migrations(&pool).await.expect("migrations apply");
    let tenant = TenantId::from_uuid(Uuid::from_u128(992_001));
    let mut app = pool.begin().await.expect("app transaction");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *app)
        .await
    ...
    let denied = sqlx::query(
        "INSERT INTO course_retention_notification (tenant_id, course_id, generation, intent) \
         VALUES ($1, $2, 1, 'archive')",
    )
    .bind(tenant.as_uuid())
    .bind(Uuid::from_u128(992_002))
    .execute(&mut *app)
    .await;
    assert!(
        denied.is_err(),
        "ple_app cannot forge a retention notification"
    );
    app.rollback().await.expect("close denied transaction");

    let mut broker = pool.begin().await.expect("broker transaction");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *broker)
        .await
        .expect("app capability caller");
    sqlx::query("SELECT set_config('ple.tenant_id', $1, true)")
        .bind(tenant.to_string())
        .execute(&mut *broker)
        .await
        .expect("tenant context");
    let prepared: Option<serde_json::Value> =
        sqlx::query_scalar("SELECT ple_prepare_retention_work($1, $2, $3, $4, 'notify', 1)")
            .bind(tenant.as_uuid())
            .bind(Uuid::from_u128(992_003))
            .bind(Uuid::from_u128(992_004))
            .bind(Uuid::from_u128(992_002))
            .fetch_one(&mut *broker)
            .await
            .expect("invalid capability is a non-mutating prepare refusal");
    assert_eq!(prepared, None);
    broker.commit().await.expect("broker smoke commit");
}

#[cfg(feature = "postgres")]
#[tokio::test]
#[ignore = "requires PLE_POSTGRES_TEST_URL with migration-role privileges and a dedicated empty database"]
async fn postgres_retention_r3_archive_cleanup_is_lease_fenced_and_rls_owned() {
    let database_url = std::env::var("PLE_POSTGRES_TEST_URL")
        .expect("set PLE_POSTGRES_TEST_URL to a dedicated empty PostgreSQL database");
    let pool = lazy_pool(&database_url).expect("valid PostgreSQL test URL");
    apply_migrations(&pool).await.expect("migrations apply");

    let tenant = Uuid::from_u128(993_001);
    let foreign_tenant = Uuid::from_u128(993_002);
    let course = Uuid::from_u128(993_003);
    let retention_job = Uuid::from_u128(993_004);
    let lease = Uuid::from_u128(993_005);
    let queued_export = Uuid::from_u128(993_006);
    let queued_export_job = Uuid::from_u128(993_007);
    let ready_export = Uuid::from_u128(993_008);
    let ready_export_job = Uuid::from_u128(993_009);
    let manifest = Uuid::from_u128(993_010);
    let ready_manifest = Uuid::from_u128(993_011);
    let expected = [
        Uuid::from_u128(993_012),
        Uuid::from_u128(993_013),
        Uuid::from_u128(993_014),
        Uuid::from_u128(993_015),
    ];
    let ready_object = Uuid::from_u128(993_016);
    let assignment = Uuid::from_u128(993_017);
    let requester = Uuid::from_u128(993_018);
    let digest = "0".repeat(64);

    // Migration-role setup deliberately uses raw rows only for worker-internal
    // states that normal Store APIs cannot manufacture. The SDF calls below
    // run as ple_app with a transaction-local tenant context.
    let mut setup = pool
        .begin()
        .await
        .expect("migration-role setup transaction");
    sqlx::query("INSERT INTO course (tenant_id, course_id, title) VALUES ($1, $2, 'Retention R3')")
        .bind(tenant)
        .bind(course)
        .execute(&mut *setup)
        .await
    ...
    sqlx::query(
        "INSERT INTO course_retention \
         (tenant_id, course_id, ended_at, notify_days, archive_days, delete_days, assignment_disposition, generation, lifecycle) \
         VALUES ($1, $2, transaction_timestamp(), 1, 2, 3, 'retain', 1, 'active')",
    )
    .bind(tenant)
    .bind(course)
    .execute(&mut *setup)
    .await
    .expect("retention snapshot setup");
    sqlx::query(
        "INSERT INTO course_retention_stage \
         (tenant_id, course_id, stage, generation, due_at, state) \
         VALUES ($1, $2, 'archiveStudentRecords', 1, transaction_timestamp() + interval '1 hour', 'scheduled')",
    )
    .bind(tenant)
    .bind(course)
    .execute(&mut *setup)
    .await
    .expect("future archive stage setup");
    let retention_payload = serde_json::json!({
        "kind": "retention", "course": course.to_string(),
        "stage": "archiveStudentRecords", "generation": 1
    });
    for (job, payload, state, token) in [
        (retention_job, retention_payload, "leased", Some(lease)),
        (
            queued_export_job,
            serde_json::json!({"kind":"export", "deliveryObject": manifest.to_string()}),
            "leased",
            Some(Uuid::from_u128(993_019)),
        ),
        (
            ready_export_job,
            serde_json::json!({"kind":"export", "deliveryObject": ready_manifest.to_string()}),
            "completed",
            None,
        ),
    ] {
        sqlx::query(
            "INSERT INTO worker_job \
             (job_id, tenant_id, payload, state, lease_token, lease_expires_at, attempt_count, max_attempts, last_error, completed_at) \
             VALUES ($1, $2, $3, $4, $5, \
                 CASE WHEN $4 = 'leased' THEN transaction_timestamp() + interval '1 hour' ELSE NULL END, \
                 1, 3, CASE WHEN $4 = 'dead' THEN 'permanent' ELSE NULL END, \
                 CASE WHEN $4 = 'completed' THEN transaction_timestamp() ELSE NULL END)",
        )
        .bind(job)
        .bind(tenant)
        .bind(payload)
        ...
        .execute(&mut *setup)
        .await
        .expect("worker-internal job setup");
    }
    for (export, job, manifest_object, state) in [
        (queued_export, queued_export_job, manifest, "queued"),
        (ready_export, ready_export_job, ready_manifest, "ready"),
    ] {
        sqlx::query(
            "INSERT INTO student_export_request \
             (export_id, tenant_id, course_id, assignment_id, requester_id, job_id, manifest_object_id, \
              frozen_payload, frozen_payload_sha256, state, ready_at) \
             VALUES ($1,$2,$3,$4,$5,$6,$7,'{}'::jsonb,decode($8,'hex'),$9, \
                 CASE WHEN $9 = 'ready' THEN transaction_timestamp() ELSE NULL END)",
        )
        .bind(export)
        .bind(tenant)
        .bind(course)
        ...
        .bind(requester)
        .bind(job)
        .bind(manifest_object)
        .bind(&digest)
        .bind(state)
        .execute(&mut *setup)
        .await
        .expect("export request setup");
    }
    for (kind, object) in ["docx", "pdf", "accessibleDocx", "accessiblePdf"]
        .into_iter()
        .zip(expected)
    {
        sqlx::query(
        ...
        .execute(&mut *setup)
        .await
        .expect("four incomplete artifacts");
    }
    let ready_object_payload = serde_json::json!({
        "id": ready_object.to_string(), "bucket": "student-records", "category": "export",
        "key": {"kind":"studentRecord", "tenant":tenant.to_string(), "object":ready_object.to_string()}
    });
    sqlx::query(
        "INSERT INTO student_export_artifact \
         (export_id, kind, object_id, delivery_id, filename, media_type, object_payload) \
         VALUES ($1,'docx',$2,$2,'ready.docx','application/vnd.openxmlformats-officedocument.wordprocessingml.document',$3)",
    )
    .bind(ready_export)
    .bind(ready_object)
    .bind(&ready_object_payload)
    .execute(&mut *setup)
    .await
    .expect("ready artifact setup");
    sqlx::query(
        "INSERT INTO asset_delivery (delivery_id, delivery_kind, tenant_id, object_id, payload, payload_sha256) \
         VALUES ($1,'student_record',$2,$1,'{}'::jsonb,$3)",
    )
    .bind(ready_object)
    .bind(tenant)
    .bind(&digest)
    .execute(&mut *setup)
    .await
    .expect("ready delivery setup");
    setup.commit().await.expect("worker fixture committed");

    async fn retention_sdf(
        pool: &sqlx::PgPool,
        tenant: Uuid,
        job: Uuid,
        token: Uuid,
        ...
        let mut transaction = pool.begin().await.expect("app SDF transaction");
        sqlx::query("SET LOCAL ROLE ple_app")
            .execute(&mut *transaction)
            .await
            .expect("app role");
        ...
        let result = sqlx::query_scalar(
            "SELECT ple_prepare_retention_work($1,$2,$3,$4,'archiveStudentRecords',$5)",
        )
        .bind(tenant)
        .bind(job)
        .bind(token)
        ...
        .fetch_one(&mut *transaction)
        .await
        .expect("SDF prepares or refuses");
        transaction.commit().await.expect("SDF transaction commits");
        result
    }
    async fn retention_commit(
        pool: &sqlx::PgPool,
        tenant: Uuid,
        job: Uuid,
        token: Uuid,
        ...
        let mut transaction = pool.begin().await.expect("app commit transaction");
        sqlx::query("SET LOCAL ROLE ple_app")
            .execute(&mut *transaction)
            .await
            .expect("app role");
        sqlx::query("SELECT set_config('ple.tenant_id', $1, true)")
            ...
            .expect("tenant context");
        let result = sqlx::query_scalar(
            "SELECT ple_commit_retention_work($1,$2,$3,$4,'archiveStudentRecords',$5)",
        )
        .bind(tenant)
        .bind(job)
        .bind(token)
    ...
    assert_eq!(
        retention_sdf(&pool, tenant, retention_job, lease, course, 1).await,
        None
    );
    let scheduled: String = sqlx::query_scalar("SELECT state FROM course_retention_stage WHERE tenant_id=$1 AND course_id=$2 AND stage='archiveStudentRecords'")
        .bind(tenant).bind(course).fetch_one(&pool).await.expect("early prepare leaves stage scheduled");
    assert_eq!(scheduled, "scheduled");
    sqlx::query("UPDATE course_retention_stage SET due_at = transaction_timestamp() WHERE tenant_id=$1 AND course_id=$2 AND stage='archiveStudentRecords'")
        .bind(tenant).bind(course).execute(&pool).await.expect("make stage due");
    for (job, token, generation) in [
        (Uuid::from_u128(993_099), lease, 1),
        (retention_job, Uuid::from_u128(993_098), 1),
        (retention_job, lease, 2),
    ] {
        assert_eq!(
            retention_sdf(&pool, tenant, job, token, course, generation).await,
            None
        );
        assert!(!retention_commit(&pool, tenant, job, token, course, generation).await);
    }
    assert_eq!(
        retention_sdf(&pool, foreign_tenant, retention_job, lease, course, 1).await,
        None
    );

    let prepared = retention_sdf(&pool, tenant, retention_job, lease, course, 1)
        .await
        .expect("exact due lease prepares cleanup");
    let objects = prepared
        .get("objects")
        .and_then(serde_json::Value::as_array)
        .expect("cleanup manifest objects");
    for object in expected {
        assert!(
            objects
                .iter()
                .any(|value| value.as_str() == Some(&object.to_string()))
        );
    }
    let queued_state: String =
        sqlx::query_scalar("SELECT state FROM student_export_request WHERE export_id=$1")
            .bind(queued_export)
            .fetch_one(&pool)
            .await
            .expect("queued export state");
    assert_eq!(queued_state, "failed");
    let queued_job_state: String =
        sqlx::query_scalar("SELECT state FROM worker_job WHERE job_id=$1")
            .bind(queued_export_job)
            .fetch_one(&pool)
            .await
            .expect("queued export job state");
    assert_eq!(queued_job_state, "dead");
    let ready_delivery: i64 =
        sqlx::query_scalar("SELECT count(*) FROM asset_delivery WHERE delivery_id=$1")
            .bind(ready_object)
            .fetch_one(&pool)
            .await
            .expect("delivery visibility");
    assert_eq!(
        ready_delivery, 0,
        "ready student-record delivery is revoked before cleanup"
    );
    let (left, right) = tokio::join!(
        retention_commit(&pool, tenant, retention_job, lease, course, 1),
        retention_commit(&pool, tenant, retention_job, lease, course, 1)
    );
    assert_eq!(
        usize::from(left) + usize::from(right),
        1,
        "only one finalization commits"
    );
    assert!(!retention_commit(&pool, tenant, retention_job, lease, course, 1).await);
    let stage_state: String = sqlx::query_scalar("SELECT state FROM course_retention_stage WHERE tenant_id=$1 AND course_id=$2 AND stage='archiveStudentRecords'")
        .bind(tenant).bind(course).fetch_one(&pool).await.expect("completed stage");
    assert_eq!(stage_state, "completed");
    let lifecycle: String = sqlx::query_scalar(
        "SELECT lifecycle FROM course_retention WHERE tenant_id=$1 AND course_id=$2",
    )
    .bind(tenant)
    .bind(course)
    .fetch_one(&pool)
    .await
    .expect("course lifecycle");
    assert_eq!(lifecycle, "active");

    for statement in [
        "INSERT INTO course_retention_notification (tenant_id,course_id,generation,intent) VALUES ('00000000-0000-0000-0000-000000000001','00000000-0000-0000-0000-000000000002',1,'archive')",
        "UPDATE course_retention_stage SET state='completed'",
        "UPDATE student_export_request SET state='failed'",
        "DELETE FROM asset_delivery",
    ] {
        let mut transaction = pool.begin().await.expect("fresh denied app transaction");
        sqlx::query("SET LOCAL ROLE ple_app")
            .execute(&mut *transaction)
            .await
            .expect("app role");
        ...
        sqlx::query(statement)
            .execute(&mut *transaction)
            .await
            .expect_err("raw retention mutation must be denied");
        transaction
            .rollback()
            .await
            .expect("denied raw transaction rolls back");
    }
}

#[cfg(feature = "postgres")]
#[tokio::test]
#[ignore = "requires dedicated app and QTI grader databases in PLE_POSTGRES_TEST_URL and PLE_GRADER_DATABASE_URL"]
async fn postgres_store_conforms_and_enforces_database_boundaries() {
    let database_url = std::env::var("PLE_POSTGRES_TEST_URL")
        .expect("set PLE_POSTGRES_TEST_URL to a dedicated empty PostgreSQL database");
    let grader_database_url = std::env::var("PLE_GRADER_DATABASE_URL")
        .expect("set PLE_GRADER_DATABASE_URL to the dedicated ple_qti_grader credential");
    ...
    exercise_postgres_statistics_broker_integrity(&pool).await;
    let issuer = PostgresStore::new(pool.clone());
    let qti_grader = store::postgres::PostgresQtiGraderStore::connect(&grader_database_url)
        .await
        .expect("dedicated QTI grader credential must satisfy least privilege checks");
    let next_replica = PostgresStore::new(pool.clone());
    exercise_store(&issuer).await;
    exercise_assignment_cas(&issuer).await;
    exercise_publication_identity_boundary(&issuer).await;
    exercise_source_artifact_binding(&issuer).await;
    exercise_catalog_store(&issuer).await;
    exercise_run_api_store(&issuer, FeedbackDisclosure::Deferred).await;
    exercise_run_api_store(&issuer, FeedbackDisclosure::OnRelease).await;
    exercise_external_tool_broker(&issuer).await;
    exercise_asset_store(&issuer).await;
    exercise_session_replicas(&issuer, &next_replica).await;
    exercise_job_store_claim_boundary(&issuer).await;
    exercise_qti_import_store(&issuer, &qti_grader).await;
    exercise_qti_published_grading_visibility(&issuer, &qti_grader).await;
    let published_qti_reference = ProblemVersionRef {
        problem: ProblemId::from_uuid(uuid(9_104)),
        version: VersionId::from_uuid(uuid(9_105)),
    };
    let foreign_qti_tenant = TenantId::from_uuid(uuid(9_108));
    sqlx::query(
        "UPDATE problem_version SET publication_scope = 'institution' \
         WHERE problem_id = $1 AND version_id = $2",
    )
    .bind(published_qti_reference.problem.as_uuid())
    .bind(published_qti_reference.version.as_uuid())
    .execute(&pool)
    .await
    .expect("live fixture may make the QTI version institution-only");
    assert_eq!(
        qti_grader
            .qti_published_grading(
                TenantContext::from_authenticated_session(foreign_qti_tenant),
                published_qti_reference,
                "item-1",
            )
            .await
            .expect("foreign institution-only grading read"),
        None,
        "the grader must not become an oracle for an ungranted institution version"
    );
    sqlx::query(
        "INSERT INTO catalog_tenant_grant (tenant_id, problem_id, version_id) \
         VALUES ($1, $2, $3)",
    )
    .bind(foreign_qti_tenant.as_uuid())
    .bind(published_qti_reference.problem.as_uuid())
    .bind(published_qti_reference.version.as_uuid())
    .execute(&pool)
    .await
    .expect("live fixture may grant the institution QTI version to the foreign tenant");
    assert!(
        qti_grader
            .qti_published_grading(
                TenantContext::from_authenticated_session(foreign_qti_tenant),
            ...
            .await
            .expect("granted institution grading read")
            .is_some(),
        "an exact ordinary catalog grant restores server-side grading access"
    );
    exercise_postgres_queue_privileges_and_payload_backstops(&pool).await;
    ...
    let visible_rows: i64 = sqlx::query_scalar(
        "SELECT \
         ...
         (SELECT count(*) FROM question_prefetch) + \
         (SELECT count(*) FROM submission_idempotency) + \
         (SELECT count(*) FROM submission_next_attempt) + \
         (SELECT count(*) FROM submission_receipt_snapshot) + \
         (SELECT count(*) FROM question_statistics_contribution_receipt) + \
         (SELECT count(*) FROM attempt_feedback) + \
         (SELECT count(*) FROM feedback_release) + \
         (SELECT count(*) FROM submission) + \
         (SELECT count(*) FROM grade_event) + \
         (SELECT count(*) FROM audit_event) + \
         (SELECT count(*) FROM asset_delivery) + \
         (SELECT count(*) FROM external_tool_exchange) + \
         (SELECT count(*) FROM external_tool_launch_session) + \
         (SELECT count(*) FROM workspace_qti_import) + \
         (SELECT count(*) FROM workspace_qti_import_item) + \
         (SELECT count(*) FROM workspace_qti_import_asset) + \
         (SELECT count(*) FROM workspace_qti_import_unsupported)",
    )
    ...
    let mut app_transaction = pool
        .begin()
        .await
        .expect("app-role transaction should start");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *app_transaction)
        .await
        .expect("migration login should be able to assume ple_app");
    sqlx::query("SELECT set_config('ple.tenant_id', $1, true)")
        .bind(TenantId::from_uuid(uuid(9_001)).to_string())
        .execute(&mut *app_transaction)
        .await
        .expect("app tenant context should be set locally");
    let qti_grading_read = sqlx::query("SELECT payload FROM workspace_qti_import_grading LIMIT 1")
        .execute(&mut *app_transaction)
        .await;
    let error = qti_grading_read.expect_err("ple_app must not read QTI grading material");
    let sqlx::Error::Database(database_error) = error else {
    ...
    app_transaction
        .rollback()
        .await
        .expect("aborted app transaction should roll back");

    let mut qti_student_transaction = pool
        .begin()
        .await
        .expect("QTI student-role transaction should start");
    sqlx::query("SET LOCAL ROLE ple_student")
        .execute(&mut *qti_student_transaction)
        .await
        .expect("migration login should be able to assume ple_student");
    let qti_grading_read = sqlx::query("SELECT payload FROM workspace_qti_import_grading LIMIT 1")
        .execute(&mut *qti_student_transaction)
        .await;
    let error = qti_grading_read.expect_err("student role must not read QTI grading material");
    let sqlx::Error::Database(database_error) = error else {
        panic!("expected PostgreSQL permission error, received {error}");
    };
    assert_eq!(database_error.code().as_deref(), Some("42501"));
    qti_student_transaction
        .rollback()
        .await
        .expect("aborted QTI student transaction should roll back");
    ...
    sqlx::query("SET LOCAL ROLE ple_student")
        ...
        .expect("migration login should be able to assume ple_student");
    sqlx::query("SELECT set_config('ple.tenant_id', $1, true)")
        .bind(TenantId::from_uuid(uuid(1)).to_string())
        .execute(&mut *student_transaction)
        .await
        .expect("student tenant context should be set locally");
    let workspace_draft_read =
        sqlx::query("SELECT payload FROM workspace_draft WHERE tenant_id = $1 LIMIT 1")
            .bind(TenantId::from_uuid(uuid(1)).as_uuid())
            .execute(&mut *student_transaction)
            .await;
    let error = workspace_draft_read
        .expect_err("same-tenant student must not read full private workspace draft payloads");
    let sqlx::Error::Database(database_error) = error else {
        panic!("expected PostgreSQL permission error, received {error}");
    };
    assert_eq!(database_error.code().as_deref(), Some("42501"));
    ...
    let mut student_transaction = pool
        .begin()
        .await
        .expect("student-role transaction should start for answer-key check");
    sqlx::query("SET LOCAL ROLE ple_student")
        .execute(&mut *student_transaction)
        .await
        .expect("migration login should be able to assume ple_student");
...
#[tokio::test]
#[ignore = "requires PLE_POSTGRES_TEST_URL with migration-role privileges and a dedicated empty database"]
async fn postgres_retention_r4_scheduler_dispatch_is_single_bound_and_generation_safe() {
    let database_url = std::env::var("PLE_POSTGRES_TEST_URL")
        .expect("set PLE_POSTGRES_TEST_URL to a dedicated empty PostgreSQL database");
    let pool = lazy_pool(&database_url).expect("valid PostgreSQL test URL");
    apply_migrations(&pool).await.expect("migrations apply");
    let store = PostgresStore::new(pool.clone());
    let tenant = TenantId::from_uuid(uuid(994_001));
    let context = TenantContext::from_authenticated_session(tenant);
    let course = CourseId::from_uuid(uuid(994_002));
    let administrator = UserId::from_uuid(uuid(994_003));
    let session = SessionTokenHash::compute(&[94; 32]);
    store
        .create_session(
            session,
            SessionSubject::new(
    ...
    store
        .upsert_course(
            ...
            CourseRecord {
                id: course,
                tenant,
                title: "Scheduler retention".to_string(),
                members: vec![CourseMembership {
                    user: administrator,
                    role: CourseMembershipRole::Instructor,
                }],
    ...
    store
        .configure_retention_policy(
            context,
            session,
            InstitutionRetentionPolicy::new(
                RetentionDays::new(1).unwrap(),
                RetentionDays::new(2).unwrap(),
                RetentionDays::new(3).unwrap(),
            )
            .unwrap(),
        )
        .await
        .expect("policy");
    store
        .end_course_retention(context, session, course)
        .await
        .expect("end");
    // A migration-role setup moves only this test course's notify deadline;
    // production dispatch always obtains time from the database transaction.
    sqlx::query(
        "UPDATE course_retention_stage SET due_at = transaction_timestamp() \
         WHERE tenant_id = $1 AND course_id = $2 AND stage = 'notify' AND generation = 1",
    )
    .bind(tenant.as_uuid())
    .bind(course.as_uuid())
    .execute(&pool)
    .await
    .expect("make notify due");
    let batch = RetentionDispatchBatch::new(10).expect("batch");
    let left_store = PostgresStore::new(pool.clone());
    let right_store = PostgresStore::new(pool.clone());
    let (left, right) = tokio::join!(
        left_store.dispatch_due_retention_stages(batch),
        right_store.dispatch_due_retention_stages(batch)
    );
    assert_eq!(
        left.expect("left dispatch") + right.expect("right dispatch"),
        1
    );
    let dispatched: i64 = sqlx::query_scalar(
        "SELECT count(*) FROM course_retention_dispatch \
         WHERE tenant_id = $1 AND course_id = $2 AND stage = 'notify' AND generation = 1",
    )
    .bind(tenant.as_uuid())
    .bind(course.as_uuid())
    ...
    let extended = store
        .extend_course_retention(context, session, course, RetentionDays::new(7).unwrap())
        .await
        .expect("administrator extension");
    assert_eq!(extended.snapshot.generation(), 2);
    let mut app = pool.begin().await.expect("application transaction");
    sqlx::query("SET LOCAL ROLE ple_app")
        .execute(&mut *app)
        .await
        .expect("app role");
    sqlx::query("INSERT INTO course_retention_dispatch (tenant_id, course_id, stage, generation, job_id) VALUES ($1,$2,'notify',2,$3)")
        .bind(tenant.as_uuid())
        .bind(course.as_uuid())
        .bind(uuid(994_004))
        .execute(&mut *app)
        .await
        .expect_err("ple_app must not forge retention dispatch rows");
    app.rollback().await.expect("close denied transaction");
    pool.close().await;
}
