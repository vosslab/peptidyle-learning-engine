...
pub struct ExternalToolLaunchReference(String);

impl ExternalToolLaunchReference {
...
impl std::fmt::Debug for ExternalToolLaunchReference {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str("ExternalToolLaunchReference(REDACTED)")
    }
}
...
impl<'a> ProxyRequest<'a> {
    fn activity_get(handle: &'a ExternalToolLaunchReference) -> Self {
        Self {
            handle,
    ...
    fn activity_post(
        handle: &'a ExternalToolLaunchReference,
        body: &'a [u8],
    ) -> Result<Self, ScoredEmbedTransportFailure> {
    ...
    #[allow(dead_code)]
    pub(crate) fn handle(&self) -> &ExternalToolLaunchReference {
        self.handle
    }
}
...
pub struct ContractedLaunchSession {
    ...
    handle: ExternalToolLaunchReference,
}
...
#[cfg(test)]
#[path = "external_question_provider/tests.rs"]
mod tests;
