...
use super::{
    ContractedLaunchSession, GradeBinding, ImathasAdapterError, LaunchLedgerStorageParts,
    ProviderLaunchHandle, ScoredEmbedFailure, ScoredEmbedLaunchLedger, map_transport,
};
...
impl LaunchSessionCodec {
    ...
    pub fn restore(
        ...
        let handle =
            ProviderLaunchHandle::from_server_handle(cursor.text()?).map_err(map_transport)?;
        if !cursor.finished()
