...
fn public_mapping() -> QtiPublicMappingDigestInput {
...
fn private_mapping() -> QtiPrivateMappingDigestInput {
    QtiPrivateMappingDigestInput::new(
        "choice_2".to_string(),
        "blue".to_string(),
        vec![QtiPrivateChoiceMapDigestInput::new(
            "choice_2".to_string(),
            "blue".to_string(),
        )],
        Vec::new(),
...
fn report(public_mapping: &QtiPublicMappingDigestInput) -> QtiProfileReportDigestInput {
    QtiProfileReportDigestInput {
        ...
        items: vec![QtiProfileItemDisposition {
            ...
            accepted: true,
            public_mapping_sha256: Some(public_mapping.digest().expect("public digest")),
            diagnostics: Vec::new(),
        }],
        defaults: Vec::new(),
    }
...
#[test]
fn integrity_digests_are_deterministic_and_private_debug_is_redacted() {
    let public_mapping = public_mapping();
    let report = report(&public_mapping);
    let private_mapping = private_mapping();
    let first = QtiImportIntegrityDigests::compute(&report, &public_mapping, &private_mapping)
        .expect("contract encodes");
    let second = QtiImportIntegrityDigests::compute(&report, &public_mapping, &private_mapping)
        .expect("same contract encodes");
    assert_eq!(first, second);
    assert!(!format!("{first:?}").contains("choice_2"));
    assert!(!format!("{first:?}").contains(&first.mapping_sha256.to_string()));
}

#[test]
fn digest_contract_refuses_dispositions_and_private_binding_contradictions() {
    let public_mapping = public_mapping();
    let mut invalid_report = report(&public_mapping);
    invalid_report.items[0].public_mapping_sha256 = None;
    assert_eq!(
        QtiImportIntegrityDigests::compute(&invalid_report, &public_mapping, &private_mapping()),
        Err(QtiProfileContractError::ItemDigestDisposition)
    );
    let report = report(&public_mapping);
    let missing_binding = QtiPrivateMappingDigestInput::new(
        "choice_2".to_string(),
        "blue".to_string(),
        Vec::new(),
        Vec::new(),
    ...
    assert_eq!(
        QtiImportIntegrityDigests::compute(&report, &public_mapping, &missing_binding),
        Err(QtiProfileContractError::PrivateBindingMissing)
    );
}

#[test]
fn digest_contract_is_sensitive_to_one_field_and_choice_order() {
    ...
    reordered.choices.push(QtiPublicChoiceDigestInput {
        ple_choice_id: "red".to_string(),
        text_markdown: "Red".to_string(),
    });
    let ordered = reordered.digest().expect("digest");
    reordered.choices.reverse();
    assert_ne!(ordered, reordered.digest().expect("digest"));
}

#[test]
fn public_mapping_canonical_encoding_and_digest_are_golden() {
    let public_mapping = public_mapping();
    ...
    assert_eq!(
        public_mapping.digest().expect("digest").to_string(),
        "e511e37a056973d2f21e3522f6c8362603e0de4fc807c8ccd006b32515db39c0"
    );
}

#[test]
fn generic_profile_uses_generic_compatibility_for_a_valid_digest_contract() {
    let public_mapping = public_mapping();
    ...
    assert!(
        QtiImportIntegrityDigests::compute(&report, &public_mapping, &private_mapping()).is_ok()
    );
}
