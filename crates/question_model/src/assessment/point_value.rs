//! Exact Assessment-owned point values.
...
/// Exact Assessment-owned point value with four decimal places of precision.
///
...
pub struct AssessmentPointValue(i64);

impl AssessmentPointValue {
...
impl std::fmt::Display for AssessmentPointValue {
...
impl FromStr for AssessmentPointValue {
    type Err = &'static str;
...
impl TryFrom<String> for AssessmentPointValue {
...
impl From<AssessmentPointValue> for String {
    fn from(value: AssessmentPointValue) -> Self {
        value.to_string()
    }
}
...
mod tests {
    ...
    fn point_values_round_trip_without_binary_floating_point() {
        ...
        for invalid in ["", "-1", "+1", "1.00001", "NaN", "1000000001"] {
            assert!(
                invalid.parse::<AssessmentPointValue>().is_err(),
                "{invalid}"
            );
        }
    }
    ...
    fn point_values_add_and_multiply_exact_scaled_values() {
        let one_and_a_half = AssessmentPointValue::from_scaled(15_000).expect("in range");
        let quarter = AssessmentPointValue::from_scaled(2_500).expect("in range");

        assert_eq!(
            one_and_a_half
                .checked_add(quarter)
                .map(AssessmentPointValue::scaled),
            Some(17_500)
        );
        assert_eq!(
            quarter.checked_mul_u32(6).map(AssessmentPointValue::scaled),
            Some(15_000)
        );
    }
    ...
    fn point_values_reject_negative_and_overflowing_scaled_arithmetic() {
        let maximum =
            AssessmentPointValue::from_scaled(MAX_SCALED_POINTS).expect("maximum is valid");
        let smallest = AssessmentPointValue::from_scaled(1).expect("smallest is valid");

        assert!(AssessmentPointValue::from_scaled(-1).is_none());
        assert!(
            AssessmentPointValue::from_scaled(MAX_SCALED_POINTS.checked_add(1).expect("i64 room"))
                .is_none()
        );
