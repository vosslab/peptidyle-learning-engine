//! Safe provenance for server-resolved teaching-policy fields.
...
mod tests {
    ...
    #[test]
    fn accommodation_provenance_contains_only_the_direct_membership() {
        let value = serde_json::to_value(TeachingPreviewFieldSource::Accommodation {
            membership: "M-4".parse().expect("membership reference"),
            label: TeachingDisplayLabel::try_from("Extended due time".to_owned())
                .expect("label"),
        })
        .expect("wire");
