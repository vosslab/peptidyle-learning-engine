...
use learning_data_access::{
    StoreError, StudentAssignmentAttemptFinalizationBackend,
    StudentAssignmentAttemptFinalizationEvaluation, StudentAssignmentAttemptFinalizationSource,
};
use objects::s3::S3ObjectStore;
use question_model::{
    ObjectId, QuestionRevisionNumber, QuestionRevisionReference, SourceObjectChecksum,
    SourceObjectReference, generation::QuestionSeed,
};
use uuid::Uuid;
...
pub(crate) async fn evaluate_saved_responses(
    ...
    responses: &[StudentAssignmentAttemptFinalizationSource],
) -> Result<Vec<StudentAssignmentAttemptFinalizationEvaluation>, StoreError> {
    let mut evaluations = Vec::with_capacity(responses.len());
    for response in responses {
        evaluations.push(StudentAssignmentAttemptFinalizationEvaluation {
...
async fn evaluate_one(
    objects: &S3ObjectStore,
    webwork: &WebworkAdapter<HttpWebworkRenderer>,
    response: &StudentAssignmentAttemptFinalizationSource,
    ...
    match &response.backend {
        StudentAssignmentAttemptFinalizationBackend::Ple => {
            let source =
        ...
        StudentAssignmentAttemptFinalizationBackend::Webwork { pg_path } => {
            ...
            match webwork
                .grade(
                    QuestionSeed::new(response.question_seed),
                    &source,
