//! Student-safe assignment and aggregate-progress route projections.
...
use learning_data_access::{
    ...
    ResolveEffectivePolicyCommand, SessionStore, Store, StoreError, StoredAssignment,
    StudentAssignmentSummarySnapshot,
};
use question_model::AssignmentId;

use crate::auth::{auth_error_response, no_store, resolve_request_session};
use crate::run::support::student_assignment_progress;
...
/// Student view. Student-specific late status remains part of Student detail.
...
/// ordinary Student detail and the Instructor's stable-identity Student view.
...
pub(in crate::course) async fn get_student_assignment<S>(
        ...
        + 'static,
{
    ...
    if member_role == question_model::CourseMembershipRole::Student {
        ...
        let prior_run_count = match state
            .store
            .student_get_enrollment_for_assignment(
                authenticated.tenant_context,
                authenticated.record.subject.user(),
                assignment.record.id,
            )
        ...
        return student_assignment_detail_response(
            &state,
            &authenticated,
            assignment,
            course.term.time_zone().clone(),
            question_model::course::StudentAssignmentDelivery {
                ...
                late_status: match late {
                    domain::effective_assignment_policy::LateVerdict::OnTime => {
                        question_model::course::StudentLateStatus::OnTime
                    }
                    domain::effective_assignment_policy::LateVerdict::AcceptedLate => {
                        question_model::course::StudentLateStatus::AcceptedLate
                    }
                    domain::effective_assignment_policy::LateVerdict::MarkedLate => {
                        question_model::course::StudentLateStatus::MarkedLate
                    }
...
async fn student_assignment_detail_response<S>(
    state: &CourseRouteState<S>,
    authenticated: &crate::auth::AuthenticatedSession,
    assignment: StoredAssignment,
    time_zone: question_model::IanaTimeZone,
    delivery: question_model::course::StudentAssignmentDelivery,
) -> Response
where
    S: Store + CatalogStore + SessionStore + 'static,
{
    ...
    no_store(
        Json(
            question_model::course::StudentAssignmentDetail::from_landing(
                assignment
                    .record
        ...
        + 'static,
{
        ...
        .await
    {
        ...
        Ok(None) => {
            return match student_assignment_progress(
                state.store.as_ref(),
                &authenticated,
                assignment,
                None,
...
enum MaterializedSummaryLookup {
    Found(StudentAssignmentSummarySnapshot),
    Failed(Response),
}

fn required_materialized_summary(
    result: Result<Option<StudentAssignmentSummarySnapshot>, StoreError>,
