...
use super::{id, issued_cookie_for_tenant, policies, publish_fixture};
use crate::course::{
    CourseInvitationDelivery, CourseInvitationDeliveryError, CourseInvitationIssuer,
    CourseInvitationSecret, LocalDevelopmentRosterDirectory, LocalDevelopmentRosterIdentity,
...
async fn roster_http_is_manager_scoped_secret_free_and_idempotent() {
    ...
    let local_development_route = app
        .clone()
        .oneshot(request(
            "POST",
            format!("/api/courses/{course}/local-development-members"),
            &instructor_cookie,
            Some(serde_json::json!({ "learnerAlias": "student-local" })),
        ))
        .await
        .expect("production-style local roster route response");
    assert_eq!(local_development_route.status(), StatusCode::NOT_FOUND);
...
#[tokio::test]
async fn local_development_activation_is_manager_scoped_alias_only_and_idempotent() {
    let store = std::sync::Arc::new(learning_data_access::in_memory::MemoryStore::default());
    let tenant = TenantId::from_uuid(id(1_150));
    let instructor = UserId::from_uuid(id(1_151));
    let learner = UserId::from_uuid(id(1_152));
    let outsider = UserId::from_uuid(id(1_153));
    let conflicting_instructor = UserId::from_uuid(id(1_157));
    ...
    store
        .upsert_course(
            ...
            CourseRecord {
                id: course,
                tenant,
                title: "Local roster course".to_string(),
                members: vec![
                    CourseMembership {
                        user: instructor,
                        role: CourseMembershipRole::Instructor,
    ...
    store
        .create_untimed_assignment(
            ...
            AssignmentRecord {
                ...
                tenant,
                course_id: course,
                title: "Existing local assignment".to_string(),
                items: vec![question_model::AssignmentItem {
                    id: question_model::AssignmentItemId::from_uuid(id(1_156)),
                    reference,
                    position: 0,
    ...
    store
        .create_untimed_assignment(
            ...
            AssignmentRecord {
                ...
                tenant,
                course_id: course,
                title: "Second existing local assignment".to_string(),
                items: vec![question_model::AssignmentItem {
                    id: question_model::AssignmentItemId::from_uuid(id(1_159)),
                    reference,
                    position: 0,
                    points_possible: question_model::PointValue::from_whole(1),
                    delivery_state: question_model::AssignmentDeliveryState::Active,
        ...
        .await
        .expect("second assignment fixture");
    let instructor_cookie =
        issued_cookie_for_tenant(&store, tenant, vec![UserRole::Instructor], instructor).await;
    let outsider_cookie =
        issued_cookie_for_tenant(&store, tenant, vec![UserRole::Instructor], outsider).await;
    let directory = LocalDevelopmentRosterDirectory::new([
        (
            "student-local".to_string(),
            LocalDevelopmentRosterIdentity {
                tenant,
                user: learner,
                display_name: "Local Learner".to_string(),
                roles: vec![UserRole::Student],
            },
        ),
        (
            "instructor-local".to_string(),
            LocalDevelopmentRosterIdentity {
                tenant,
                user: conflicting_instructor,
                display_name: "Wrong role learner".to_string(),
                roles: vec![UserRole::Student],
            },
        ),
    ])
    .expect("local learner directory");
    let app = router_with_invitations_and_local_development(
        std::sync::Arc::clone(&store),
    ...
    let missing = app
        ...
        .oneshot(request(
            ...
            format!("/api/courses/{course}/local-development-members"),
            &instructor_cookie,
            Some(serde_json::json!({ "learnerAlias": "not-configured" })),
        ))
        .await
        .expect("unknown local alias response");
    ...
    let forbidden = app
        ...
        .await
        .expect("outsider local activation response");
    ...
    let (first, second) = tokio::join!(activate(), activate());
    let mut accepted_bodies = Vec::new();
    for accepted in [first, second] {
        let accepted = accepted.expect("local activation response");
        assert_eq!(accepted.status(), StatusCode::OK);
        accepted_bodies.push(json(accepted).await);
    }
    assert_eq!(accepted_bodies[0], accepted_bodies[1]);
    assert_eq!(accepted_bodies[0]["member"]["source"], "localDevelopment");
    assert!(accepted_bodies[0].to_string().find("userId").is_none());
    assert!(accepted_bodies[0].to_string().find("studentId").is_none());
    ...
    let conflicting = app
        ...
        .oneshot(request(
            ...
            format!("/api/courses/{course}/local-development-members"),
            &instructor_cookie,
            Some(serde_json::json!({ "learnerAlias": "instructor-local" })),
        ))
        .await
        .expect("conflicting membership response");
    assert_eq!(conflicting.status(), StatusCode::CONFLICT);
    let roster_after_conflict = app
        .clone()
        .oneshot(request(
            "GET",
            format!("/api/courses/{course}/roster"),
        ...
        .await
        .expect("rolled back local roster response");
    assert_eq!(
        json(roster_after_conflict).await["members"]
            .as_array()
            .expect("members")
            .len(),
        1,
        "a conflicting course role must roll back the local roster row"
    );

    let gradebook = app
        .oneshot(request(
            "GET",
            format!("/api/courses/{course}/gradebook"),
            &instructor_cookie,
            None,
        ))
        .await
